# J-Quants株価予測システム v2.2 - 感情分析統合版

J-Quants APIを使用して株価データを取得し、機械学習で株価予測を行う**完全統合システム**です。

## 🆕 新機能: 高度なバックテストシステム

**月間10-20%の利益向上を目指す高度なバックテストシステム**を追加しました。

### 📈 高度なバックテスト機能

- **複数戦略バックテスト**: モメンタム、平均回帰、ブレイクアウト戦略の同時実行
- **戦略最適化**: 遺伝的アルゴリズム、グリッドサーチによるパラメータ最適化
- **ポートフォリオ最適化**: シャープレシオ最大化、リスクパリティ配分
- **詳細パフォーマンス分析**: VaR、CVaR、ストレステスト、統計指標
- **包括的可視化**: エクイティカーブ、ドローダウン、リスク分析グラフ
- **統合システム**: 既存のリアルタイムシグナルシステムとの完全統合

## 🚀 新アーキテクチャ（v2.2） - コードスタイル統一・警告フィルタリング改善版
...
2. **感情分析統合システム** (`integrated_sentiment_system.py`) - 感情分析・ニュース統合システム
3. **拡張感情分析システム** (`enhanced_sentiment_trading.py`) - 拡張感情分析トレーディング
4. **感情分析システム** (`sentiment_analysis_system.py`) - 基盤感情分析エンジン
   - 統合エラーハンドリング・ログシステム
   - 統合設定管理
   - セキュアな認証処理
   - 堅牢なデータ取得処理
...
2. **統合設定管理** (`config_final.yaml`) - 単一設定ファイル
   ...
   - **感情分析設定** (`sentiment_config.yaml`) - 感情分析専用設定
