...
import logging
from risk_management_system import RiskManagementSystem, Position, RiskLevel, PositionStatus
import os
...
class RiskDashboardDataGenerator:
    ...
    def generate_portfolio_overview(self) -> Dict[str, Any]:
        ...
        total_exposure = sum(pos.current_price * pos.quantity for pos in self.risk_system.positions.values())
        total_unrealized_pnl = sum(pos.unrealized_pnl for pos in self.risk_system.positions.values())
    ...
    def generate_positions_data(self) -> List[Dict[str, Any]]:
        ...
        for symbol, position in self.risk_system.positions.items():
            ...
            if position.position_type == "LONG":
                stop_distance = (position.current_price - position.stop_loss_price) / position.current_price * 100
            else:
                stop_distance = (position.stop_loss_price - position.current_price) / position.current_price * 100
            
            # 利確までの距離計算
            if position.position_type == "LONG":
                profit_distance = (position.take_profit_price - position.current_price) / position.current_price * 100
            else:
                profit_distance = (position.current_price - position.take_profit_price) / position.current_price * 100
            ...
            position_data = {
                ...
                "unrealized_pnl_percent": (position.unrealized_pnl / (position.entry_price * position.quantity)) * 100,
                "pnl_color": pnl_color,
                ...
                "weight": (position.current_price * position.quantity) / self.account_value * 100
            }
    ...
    def generate_risk_metrics_chart(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_portfolio_value": self.account_value + sum(pos.unrealized_pnl for pos in self.risk_system.positions.values()),
    ...
    def generate_risk_alerts(self) -> List[Dict[str, Any]]:
        ...
        for symbol, position in self.risk_system.positions.items():
            ...
            if position.unrealized_pnl < -position.entry_price * position.quantity * 0.1:  # 10%以上の損失
                alerts.append({
