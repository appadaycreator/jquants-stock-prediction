...
class UnifiedErrorHandler:
    ...
    def _prepare_context(self, error: Exception, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """コンテキスト情報の準備"""
        context_info = {
    ...
    def _log_error(self, error: Exception, category: ErrorCategory, 
                  context_info: Dict[str, Any], level: LogLevel) -> None:
        """エラーのログ出力"""
    ...
    def _attempt_recovery(self, error: Exception, category: ErrorCategory, 
        ...
        try:
            if category == ErrorCategory.API_ERROR:
                ...
                self.logger.warning(f"No specific recovery strategy for {category.value}")
                
        except Exception as recovery_error:
            self.logger.error(f"Recovery attempt failed: {recovery_error}")
    
    def _recover_api_error(self, error: Exception, context_info: Dict[str, Any]) -> None:
        """APIエラーの復旧"""
        self.logger.info("Attempting API error recovery...")
        # APIエラーの復旧ロジックを実装
        pass
    
    def _recover_file_error(self, error: Exception, context_info: Dict[str, Any]) -> None:
        """ファイルエラーの復旧"""
        self.logger.info("Attempting file error recovery...")
        # ファイルエラーの復旧ロジックを実装
        pass
    
    def _recover_data_processing_error(self, error: Exception, context_info: Dict[str, Any]) -> None:
        """データ処理エラーの復旧"""
    ...
    def log_error(self, error: Exception, message: str, context: Optional[Dict[str, Any]] = None) -> None:
        """エラーログの出力"""
        try:
            context_info = self._prepare_context(error, context)
    ...
    def handle_data_error(self, error: Exception, operation: str, data_shape: tuple, column: str = None) -> None:
        """データエラーの処理"""
        try:
            context = {
                'operation': operation,
