...
# 既存システムのインポート
try:
    from enhanced_individual_stock_monitor import EnhancedIndividualStockMonitor, AlertType, AlertPriority
...
class WebMonitoringIntegration:
    ...
    def check_alerts(self, stock: Dict, stock_data: Dict) -> List[str]:
        ...
        # 価格変動アラート
        if abs(stock_data["changePercent"]) >= config.get("priceChangeThreshold", 3.0):
                ...
                for stock in active_stocks:
                    try:
                        ...
                        if stock_data:
                            ...
                            logger.info(f"監視更新: {stock['code']} - {stock_data['changePercent']:+.2f}%")
