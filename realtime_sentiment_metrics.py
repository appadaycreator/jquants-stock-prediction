...
class RealtimeSentimentMetricsGenerator:
        ...
        for symbol in symbols:
            try:
                ...
                risk_level = await self._calculate_risk_level(
                    symbol, sentiment_score, volatility_index
                )
    ...
    async def _calculate_risk_level(
        self, symbol: str, sentiment: float, volatility: float
    ) -> float:
        """リスクレベルの計算"""
        try:
    ...
    async def _calculate_momentum(self, symbol: str) -> float:
        ...
        try:
            ...
            # 価格モメンタムの計算
            momentum = (hist["Close"].iloc[-1] - hist["Close"].iloc[-5]) / hist[
                "Close"
            ].iloc[-5]
    ...
    def _calculate_confidence(
        self, symbol: str, metric_type: MetricType, value: float
    ) -> float:
        """信頼度の計算"""
        try:
            # 履歴データに基づく信頼度計算
    ...
    async def _update_trends(self, symbol: str, metrics: List[RealtimeSentimentMetric]):
        ...
        try:
            sentiment_metrics = [
                m for m in metrics if m.metric_type == MetricType.SENTIMENT_SCORE
            ]
            ...
            # ボラティリティの更新
            volatility_metrics = [
                m for m in metrics if m.metric_type == MetricType.VOLATILITY_INDEX
            ]
    ...
    def visualize_metrics(self, symbol: str, save_path: str = None):
        ...
        try:
            ...
            sentiment_scores = [
                ...
                if m.metric_type == MetricType.SENTIMENT_SCORE
            ]
            volatility_scores = [
                m.value
                for m in symbol_metrics
                if m.metric_type == MetricType.VOLATILITY_INDEX
            ]
            ...
            if sentiment_scores:
                axes[0, 0].plot(
                    timestamps[: len(sentiment_scores)],
                    sentiment_scores,
                    "b-",
                    linewidth=2,
                )
            ...
            if volatility_scores:
                axes[0, 1].plot(
                    timestamps[: len(volatility_scores)],
                    volatility_scores,
                    "r-",
                    linewidth=2,
                )
            ...
            risk_scores = [
                m.value
                for m in symbol_metrics
                if m.metric_type == MetricType.RISK_LEVEL
            ]
            if risk_scores:
                axes[1, 0].plot(
                    timestamps[: len(risk_scores)], risk_scores, "orange", linewidth=2
                )
            ...
            # モメンタムのプロット
            momentum_scores = [
                m.value for m in symbol_metrics if m.metric_type == MetricType.MOMENTUM
            ]
            if momentum_scores:
                axes[1, 1].plot(
                    timestamps[: len(momentum_scores)],
                    momentum_scores,
                    "g-",
                    linewidth=2,
                )
