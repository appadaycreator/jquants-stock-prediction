...
class RiskDashboardRealtimeUpdater:
    ...
    def _update_loop(self):
        ...
        while self.is_running:
            try:
                ...
                logger.info(
                    f"リスク管理データを更新しました: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def _add_alert(
        self, alert_id: str, alert_type: str, title: str, message: str, priority: str
    ):
        """アラートを追加"""
        # 重複チェック（同じアラートIDが既に存在する場合はスキップ）
        existing_alert = next(
            (alert for alert in self.alert_history if alert["id"] == alert_id), None
        )
        if existing_alert:
            # 既存のアラートのタイムスタンプを更新
    ...
    def _save_realtime_data(self):
        ...
        with open(
            "web-app/public/data/risk_realtime_data.json", "w", encoding="utf-8"
        ) as f:
