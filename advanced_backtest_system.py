...
class BaseStrategy(ABC):
    ...
    def calculate_position_size(
        self, price: float, signal_strength: float = 1.0
    ) -> int:
        """ポジションサイズ計算"""
...
class BacktestEngine:
    ...
    def _process_day(
        self, date: datetime, row: pd.Series, strategy: BaseStrategy, data: pd.DataFrame
    ):
        """1日の処理"""
        # 既存ポジションのエグジット判定
        positions_to_close = []
    ...
    def _execute_trade(
        self, date: datetime, row: pd.Series, signal: int, strategy: BaseStrategy
    ):
        """取引実行"""
        price = row["Close"]
    ...
    def _open_long_position(
        self, date: datetime, price: float, quantity: int, strategy_name: str
    ):
        """ロングポジション開設"""
    ...
    def _close_position(
        self, symbol: str, row: pd.Series, strategy: BaseStrategy, reason: str
    ):
        """ポジションクローズ"""
        ...
        if position.quantity > 0:  # ロング
            self.current_capital += (
                exit_price * position.quantity * (1 - self.commission)
            )
        else:  # ショート
            self.current_capital += (
                position.entry_price * abs(position.quantity) * (1 - self.commission)
            )
    ...
    def _update_equity_curve(self):
        ...
        for position in self.positions.values():
            if position.quantity > 0:  # ロング
                ...
                unrealized_pnl = (position.entry_price - position.current_price) * abs(
                    position.quantity
                )
    ...
    def _calculate_results(
        self, strategy: BaseStrategy, start_date: datetime, end_date: datetime
    ) -> BacktestResult:
        """結果計算"""
    ...
    def plot_equity_curves(self, results: List[BacktestResult], save_path: str = None):
        ...
        for result in results:
            ...
            plt.plot(
                result.equity_curve.index,
                drawdown,
                label=f"{result.strategy_name} (Max: {result.max_drawdown:.1%})",
                linewidth=2,
            )
        ...
        bars = plt.bar(
            strategies, sharpe_ratios, color=["skyblue", "lightcoral", "lightgreen"]
        )
