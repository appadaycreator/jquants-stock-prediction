...
class UnifiedPerformanceSystem:
        ...
        try:
            ...
            if self.config.enable_memory_optimization and self.memory_optimizer:
                with self.memory_optimizer.memory_monitoring("memory_optimization"):
                    optimized_data = self.memory_optimizer.optimize_dataframe_aggressive(optimized_data)
            ...
            if self.config.enable_parallel_processing and self.parallel_processor:
                with parallel_context(max_workers=self.config.max_workers) as processor:
                    # データをチャンクに分割して並列処理
                    chunk_size = max(1000, len(optimized_data) // self.config.max_workers)
                    chunks = [
            ...
            memory_reduction = self._calculate_memory_reduction(start_memory, end_memory)
            speed_improvement = self._calculate_speed_improvement(execution_time, len(data))
            ...
            result = OptimizationResult(
                ...
                metrics={
                    ...
                    "target_achieved": chart_render_time <= self.config.target_render_time
                }
            )
    ...
    def _calculate_memory_reduction(self, start_memory: float, end_memory: float) -> float:
        """メモリ削減率を計算"""
        if start_memory > 0:
            return max(0, (start_memory - end_memory) / start_memory * 100)
        return 0

    def _calculate_speed_improvement(self, execution_time: float, data_size: int) -> float:
        """処理速度向上率を計算"""
        if execution_time > 0:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        successful_optimizations = sum(1 for h in self.optimization_history if h["result"].success)
        success_rate = (successful_optimizations / total_optimizations * 100) if total_optimizations > 0 else 0
        
        avg_execution_time = np.mean([h["result"].execution_time for h in self.optimization_history])
        avg_memory_reduction = np.mean([h["result"].memory_reduction_percent for h in self.optimization_history])
        avg_speed_improvement = np.mean([h["result"].processing_speed_improvement for h in self.optimization_history])
    ...
    def _validate_dod(self, result: OptimizationResult) -> Dict[str, bool]:
        ...
        return {
            ...
            "overall_success": (
                result.memory_reduction_percent >= 30 and
                result.processing_speed_improvement >= 2.0 and
                result.chart_render_time <= 3.0
            )
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not latest_result.success:
            recommendations.append("最適化に失敗しました。エラーログを確認してください。")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n🎯 DoD検証結果:")
        print(f"   - メモリ削減30%以上: {'✅' if dod_validation.get('memory_reduction_30_percent', False) else '❌'}")
        print(f"   - 処理速度2倍以上: {'✅' if dod_validation.get('processing_speed_2x', False) else '❌'}")
        print(f"   - チャート描画3秒以内: {'✅' if dod_validation.get('chart_render_3_seconds', False) else '❌'}")
        print(f"   - 総合判定: {'✅ 成功' if dod_validation.get('overall_success', False) else '❌ 失敗'}")
