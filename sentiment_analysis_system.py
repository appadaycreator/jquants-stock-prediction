...
class SentimentAnalyzer:
    ...
    def _load_stock_keywords(self) -> List[str]:
        """株式関連キーワードの読み込み"""
        return [
            'stock', 'stocks', 'equity', 'equities', 'market', 'trading', 'investment',
            'portfolio', 'dividend', 'earnings', 'revenue', 'profit', 'loss', 'bull',
            'bear', 'rally', 'crash', 'volatility', 'analyst', 'upgrade', 'downgrade',
            'buy', 'sell', 'hold', 'price', 'valuation', 'growth', 'decline'
        ]
    ...
    def calculate_relevance_score(self, text: str) -> float:
        ...
        try:
            ...
            keyword_matches = sum(1 for keyword in self.stock_keywords if keyword in text_lower)
    ...
    def extract_keywords(self, text: str, max_keywords: int = 10) -> List[str]:
        ...
        try:
            ...
            word_freq = {}
            for word in words:
                if len(word) > 3 and word not in ['this', 'that', 'with', 'from', 'they', 'have', 'been', 'were', 'said', 'each', 'which', 'their', 'time', 'will', 'about', 'there', 'could', 'other', 'after', 'first', 'well', 'also', 'where', 'much', 'some', 'very', 'when', 'here', 'just', 'into', 'your', 'work', 'know', 'year', 'take', 'than', 'its', 'over', 'think', 'also', 'back', 'after', 'use', 'two', 'how', 'our', 'work', 'life', 'only', 'new', 'way', 'may', 'say', 'each', 'which', 'she', 'do', 'how', 'their', 'if', 'up', 'out', 'many', 'then', 'them', 'can', 'only', 'other', 'new', 'some', 'what', 'time', 'there', 'very', 'when', 'much', 'then', 'them', 'can', 'only', 'other', 'new', 'some', 'what', 'time', 'there', 'very', 'when', 'much']:
...
class NewsAnalyzer:
    ...
    async def fetch_news(self, query: str, language: str = 'en', 
    ...
    async def analyze_news_sentiment(self, articles: List[Dict]) -> List[NewsSentiment]:
        ...
        for article in articles:
            try:
                ...
                # 感情分析実行
                sentiment_score, sentiment_type, confidence = self.sentiment_analyzer.analyze_text_sentiment(content)
                
                # 関連性スコア計算
                relevance_score = self.sentiment_analyzer.calculate_relevance_score(content)
...
class SNSTrendAnalyzer:
    ...
    async def analyze_trending_hashtags(self, stock_symbols: List[str]) -> List[SNSTrend]:
        """トレンドハッシュタグの分析"""
        trends = []
        ...
        for symbol in stock_symbols:
            try:
                ...
                sentiment_score, sentiment_type, confidence = self.sentiment_analyzer.analyze_text_sentiment(combined_text)
                
                # トレンドスコアの計算
                trend_score = min(mention_count / 100, 1.0) * confidence
                
                # 関連株式の特定
                related_stocks = self._identify_related_stocks(combined_text, stock_symbols)
    ...
    def _identify_related_stocks(self, text: str, stock_symbols: List[str]) -> List[str]:
        """関連株式の特定"""
        related = []
...
class SentimentTradingSystem:
    ...
    async def generate_sentiment_signals(self, stock_symbols: List[str]) -> Dict[str, Any]:
        """感情分析シグナルの生成"""
        try:
            # ニュース感情分析
            news_sentiments = []
            ...
            # SNSトレンド分析
            sns_trends = await self.sns_analyzer.analyze_trending_hashtags(stock_symbols)
            
            # 統合感情指標の計算
            overall_sentiment = self._calculate_overall_sentiment(news_sentiments, sns_trends)
            
            # トレーディングシグナルの生成
            trading_signals = self._generate_trading_signals(overall_sentiment, stock_symbols)
    ...
    def _calculate_overall_sentiment(self, news_sentiments: List[NewsSentiment], 
                                   sns_trends: List[SNSTrend]) -> Dict[str, Any]:
        """統合感情指標の計算"""
        try:
            # ニュース感情スコアの計算
            news_scores = [ns.sentiment_score * ns.relevance_score for ns in news_sentiments]
    ...
    def _generate_trading_signals(self, overall_sentiment: Dict[str, Any], 
    ...
    def get_sentiment_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'positive_ratio': len([s for s in recent_scores if s > 0.1]) / len(recent_scores),
                'negative_ratio': len([s for s in recent_scores if s < -0.1]) / len(recent_scores)
            }
