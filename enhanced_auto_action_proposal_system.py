...
class EnhancedAutoActionProposalSystem:
        ...
        default_config = {
            "action_proposal": {
                ...
                "market_conditions": {
                    ...
                    "volatile_market": {"cautious_actions": True, "risk_multiplier": 0.6}
                }
            }
        }
    ...
    async def generate_action_proposals(self, context: AnalysisContext) -> List[ActionProposal]:
    ...
    async def _generate_technical_proposals(self, context: AnalysisContext) -> List[ActionProposal]:
        """技術分析に基づく提案の生成"""
        proposals = []
            ...
            if 'rsi' in indicators:
                ...
                if rsi < 30:  # オーバーセル
                    proposal = self._create_proposal_from_template(
                        "buy_signal", context, {
                            "symbol": context.target_symbols[0] if context.target_symbols else "対象銘柄",
                            "rsi_value": rsi
                        }
                    )
    ...
    async def _generate_risk_proposals(self, context: AnalysisContext) -> List[ActionProposal]:
        """リスク管理に基づく提案の生成"""
        proposals = []
    ...
    async def _generate_market_proposals(self, context: AnalysisContext) -> List[ActionProposal]:
        """市場環境に基づく提案の生成"""
        proposals = []
    ...
    async def _generate_portfolio_proposals(self, context: AnalysisContext) -> List[ActionProposal]:
        """ポートフォリオ最適化に基づく提案の生成"""
        proposals = []
    ...
    def _create_proposal_from_template(self, template_name: str, context: AnalysisContext, variables: Dict[str, Any]) -> ActionProposal:
        """テンプレートからプロポーザルを作成"""
        ...
        # 信頼度スコアの計算
        confidence_score = self._calculate_confidence_score(action_type, context, variables)
        ...
        proposal = ActionProposal(
            ...
            reasoning=self._generate_reasoning(template.get('reasoning_templates', []), variables),
    ...
    def _determine_priority(self, action_type: ActionType, context: AnalysisContext, variables: Dict[str, Any]) -> ActionPriority:
        """優先度の決定"""
        # リスク管理は常に高優先度
    ...
    def _adjust_risk_level(self, base_risk: float, context: AnalysisContext, variables: Dict[str, Any]) -> float:
        """リスクレベルの調整"""
        # 市場状況による調整
        if context.market_condition == MarketCondition.VOLATILE:
            base_risk *= 1.2
    ...
    def _calculate_confidence_score(self, action_type: ActionType, context: AnalysisContext, variables: Dict[str, Any]) -> float:
        """信頼度スコアの計算"""
        base_confidence = 0.7
    ...
    def _generate_reasoning(self, templates: List[str], variables: Dict[str, Any]) -> List[str]:
        """推論の生成"""
        reasoning = []
    ...
    def _generate_prerequisites(self, action_type: ActionType, context: AnalysisContext) -> List[str]:
        """前提条件の生成"""
        prerequisites = []
    ...
    def _generate_follow_up_actions(self, action_type: ActionType, context: AnalysisContext) -> List[str]:
        """フォローアップアクションの生成"""
        follow_ups = []
    ...
    def _calculate_deadline(self, action_type: ActionType, context: AnalysisContext) -> Optional[datetime]:
        """期限の計算"""
        if action_type == ActionType.RISK_MANAGEMENT:
    ...
    def _filter_and_rank_proposals(self, proposals: List[ActionProposal], context: AnalysisContext) -> List[ActionProposal]:
        """提案のフィルタリングとランキング"""
        # 信頼度フィルタリング
        min_confidence = self.config.get("action_proposal", {}).get("min_confidence_threshold", 0.6)
        filtered_proposals = [p for p in proposals if p.confidence_score >= min_confidence]
        ...
        # 最大提案数の制限
        max_proposals = self.config.get("action_proposal", {}).get("max_proposals_per_analysis", 5)
        return sorted_proposals[:max_proposals]
    
    def _remove_duplicate_proposals(self, proposals: List[ActionProposal]) -> List[ActionProposal]:
        """重複提案の除去"""
        seen_actions = set()
        unique_proposals = []
    ...
    def get_proposal_statistics(self) -> Dict[str, Any]:
        ...
        total_proposals = sum(record["proposal_count"] for record in self.action_history)
        ...
        return {
            ...
            "recent_proposals": self.action_history[-10:] if self.action_history else []
        }
