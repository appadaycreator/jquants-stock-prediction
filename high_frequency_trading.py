...
class HighFrequencyTrading:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        Args:
            config: 設定辞書
        """
        self.latency_threshold = config.get('latency_threshold', 0.001) if config else 0.001  # 1ms
        self.profit_threshold = config.get('profit_threshold', 0.001) if config else 0.001   # 0.1%
        self.max_position_size = config.get('max_position_size', 1000000) if config else 1000000
    ...
    def execute_arbitrage(self, price_differences: Dict[str, Dict[str, float]]) -> List[Trade]:
        """
        裁定取引の実行
        ...
        try:
            ...
            if execution_time > self.latency_threshold:
                logger.warning(f"実行時間が閾値を超過: {execution_time:.4f}s > {self.latency_threshold}s")
            
            logger.info(f"裁定取引を実行: {len(executed_trades)}件, 実行時間: {execution_time:.4f}s")
    ...
    def _find_arbitrage_opportunities(self, price_differences: Dict[str, Dict[str, float]]) -> List[ArbitrageOpportunity]:
        """
        裁定取引機会を発見
    ...
    def _create_trade(self, symbol: str, side: str, price: float, volume: int, exchange: str) -> Trade:
        """取引オブジェクトを作成"""
        return Trade(
            symbol=symbol,
            side=side,
