...
class RoutineAnalysisAPI:
    ...
    def get_simplified_risk_dashboard_data(
        ...
        try:
            ...
            # 簡素化リスクAPIを使用してデータ取得
            dashboard_data = self.simplified_risk_api.get_visual_risk_display(portfolio_data)
    ...
    def get_portfolio_risk_summary(
        ...
        try:
            ...
            summary_data = {
                ...
                "risk_alerts": alerts_response["data"] if alerts_response["success"] else {"alerts": []},
                "risk_statistics": self.simplified_risk_api.get_risk_statistics()["data"],
                "recommendations": self._generate_risk_recommendations(
                    balance_response["data"], 
                    alerts_response["data"] if alerts_response["success"] else {"alerts": []}
                )
            }
    ...
    def export_risk_report(self, portfolio_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if report_result["success"]:
                # レポートファイル保存
                report_filename = f"risk_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    # ヘルパーメソッド群
    def _execute_stock_analysis(self, stock_codes: List[str], analysis_date: str) -> Dict[str, Any]:
        """株式分析実行（既存機能）"""
        try:
            # 既存の分析ロジックを実行
            analysis_results = {
            ...
            for stock_code in stock_codes:
                try:
                    ...
                    analysis_results["recommendations"][stock_code] = investment_decision
                    
                    # テクニカル分析
                    technical_indicators = self.technical_analysis.calculate_indicators(stock_data)
                    analysis_results["technical_indicators"][stock_code] = technical_indicators
    ...
    def _add_simplified_risk_assessment(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        """簡素化リスク評価追加"""
        try:
            risk_assessment = {
                "stock_risk_metrics": {},
            ...
            # 各銘柄のリスク評価
            for stock_code, prediction in analysis_results.get("predictions", {}).items():
                try:
                    # 株式データ取得
                    stock_data = self.json_manager.get_stock_data(stock_code)
                    if stock_data is None or stock_data.empty:
                    ...
                    # 簡素化リスクメトリクス計算
                    risk_metrics = self.simplified_risk_manager.calculate_simplified_risk_metrics(
            ...
            if risk_assessment["stock_risk_metrics"]:
                portfolio_data = {
                    stock_code: {
                        "stock_data": self.json_manager.get_stock_data(stock_code),
                        "current_price": self.json_manager.get_stock_data(stock_code)['Close'].iloc[-1] if not self.json_manager.get_stock_data(stock_code).empty else 0,
                ...
                portfolio_balance = self.simplified_risk_manager.calculate_portfolio_risk_balance(
                ...
                # リスクアラート取得
                risk_alerts = self.simplified_risk_manager.get_risk_alerts(portfolio_data)
