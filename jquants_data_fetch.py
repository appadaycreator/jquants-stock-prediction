...
class JQuantsAPIClient:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"⚠️ APIリクエスト失敗: {response.status_code} - {response.text}"
                    )

            except requests.exceptions.Timeout as e:
                if self.specific_error_handler.handle_connection_error(
    ...
    def _get_refresh_token(self) -> str:
        ...
        try:
            ...
            response = self._make_request_with_retry(
                "POST", auth_url, json=auth_payload
            )
            ...
            if "refreshToken" not in auth_data:
                ...
                logger.error(
                    f"❌ リフレッシュトークンが取得できませんでした: {auth_data}"
                )
                raise ValueError(error_msg)
    ...
    def _get_id_token(self) -> str:
        ...
        try:
            ...
            response = self._make_request_with_retry(
                "POST", id_token_url, params=id_token_params
            )
        ...
        try:
            ...
            self.error_handler.log_error(
                ...
                "データ保存エラー",
                {
                    ...
                    "output_dir_exists": (
                        os.path.exists(os.path.dirname(output_file))
                        if output_file
                        else None
                    ),
                },
