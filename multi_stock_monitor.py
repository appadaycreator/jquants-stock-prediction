...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('multi_stock_monitor.log'),
...
class TechnicalAnalyzer:
    ...
    def calculate_technical_indicators(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        indicators['bb_middle'] = sma_20.iloc[-1] if not sma_20.empty else close.iloc[-1]
        ...
        indicators['volatility'] = returns.std() * np.sqrt(252) if not returns.empty else 0
        
        # サポート・レジスタンス
        indicators['support'] = low.rolling(window=20).min().iloc[-1] if len(low) >= 20 else low.iloc[-1]
        indicators['resistance'] = high.rolling(window=20).max().iloc[-1] if len(high) >= 20 else high.iloc[-1]
        
        return indicators
    
    def calculate_technical_score(self, indicators: Dict[str, float], current_price: float) -> float:
        """技術スコアを計算（0-100）"""
        score = 50  # ベーススコア
...
class FundamentalAnalyzer:
    ...
    def calculate_fundamental_score(self, symbol: str, current_price: float, 
                                  market_cap: float, pe_ratio: float) -> float:
...
class MomentumAnalyzer:
    ...
    def calculate_momentum_score(self, data: pd.DataFrame) -> float:
        ...
        # 短期・中期・長期のリターン
        short_return = (close.iloc[-1] / close.iloc[-5] - 1) * 100 if len(close) >= 5 else 0
        medium_return = (close.iloc[-1] / close.iloc[-20] - 1) * 100 if len(close) >= 20 else 0
        long_return = (close.iloc[-1] / close.iloc[-60] - 1) * 100 if len(close) >= 60 else 0
...
class MultiStockMonitor:
    ...
    def analyze_single_stock(self, symbol: str) -> Optional[StockAnalysis]:
        ...
        try:
            ...
            # 技術指標計算
            technical_indicators = self.technical_analyzer.calculate_technical_indicators(data)
            technical_score = self.technical_analyzer.calculate_technical_score(technical_indicators, current_price)
            ...
            # 信頼度計算
            confidence = self._calculate_confidence(technical_score, fundamental_score, momentum_score)
            ...
            recommendation_reason = self._generate_recommendation_reason(
                technical_indicators, technical_score, fundamental_score, momentum_score
            )
            ...
            analysis = StockAnalysis(
                ...
                macd_signal="BUY" if technical_indicators.get('macd', 0) > technical_indicators.get('macd_signal', 0) else "SELL",
                trend_direction=self._determine_trend_direction(technical_indicators, current_price),
                support_level=technical_indicators.get('support', current_price * 0.9),
                resistance_level=technical_indicators.get('resistance', current_price * 1.1),
    ...
    def _determine_investment_opportunity(self, technical_score: float, 
                                        fundamental_score: float, 
                                        momentum_score: float) -> InvestmentOpportunity:
        """投資機会を判定"""
    ...
    def _determine_trend_direction(self, indicators: Dict[str, float], current_price: float) -> str:
        """トレンド方向を判定"""
        sma_20 = indicators.get('sma_20', current_price)
        sma_50 = indicators.get('sma_50', current_price)
    ...
    def _generate_recommendation_reason(self, indicators: Dict[str, float], 
                                      technical_score: float, 
                                      fundamental_score: float, 
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    if result:
                        ...
                        logger.info(f"分析完了: {symbol} - {result.investment_opportunity.value}")
                except Exception as e:
                    logger.error(f"分析エラー {symbol}: {e}")
    ...
    def _calculate_risk_return_analysis(self) -> Dict[str, float]:
        ...
        returns = [analysis.change_percent for analysis in self.analysis_results.values()]
        volatilities = [analysis.volatility for analysis in self.analysis_results.values()]
        ...
        return {
            ...
            'sharpe_ratio': np.mean(returns) / np.mean(volatilities) if np.mean(volatilities) > 0 else 0
        }
    ...
    def _get_best_opportunities(self) -> List[Dict]:
        ...
        for symbol, analysis in self.analysis_results.items():
            if analysis.investment_opportunity in [InvestmentOpportunity.STRONG_BUY, InvestmentOpportunity.BUY]:
...
def main():
    ...
    for i, performer in enumerate(portfolio_comparison.top_performers[:3], 1):
        print(f"  {i}. {performer['symbol']}: {performer['change_percent']:+.2f}% "
    ...
    for i, opportunity in enumerate(portfolio_comparison.best_opportunities[:5], 1):
        print(f"  {i}. {opportunity['symbol']} - {opportunity['investment_opportunity']} "
