...
async function testAPI() {
  try {
    // 1. トークンテスト - 正しいエンドポイント: /listed/info
    console.log("\n--- トークンで銘柄一覧を取得 ---");
    ...
    if (listsData.info && listsData.info.length > 0) {
      ...
      console.log("Full response:", JSON.stringify(listsData, null, 2).substring(0, 500));
    }
    
    // 2. 株価データ取得テスト（トヨタ: 72030）
    console.log("\n--- 株価データ取得テスト (72030: トヨタ) ---");
    ...
    if (pricesData.daily_quotes && pricesData.daily_quotes.length > 0) {
      ...
      console.log("Full response:", JSON.stringify(pricesData, null, 2).substring(0, 500));
    }
