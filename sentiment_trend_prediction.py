...
class SentimentTrendPredictor:
    ...
    async def predict_sentiment_trend(self, symbol: str, horizon: int = 15) -> SentimentPrediction:
        """感情トレンドの予測"""
        try:
            # 履歴データの取得
            ...
            # トレンド方向の判定
            trend_direction = self._determine_trend_direction(ensemble_prediction, history_data)
            
            # ボラティリティ予測
            volatility_prediction = self._predict_volatility(history_data, horizon)
            
            # リスクレベルの判定
            risk_level = self._determine_risk_level(ensemble_prediction, volatility_prediction)
    ...
    async def _get_sentiment_history(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.sentiment_metrics_generator:
                metrics = await self.sentiment_metrics_generator.generate_realtime_metrics([symbol])
                for metric in metrics:
                    self.sentiment_history[symbol].append({
                        'timestamp': metric.timestamp,
                        'sentiment_score': metric.value if metric.metric_type == MetricType.SENTIMENT_SCORE else 0.0,
                        'volatility': metric.value if metric.metric_type == MetricType.VOLATILITY_INDEX else 0.0,
                        'trend_strength': metric.value if metric.metric_type == MetricType.TREND_STRENGTH else 0.0,
                        'risk_level': metric.value if metric.metric_type == MetricType.RISK_LEVEL else 0.0
                    })
    ...
    def _prepare_features(self, history_data: List[Dict[str, Any]]) -> Optional[np.ndarray]:
        """特徴量の準備"""
        try:
    ...
    async def _predict_with_model(self, model, model_name: PredictionModel, 
                                features: np.ndarray, horizon: int) -> Tuple[float, float]:
        """モデルによる予測"""
        try:
            # スケーラーの適用
    ...
    def _ensemble_predictions(self, predictions: Dict[PredictionModel, float], 
                            confidences: Dict[PredictionModel, float]) -> float:
        """アンサンブル予測"""
        try:
            # 信頼度で重み付き平均
            weighted_sum = 0.0
    ...
    def _determine_trend_direction(self, prediction: float, history_data: List[Dict[str, Any]]) -> str:
        """トレンド方向の判定"""
        try:
    ...
    def _predict_volatility(self, history_data: List[Dict[str, Any]], horizon: int) -> float:
        """ボラティリティの予測"""
        try:
            ...
            if len(recent_volatilities) >= 3:
                trend = np.polyfit(range(len(recent_volatilities)), recent_volatilities, 1)[0]
                # トレンドを予測期間に適用
                predicted_volatility = avg_volatility + (trend * horizon / 60)  # 時間単位で調整
            else:
    ...
    def _create_default_prediction(self, symbol: str, horizon: int) -> SentimentPrediction:
        """デフォルト予測の作成"""
    ...
    def _prepare_training_data(self, history_data: List[Dict[str, Any]]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """訓練データの準備"""
        try:
    ...
    def visualize_predictions(self, symbol: str, save_path: str = None):
        ...
        try:
            ...
            # 感情スコアのプロット
            axes[0, 0].plot(timestamps, sentiment_scores, 'b-', linewidth=2, label='実際の感情スコア')
            if predictions:
                pred_timestamps = [p.timestamp for p in predictions]
                pred_scores = [p.predicted_sentiment for p in predictions]
                axes[0, 0].scatter(pred_timestamps, pred_scores, color='red', s=50, label='予測値')
            ...
            # ボラティリティのプロット
            axes[0, 1].plot(timestamps, volatilities, 'orange', linewidth=2, label='実際のボラティリティ')
            if predictions:
                pred_volatilities = [p.volatility_prediction for p in predictions]
                axes[0, 1].scatter(pred_timestamps, pred_volatilities, color='red', s=50, label='予測値')
            ...
            if predictions:
                ...
                for i, (score, direction) in enumerate(zip(trend_scores, trend_directions)):
                    axes[1, 0].scatter(i, score, color=trend_colors.get(direction, 'gray'), s=100)
            ...
            if predictions:
                ...
                axes[1, 1].plot(range(len(confidences)), confidences, 'purple', linewidth=2, marker='o')
        ...
        try:
            ...
            if patterns:
                ...
                for pattern in patterns:
                    logger.info(f"  - {pattern.pattern_name}: {pattern.expected_outcome}")
