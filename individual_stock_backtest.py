...
class SeasonalAnalyzer:
    ...
    def _analyze_seasonal_patterns(
        self, monthly_returns: Dict[int, float]
    ) -> Dict[str, Any]:
        """季節パターン分析"""
    ...
    def _identify_best_worst_months(
        self, monthly_returns: Dict[int, float]
...
class EarningsAnalyzer:
    ...
    def analyze_earnings_impact(
        ...
        post_earnings_returns = self._analyze_post_earnings_returns(
            data, earnings_dates
        )
    ...
    def _analyze_pre_earnings_returns(
        self, data: pd.DataFrame, earnings_dates: List[datetime]
    ) -> Dict[str, float]:
        """業績発表前リターン分析"""
        pre_returns = {}
    ...
    def _analyze_post_earnings_returns(
        self, data: pd.DataFrame, earnings_dates: List[datetime]
    ) -> Dict[str, float]:
        """業績発表後リターン分析"""
        post_returns = {}
    ...
    def _analyze_earnings_volatility(
        self, data: pd.DataFrame, earnings_dates: List[datetime]
    ) -> Dict[str, float]:
        """業績発表期間ボラティリティ分析"""
        volatility_metrics = {}
        ...
        for earnings_date in earnings_dates:
            ...
            if pre_start in data.index and post_end in data.index:
                ...
                volatility_metrics["volatility_change"] = (
                    post_volatility - pre_volatility
                )

        return volatility_metrics

    def _analyze_surprise_impact(
        self, data: pd.DataFrame, earnings_dates: List[datetime]
    ) -> Dict[str, float]:
        """サプライズ影響分析"""
        surprise_metrics = {}
...
class TechnicalOptimizer:
    ...
    def optimize_technical_indicators(
        ...
        # 最適化実行
        optimal_params, performance_metrics, sensitivity = self._run_optimization(
            data, param_grid
        )
    ...
    def _run_optimization(
        ...
        for params in ParameterGrid(param_grid):
            try:
                ...
                all_results.append({"params": params, "performance": performance})
    ...
    def _generate_signals(
        ...
        # RSIシグナル
        rsi_signal = np.where(
            indicators["rsi"] > 70, -1, np.where(indicators["rsi"] < 30, 1, 0)
        )
...
class IndividualStockBacktestSystem:
    ...
    def analyze_individual_stock(
        self, symbol: str, start_date: datetime, end_date: datetime
    ) -> IndividualStockResult:
        """個別銘柄分析実行"""
        ...
        # 技術指標最適化
        technical_optimization = self.technical_optimizer.optimize_technical_indicators(
            data, symbol
        )
        ...
        for strategy in strategies:
            try:
                result = self.backtest_engine.run_backtest(
                    data, strategy, start_date, end_date
                )
    ...
    def compare_multiple_stocks(
    ...
    ) -> Dict[str, IndividualStockResult]:
        """複数銘柄比較分析"""
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_symbol = {
                executor.submit(
                    self.analyze_individual_stock, symbol, start_date, end_date
                ): symbol
    ...
    def generate_comprehensive_report(self, result: IndividualStockResult) -> str:
        ...
        report.append(
            f"  トレンド強度: {result.seasonal_analysis.seasonal_patterns['trend_r_squared']:.4f}"
        )
...
def main():
    ...
    try:
        ...
        comparison_results = system.compare_multiple_stocks(
        ...
        for symbol, result in comparison_results.items():
            if result:
                ...
                comparison_report.append(
                    f"  総リターン: {result.basic_metrics['total_return']:.2%}"
                )
                comparison_report.append(
                    f"  シャープレシオ: {result.basic_metrics['sharpe_ratio']:.2f}"
                )
                comparison_report.append(
                    f"  最大ドローダウン: {result.basic_metrics['max_drawdown']:.2%}"
                )
