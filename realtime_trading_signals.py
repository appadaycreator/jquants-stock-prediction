...
class RiskManager:
    ...
    def calculate_position_size(self, account_value: float, signal: TradingSignal) -> float:
        """ポジションサイズを計算"""
    ...
    def should_stop_loss(self, entry_price: float, current_price: float, signal: TradingSignal) -> bool:
        """損切り判定"""
        loss_percent = ((current_price - entry_price) / entry_price) * 100
        return loss_percent <= -self.max_loss_percent
    
    def should_take_profit(self, entry_price: float, current_price: float, signal: TradingSignal) -> bool:
        """利確判定"""
...
class MultiStockMonitor:
    ...
    def __init__(self, symbols: List[str], signal_generator: SignalGenerator, risk_manager: RiskManager):
        ...
        for symbol in self.symbols:
            try:
                ...
                if data is not None:
                    ...
                    if signals:
                        signal = signals[0]
                        logger.info(f"{symbol}: {signal.signal_type.value} (信頼度: {signal.confidence:.2f})")
                else:
                    all_signals[symbol] = []
    ...
    def get_top_signals(self, all_signals: Dict[str, List[TradingSignal]], limit: int = 5) -> List[TradingSignal]:
        """上位シグナルを取得"""
        all_signals_list = []
        ...
        # 信頼度でソート
        sorted_signals = sorted(all_signals_list, key=lambda x: x.confidence, reverse=True)
...
class TradingSignalSystem:
    ...
    def __init__(self, symbols: List[str], account_value: float = 1000000):
        ...
        self.monitor = MultiStockMonitor(symbols, self.signal_generator, self.risk_manager)
    ...
    def run_analysis(self) -> Dict:
        ...
        for signal in top_signals:
            signal_data = {
                ...
                'position_size': self.risk_manager.calculate_position_size(self.account_value, signal)
            }
        ...
        logger.info(f"買いシグナル: {results['summary']['buy_signals'] + results['summary']['strong_buy_signals']}")
        logger.info(f"売りシグナル: {results['summary']['sell_signals'] + results['summary']['strong_sell_signals']}")
        
        return results
    
    def save_results(self, results: Dict, filename: str = "trading_signals_results.json"):
        """結果をJSONファイルに保存"""
        try:
...
def main():
    ...
    for i, signal in enumerate(results['top_signals'][:5], 1):
        ...
        print(f"     価格: ¥{signal['price']:.2f}, 推奨ポジション: ¥{signal['position_size']:,.0f}")
