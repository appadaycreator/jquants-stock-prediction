...
class UnifiedErrorHandlingSystem:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        file_handler = logging.FileHandler(
            'logs/unified_error_handling.log',
            encoding='utf-8'
        )
    ...
    def _log_error_info(self, error_info: ErrorInfo):
        ...
        if error_info.severity == ErrorSeverity.CRITICAL:
            ...
            self.logger.critical(f"Error Details: {json.dumps(log_data, ensure_ascii=False, indent=2, default=str)}")
        elif error_info.severity == ErrorSeverity.HIGH:
            self.logger.error(f"HIGH SEVERITY ERROR: {error_info.error_message}")
            self.logger.error(f"Error Details: {json.dumps(log_data, ensure_ascii=False, indent=2, default=str)}")
        elif error_info.severity == ErrorSeverity.MEDIUM:
            self.logger.warning(f"MEDIUM SEVERITY ERROR: {error_info.error_message}")
            self.logger.warning(f"Error Details: {json.dumps(log_data, ensure_ascii=False, indent=2, default=str)}")
        else:
            self.logger.info(f"LOW SEVERITY ERROR: {error_info.error_message}")
            self.logger.info(f"Error Details: {json.dumps(log_data, ensure_ascii=False, indent=2, default=str)}")
...
def get_unified_error_handler(config: Optional[Dict[str, Any]] = None) -> UnifiedErrorHandlingSystem:
    """
    統合エラーハンドリングシステムの取得
...
def log_data_error(error: Exception, data_type: str, data_shape: Optional[tuple] = None):
    """データエラーのログ記録"""
    error_handler = get_unified_error_handler()
