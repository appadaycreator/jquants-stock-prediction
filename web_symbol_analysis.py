...
class WebSymbolAnalysis:
    ...
    def _analyze_single_symbol(self, data: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            technical = {
                ...
                "bb_percent": float(data['BB_Percent'].iloc[-1]) if not pd.isna(data['BB_Percent'].iloc[-1]) else None,
                "sma_5": float(data['SMA_5'].iloc[-1]) if not pd.isna(data['SMA_5'].iloc[-1]) else None,
                "sma_25": float(data['SMA_25'].iloc[-1]) if not pd.isna(data['SMA_25'].iloc[-1]) else None,
            }
    ...
    def _analyze_combined_data(self, all_data: List[pd.DataFrame], symbols: List[str]) -> Dict[str, Any]:
        """複数銘柄の統合分析"""
        try:
            # 全データを結合
    ...
    def save_results(self, results: Dict[str, Any], filename: str = "symbol_analysis_results.json"):
        """分析結果の保存"""
        try:
    ...
    def generate_web_data(self, symbols: List[str]):
        ...
        try:
            ...
            # 各ファイルに保存
            with open(self.output_dir / "symbol_analysis_results.json", 'w', encoding='utf-8') as f:
