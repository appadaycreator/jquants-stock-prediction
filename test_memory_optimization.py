...
class MemoryOptimizationTester:
    ...
    def test_memory_optimization_effectiveness(self):
        ...
        # 効果の計算
        total_memory_saved = before_metrics['total_memory_mb'] - after_metrics['total_memory_mb']
        df_memory_saved = before_metrics['dataframe_memory_mb'] - after_metrics['dataframe_memory_mb']
        
        # パーセンテージ計算
        total_memory_reduction_percent = (total_memory_saved / before_metrics['total_memory_mb']) * 100
        df_memory_reduction_percent = (df_memory_saved / before_metrics['dataframe_memory_mb']) * 100
        ...
        logger.info(f"  💾 総メモリ節約: {total_memory_saved:.1f}MB ({total_memory_reduction_percent:.1f}%)")
        logger.info(f"  📊 データフレームメモリ節約: {df_memory_saved:.1f}MB ({df_memory_reduction_percent:.1f}%)")
    ...
    def test_large_dataset_processing(self):
        ...
        # 処理後のメモリ使用量
        after_metrics = self.measure_memory_usage(optimized_df, "after_large_processing")
        
        # 効果の計算
        memory_saved = before_metrics['total_memory_mb'] - after_metrics['total_memory_mb']
        memory_reduction_percent = (memory_saved / before_metrics['total_memory_mb']) * 100
        ...
        logger.info(f"  💾 メモリ節約: {memory_saved:.1f}MB ({memory_reduction_percent:.1f}%)")
    ...
    def run_comprehensive_test(self):
        ...
        if memory_optimization_result:
            logger.info(f"  💾 メモリ最適化効果: {memory_optimization_result['total_memory_reduction_percent']:.1f}%削減")
        
        if large_dataset_result:
            logger.info(f"  📈 大規模データ処理: {large_dataset_result['memory_reduction_percent']:.1f}%削減")
        
        if memory_limit_result:
            logger.info(f"  🛡️ メモリ制限処理: {'✅ 成功' if memory_limit_result.get('optimization_applied', False) else '⚠️ 要改善'}")
        
        # パフォーマンスメトリクスの取得
        performance_metrics = self.system.get_performance_metrics()
        logger.info(f"  🚀 システム状態: {performance_metrics.get('memory_status', 'unknown')}")
