...
class DataSourceComparator:
    ...
    def compare_data_sources(self):
        ...
        print(
            f"   jQuants APIデータ: {len(api_data.get('info', []))}銘柄"
            if api_data
            else "   jQuants APIデータ: 取得不可"
        )
        ...
        for code, data in list(current_data.items())[:100]:  # 最初の100銘柄をサンプル
            if isinstance(data, list) and len(data) > 0:
                ...
                has_zero_values = any(
                    ...
                    or record.get("open", 0) == 0
                    or record.get("high", 0) == 0
                    or record.get("low", 0) == 0
                    for record in data
                )
                if has_zero_values:
    ...
    def compare_sample_data(
        ...
        for code in sample_codes:
            ...
            if api_data and "info" in api_data:
                api_info = next(
                    (item for item in api_data["info"] if item.get("Code") == code),
                    None,
                )
                if api_info:
