...
class StructuredDataGenerator:
    ...
    def generate_stock_data_structure(self, stock_data: Dict[str, Any]) -> Dict[str, Any]:
        """株価データの構造化"""
        structured_data = {
            "metadata": {
        ...
        for code, stock_info in stock_data.items():
            structured_data["stocks"][code] = {
                ...
                "current_price": {
                    ...
                    "updated_at": stock_info.get("updated_at", datetime.now().isoformat())
                },
                "volume": {
                ...
                "prediction": {
                    ...
                    "prediction_date": stock_info.get("prediction_date", datetime.now().isoformat())
                },
                "risk_metrics": {
                ...
                "metadata": {
                    "created_at": stock_info.get("created_at", datetime.now().isoformat()),
                    "updated_at": stock_info.get("updated_at", datetime.now().isoformat()),
                    "data_quality": stock_info.get("data_quality", "good"),
                    "last_trade_date": stock_info.get("last_trade_date", datetime.now().isoformat())
                }
            }
    ...
    def generate_index_file(self, structured_data: Dict[str, Any]):
        ...
        index_data = {
            "metadata": {
                ...
                "last_updated": structured_data.get("metadata", {}).get("generated_at", "")
            },
            "stocks": []
        }
        ...
        for code, stock_info in structured_data.get("stocks", {}).items():
            index_data["stocks"].append({
                ...
                "name": stock_info.get("name", ""),
                "sector": stock_info.get("sector", ""),
                "last_price": stock_info.get("current_price", {}).get("last_price", 0),
                "change_percent": stock_info.get("current_price", {}).get("change_percent", 0),
                "updated_at": stock_info.get("current_price", {}).get("updated_at", ""),
