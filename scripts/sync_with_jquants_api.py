...
class JQuantsAPISyncer:
    ...
    def get_stock_prices_from_api(self, code: str, days: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    logger.warning(f"API呼び出しエラー (試行 {attempt + 1}/{self.max_retries}): {e}")
                    if attempt == self.max_retries - 1:
                        return []
                    time.sleep(1)
                    continue
        ...
        for i, stock in enumerate(stocks):
            ...
            try:
                ...
                if processed_count % 50 == 0:
                    logger.info(f"進捗: {processed_count}/{len(stocks)}銘柄処理完了 (API成功: {api_success_count})")
    ...
    def save_synced_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if original_file.exists():
                backup_file = self.data_dir / f"stock_data_api_sync_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
