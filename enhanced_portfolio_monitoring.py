...
# 既存システムのインポート
try:
    from enhanced_individual_stock_monitor import EnhancedIndividualStockMonitor, IndividualStockMonitor, AlertType, AlertPriority
    from enhanced_news_sentiment_integration import EnhancedNewsSentimentIntegration, IndividualStockSentiment
    from enhanced_technical_indicators_realtime import EnhancedTechnicalIndicatorsRealtime, IndividualStockTechnical, TechnicalSignal
...
class EnhancedPortfolioMonitoring:
    ...
    def __init__(self, symbols: List[str], config: Dict[str, Any] = None):
        ...
        # 個別システムの初期化
        self.individual_monitor = EnhancedIndividualStockMonitor(symbols, self.config.get("individual_monitor", {}))
        self.sentiment_system = EnhancedNewsSentimentIntegration(self.config.get("sentiment", {}))
        self.technical_system = EnhancedTechnicalIndicatorsRealtime(symbols, self.config.get("technical", {}))
    ...
    async def start_portfolio_monitoring(self):
        ...
        # 個別システムの監視開始
        individual_task = asyncio.create_task(self.individual_monitor.start_monitoring())
        sentiment_task = asyncio.create_task(self.sentiment_system.start_sentiment_monitoring())
        technical_task = asyncio.create_task(self.technical_system.start_technical_monitoring())
        ...
        try:
            await asyncio.gather(
                individual_task, sentiment_task, technical_task, 
                portfolio_task, alert_task, return_exceptions=True
            )
    ...
    async def _calculate_portfolio_metrics(
        ...
        try:
            # 基本指標
            total_value = sum(monitor.current_price for monitor in individual_monitors.values())
            ...
            return PortfolioMetrics(
                total_value=0.0, total_return=0.0, daily_return=0.0,
                volatility=0.0, sharpe_ratio=0.0, max_drawdown=0.0,
                beta=0.0, alpha=0.0, correlation_matrix={},
                diversification_ratio=0.0, risk_level=PortfolioRiskLevel.MEDIUM,
                performance_rating=PortfolioPerformance.AVERAGE
            )
    
    def _calculate_total_return(self, individual_monitors: Dict[str, IndividualStockMonitor]) -> float:
        """総リターンの計算"""
        returns = [monitor.change_percent for monitor in individual_monitors.values()]
        return np.mean(returns) if returns else 0.0
    
    def _calculate_daily_return(self, individual_monitors: Dict[str, IndividualStockMonitor]) -> float:
        """日次リターンの計算"""
        # 簡易的な日次リターン計算（実際の実装ではより詳細な計算）
        returns = [monitor.change_percent for monitor in individual_monitors.values()]
        return np.mean(returns) if returns else 0.0
    
    def _calculate_portfolio_volatility(self, individual_monitors: Dict[str, IndividualStockMonitor]) -> float:
        """ポートフォリオボラティリティの計算"""
    ...
    def _calculate_max_drawdown(self, individual_monitors: Dict[str, IndividualStockMonitor]) -> float:
        """最大ドローダウンの計算"""
        # 簡易的な最大ドローダウン計算
        max_drawdowns = []
    ...
    def _calculate_portfolio_beta(self, individual_monitors: Dict[str, IndividualStockMonitor]) -> float:
        """ポートフォリオベータの計算"""
    ...
    def _calculate_portfolio_alpha(self, individual_monitors: Dict[str, IndividualStockMonitor], beta: float) -> float:
        """ポートフォリオアルファの計算"""
        # 簡易的なアルファ計算
        total_return = self._calculate_total_return(individual_monitors)
    ...
    def _calculate_correlation_matrix(self, individual_monitors: Dict[str, IndividualStockMonitor]) -> Dict[str, Dict[str, float]]:
        """相関行列の計算"""
        correlation_matrix = {}
    ...
    async def _check_portfolio_alerts(self, portfolio_metrics: PortfolioMetrics):
        ...
        if portfolio_metrics.risk_level == PortfolioRiskLevel.CRITICAL:
            alerts.append(PortfolioAlert(
                ...
                timestamp=datetime.now(),
                recommendations=["即座にリスク軽減策を実施", "ポジションサイズの縮小", "ヘッジ戦略の検討"]
            ))
        ...
        if portfolio_metrics.performance_rating == PortfolioPerformance.CRITICAL:
            alerts.append(PortfolioAlert(
                ...
                threshold_value=self.config["performance_thresholds"]["critical_return"],
                timestamp=datetime.now(),
                recommendations=["緊急のポートフォリオ見直し", "損失限定の実施", "専門家への相談"]
            ))
        ...
        if portfolio_metrics.diversification_ratio < 0.3:
            alerts.append(PortfolioAlert(
                ...
                threshold_value=0.3,
                timestamp=datetime.now(),
                recommendations=["新規銘柄の追加", "セクター分散の改善", "資産クラス分散の検討"]
            ))
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_history_length": len(self.portfolio_monitor.performance_history),
