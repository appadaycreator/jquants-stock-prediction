...
class TechnicalIndicators:
    ...
    def calculate_all_indicators(
        ...
        try:
            # 1. 移動平均系
            result_df = self._calculate_moving_averages(
                result_df, config.get("sma_windows", [5, 10, 25, 50])
            )
            result_df = self._calculate_ema(
                result_df, config.get("ema_windows", [12, 26])
            )
            ...
            result_df = self._calculate_stochastic(
                result_df, config.get("stoch_k", 14), config.get("stoch_d", 3)
            )

            # 3. ボラティリティ系指標
            result_df = self._calculate_bollinger_bands(
                result_df, config.get("bb_period", 20), config.get("bb_std", 2)
            )
            result_df = self._calculate_atr(result_df, config.get("atr_period", 14))

            # 4. ボリューム系指標
            result_df = self._calculate_volume_indicators(
                result_df, config.get("volume_sma_period", 20)
            )
            ...
            # 6. 価格変動率
            result_df = self._calculate_price_changes(
                result_df, config.get("price_change_periods", [1, 3, 5, 10])
            )

            # 7. 価格位置指標
            result_df = self._calculate_price_position(
                result_df, config.get("price_position_periods", [20, 50])
            )

            self.logger.info(
                f"✅ 技術指標計算完了: {len(result_df.columns) - len(df.columns)}個の新指標を追加"
            )
    ...
    def _calculate_volume_indicators(
        self, df: pd.DataFrame, sma_period: int = 20
    ) -> pd.DataFrame:
        """ボリューム系指標を計算"""
        result_df = df.copy()
    ...
    def _calculate_vwap(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # VWAP計算
        result_df["VWAP"] = (typical_price * df["Volume"]).cumsum() / df[
            "Volume"
        ].cumsum()

        # VWAP乖離率
        result_df["VWAP_Deviation"] = (
            (df["Close"] - result_df["VWAP"]) / result_df["VWAP"] * 100
        )
...
def get_unified_technical_evidence(df: pd.DataFrame) -> Dict[str, object]:
    ...
    Input:
        df: 最新行に指標が含まれるDataFrame（calculate_all_indicators適用済）
    ...
    try:
        ...
        if "ADX" in df.columns:
            ...
            key_items.append({"name": "ADX", "value": adx, "signal": adx_signal})
        ...
        return {"technical": {"key_indicators": []}}
...
if __name__ == "__main__":
    ...
    # 追加された指標を表示
    new_columns = [
        col for col in enhanced_data.columns if col not in sample_data.columns
    ]
    print("\n🔧 追加された技術指標:")
