...
class InvestmentStyleOptimizer:
    ...
    def _initialize_default_parameters(self) -> None:
        ...
        self.market_conditions[MarketCondition.SIDEWAYS_MARKET] = MarketConditionProfile(
        ...
        self.market_conditions[MarketCondition.VOLATILE_MARKET] = MarketConditionProfile(
            ...
            if os.path.exists(self.config_file):
                ...
                if 'market_conditions' in data:
                    for condition_name, profile in data['market_conditions'].items():
                        ...
                        self.market_conditions[condition] = MarketConditionProfile(**profile)
    ...
    def detect_market_condition(self, market_data: pd.DataFrame) -> MarketCondition:
        ...
        try:
            ...
            # モメンタムの計算
            momentum = (market_data['Close'].iloc[-1] - market_data['Close'].iloc[-20]) / market_data['Close'].iloc[-20]
    ...
    def optimize_style_parameters(self, symbol: str, current_style: InvestmentStyle, 
                                 price_data: pd.DataFrame, market_data: pd.DataFrame) -> StyleOptimizationResult:
        """投資スタイルパラメータを最適化"""
        try:
            # 市場環境を検出
            ...
            adjusted_params = self._adjust_parameters_for_market(current_params, market_profile)
            
            # 最適な投資スタイルを決定
            optimal_style = self._determine_optimal_style(price_data, market_condition)
            
            # スタイル信頼度を計算
            style_confidence = self._calculate_style_confidence(price_data, optimal_style)
    ...
    def _adjust_parameters_for_market(self, params: StyleParameters, 
                                    market_profile: MarketConditionProfile) -> StyleParameters:
        """市場環境に応じてパラメータを調整"""
        adjusted_params = StyleParameters(
            holding_period_days=params.holding_period_days,
            analysis_frequency=params.analysis_frequency,
            rebalance_frequency=params.rebalance_frequency,
            volatility_tolerance=params.volatility_tolerance * market_profile.volatility_multiplier,
            max_position_size=params.max_position_size / market_profile.risk_adjustment,
            stop_loss_percent=params.stop_loss_percent * market_profile.threshold_adjustments.get('stop_loss', 1.0),
            take_profit_percent=params.take_profit_percent * market_profile.threshold_adjustments.get('take_profit', 1.0),
            momentum_weight=params.momentum_weight * market_profile.momentum_adjustment,
            mean_reversion_weight=params.mean_reversion_weight * market_profile.threshold_adjustments.get('mean_reversion_weight', 1.0),
            technical_weight=params.technical_weight,
            fundamental_weight=params.fundamental_weight,
            rsi_period=params.rsi_period,
            rsi_overbought=params.rsi_overbought * market_profile.threshold_adjustments.get('rsi_overbought', 1.0),
            rsi_oversold=params.rsi_oversold * market_profile.threshold_adjustments.get('rsi_oversold', 1.0),
            macd_fast=params.macd_fast,
            macd_slow=params.macd_slow,
            macd_signal=params.macd_signal,
            sma_short=params.sma_short,
    ...
    def _determine_optimal_style(self, price_data: pd.DataFrame, market_condition: MarketCondition) -> InvestmentStyle:
        """最適な投資スタイルを決定"""
        try:
    ...
    def _calculate_style_confidence(self, price_data: pd.DataFrame, style: InvestmentStyle) -> float:
        """スタイル信頼度を計算"""
        try:
            if price_data.empty or len(price_data) < 30:
                return 0.5
            ...
            if style == InvestmentStyle.SHORT_TERM:
                ...
                confidence = min(volatility / 0.3, 1.0) * 0.5 + min(abs(momentum) / 0.1, 1.0) * 0.5
                
            elif style == InvestmentStyle.LONG_TERM:
                # 長期投資の適性：低ボラティリティ、安定したトレンド
                sma_short = price_data['Close'].rolling(window=20).mean()
                sma_long = price_data['Close'].rolling(window=50).mean()
                trend_consistency = 1.0 - abs(sma_short.iloc[-1] - sma_long.iloc[-1]) / sma_long.iloc[-1]
                confidence = (1.0 - min(volatility / 0.2, 1.0)) * 0.5 + trend_consistency * 0.5
    ...
    def _calculate_parameter_adjustments(self, current_params: StyleParameters, 
                                       adjusted_params: StyleParameters, 
                                       optimal_style: InvestmentStyle) -> Dict[str, Any]:
        """パラメータ調整を計算"""
        optimal_params = self.style_parameters[optimal_style]
        
        adjustments = {
            'holding_period_change': optimal_params.holding_period_days - current_params.holding_period_days,
            'volatility_tolerance_change': adjusted_params.volatility_tolerance - current_params.volatility_tolerance,
            'max_position_size_change': optimal_params.max_position_size - current_params.max_position_size,
            'stop_loss_change': optimal_params.stop_loss_percent - current_params.stop_loss_percent,
            'take_profit_change': optimal_params.take_profit_percent - current_params.take_profit_percent,
            'momentum_weight_change': adjusted_params.momentum_weight - current_params.momentum_weight,
            'mean_reversion_weight_change': adjusted_params.mean_reversion_weight - current_params.mean_reversion_weight,
            'rsi_threshold_adjustment': {
                'overbought': optimal_params.rsi_overbought - current_params.rsi_overbought,
    ...
    def _calculate_performance_improvement(self, current_style: InvestmentStyle, 
    ...
    def get_style_parameters(self, style: InvestmentStyle) -> StyleParameters:
        """指定スタイルのパラメータを取得"""
        return self.style_parameters.get(style, self.style_parameters[InvestmentStyle.MEDIUM_TERM])
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        style_changes = sum(1 for result in self.optimization_history 
                           if result.current_style != result.optimized_style)
        
        avg_confidence = np.mean([result.style_confidence for result in self.optimization_history])
        avg_improvement = np.mean([result.expected_performance_improvement for result in self.optimization_history])
        
        style_distribution = {}
        for style in InvestmentStyle:
            count = sum(1 for result in self.optimization_history 
        ...
        return {
            ...
            'change_rate': style_changes / total_optimizations if total_optimizations > 0 else 0,
