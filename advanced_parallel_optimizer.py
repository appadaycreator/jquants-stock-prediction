...
class AdvancedParallelOptimizer:
    ...
    def __init__(self, config_path: str = "config_final.yaml"):
        ...
        # 最適化設定
        self.optimization_enabled = self.config.get("performance", {}).get(
            "optimization", True
        )
        self.auto_scaling = self.config.get("performance", {}).get("auto_scaling", True)
        self.memory_threshold = self.config.get("performance", {}).get(
            "memory_threshold", 0.8
        )
        self.cpu_threshold = self.config.get("performance", {}).get(
            "cpu_threshold", 0.8
        )
    ...
    def optimize_worker_count(self) -> int:
        ...
        with self.optimization_lock:
            ...
            if len(self.performance_history) >= 3:
                recent_efficiency = [
                    m.efficiency for m in self.performance_history[-3:]
                ]
    ...
    def get_optimal_executor(
        self, task_type: str, task_profile: Optional[TaskProfile] = None
    ) -> Tuple[type, int]:
        """最適なExecutorを取得"""
    ...
    def execute_optimized_parallel(
        ...
        # タスクプロファイルの適用
        if task_profiles:
            executor_class, max_workers = self.get_optimal_executor(
                task_type, task_profiles[0]
            )
        else:
                ...
                stats.update(
                    {
                        ...
                        "avg_efficiency": np.mean(
                            [m.efficiency for m in recent_metrics]
                        ),
...
def execute_optimized_parallel(
    ...
    return optimizer.execute_optimized_parallel(
        tasks, task_type, timeout, priority, task_profiles
    )
