...
class IntegratedSentimentSystem:
    ...
    def _initialize_systems(self):
        ...
        try:
            ...
            if "api_keys" in self.config:
                self.enhanced_sentiment_system = EnhancedSentimentTradingSystem(
                    self.config
                )
    ...
    async def _get_sentiment_signal(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 感情分析シグナルの取得
            sentiment_data = (
                await self.enhanced_sentiment_system.generate_sentiment_signals(
                    [symbol]
                )
            )
    ...
    def _integrate_signals(
        ...
        try:
            ...
            # 統合スコアの計算
            jquants_score = jquants_signal.get("score", 0.0) * jquants_signal.get(
                "confidence", 0.0
            )
            technical_score = technical_signal.get("score", 0.0) * technical_signal.get(
                "confidence", 0.0
            )
            sentiment_score = sentiment_signal.get("score", 0.0) * sentiment_signal.get(
                "confidence", 0.0
            )

            combined_score = (
                jquants_score * jquants_weight
            ...
            # 統合信頼度の計算
            combined_confidence = (
                jquants_signal.get("confidence", 0.0) * jquants_weight
            ...
            risk_params = self._calculate_risk_parameters(
                symbol, combined_score, combined_confidence
            )

            # 推論の生成
            reasoning = self._generate_reasoning(
                jquants_signal, technical_signal, sentiment_signal
            )
    ...
    def _calculate_risk_parameters(
        self, symbol: str, score: float, confidence: float
    ) -> Dict[str, Any]:
        """リスク管理パラメータの計算"""
        try:
            # 現在価格の取得
        ...
        try:
            ...
            if jquants_signal.get("signal") != "HOLD":
                ...
                reasoning_parts.append(
                    f"J-Quants予測: {jquants_signal.get('signal')} (スコア: {jquants_score:.3f})"
                )
            ...
            if technical_signal.get("signal") != "HOLD":
                ...
                reasoning_parts.append(
                    f"技術分析: {technical_signal.get('signal')} (スコア: {technical_score:.3f}, RSI: {rsi:.1f})"
                )
            ...
            if sentiment_signal.get("signal") != "HOLD":
                ...
                reasoning_parts.append(
                    f"感情分析: {sentiment_signal.get('signal')} (スコア: {sentiment_score:.3f}, タイプ: {sentiment_type})"
                )
