...
class EnhancedParallelProcessor:
    ...
    def adaptive_worker_adjustment(self):
        ...
        if old_workers != self.current_workers:
            logger.info(
                f"🔄 ワーカー数適応調整: {old_workers} → {self.current_workers}"
            )
    ...
    async def async_parallel_execute(
        ...
        # 全タスクを並列実行
        task_coroutines = [
            execute_task_with_semaphore(task, i) for i, task in enumerate(tasks)
        ]
        ...
        logger.info(f"   - 実行時間: {execution_time:.2f}秒")
        logger.info(
            f"   - 成功率: {len([r for r in results if r is not None])}/{len(tasks)}"
        )
    ...
    def batch_processing(
        ...
        batch_results = self.parallel_execute_optimized(
            [partial(process_batch, batch) for batch in batches], task_type, len(data)
        )
    ...
    def _record_metrics(self, task_count: int, execution_time: float, task_type: str):
        ...
        self.metrics_history.append(
            {"timestamp": time.time(), "task_type": task_type, "metrics": metrics}
        )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        avg_execution_time = np.mean(
            [m["metrics"].execution_time for m in recent_metrics]
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if avg_efficiency < 80:
            recommendations.append(
                "処理効率が80%を下回っています。ワーカー数の調整を推奨します。"
            )

        if avg_cpu_usage > 90:
            recommendations.append(
                "CPU使用率が90%を超えています。ワーカー数を減らすことを推奨します。"
            )

        if avg_memory_usage > 85:
            recommendations.append(
                "メモリ使用率が85%を超えています。メモリ最適化を推奨します。"
            )

        if self.current_workers < self.max_workers and avg_cpu_usage < 50:
            recommendations.append(
                "CPU使用率が低いです。ワーカー数を増やすことを推奨します。"
            )
...
# コンテキストマネージャー
@contextmanager
def parallel_context(max_workers: Optional[int] = None, adaptive_mode: bool = True):
    """並列処理コンテキストマネージャー"""
...
if __name__ == "__main__":
    ...
    batch_results = processor.batch_processing(
        data, batch_size=10, processing_func=test_task, task_type="mixed"
    )
