...
class EnhancedTechnicalIndicatorsRealtime:
        ...
        try:
            ...
            with self.lock:
                ...
                for indicator_name, indicator in indicators.items():
                    ...
                    technical_data.indicator_history[indicator_name].append(
                        indicator.value
                    )
        ...
        try:
            ...
            if macd_data:
                indicators["macd"] = TechnicalIndicator(
                    ...
                    change_percent=self._calculate_change_percent(
                        macd_data["macd"],
                        self._get_previous_indicator_value(symbol, "macd"),
                    ),
                    signal=self._determine_macd_signal(macd_data),
                    strength=(
                        abs(macd_data["macd"]) / abs(macd_data["signal"])
            ...
            if bb_data:
                bb_position = (close.iloc[-1] - bb_data["lower"]) / (
                    bb_data["upper"] - bb_data["lower"]
                )
                indicators["bollinger"] = TechnicalIndicator(
                    name="Bollinger Bands",
                    value=bb_position,
                    previous_value=self._get_previous_indicator_value(
                        symbol, "bollinger"
                    ),
                    change_percent=self._calculate_change_percent(
                        bb_position,
                        self._get_previous_indicator_value(symbol, "bollinger"),
                    ),
            ...
            if stoch_data:
                indicators["stochastic"] = TechnicalIndicator(
                    ...
                    change_percent=self._calculate_change_percent(
                        stoch_data["k"],
                        self._get_previous_indicator_value(symbol, "stochastic"),
                    ),
            ...
            if not np.isnan(williams_r):
                indicators["williams_r"] = TechnicalIndicator(
                    ...
                    change_percent=self._calculate_change_percent(
                        williams_r,
                        self._get_previous_indicator_value(symbol, "williams_r"),
                    ),
            ...
            if not np.isnan(obv):
                indicators["obv"] = TechnicalIndicator(
                    ...
                    signal=self._determine_obv_signal(
                        obv, self._get_previous_indicator_value(symbol, "obv")
                    ),
                    strength=0.5,
    ...
    def _calculate_bollinger_bands(
        self, close: pd.Series, period: int = 20, std: float = 2
    ) -> Dict[str, float]:
        """ボリンジャーバンド計算"""
        try:
            if len(close) < period:
    ...
    def _calculate_williams_r(
        ...
        try:
            if len(close) < period:
                return np.nan
            return talib.WILLR(
                high.values, low.values, close.values, timeperiod=period
            )[-1]
        except:
            return np.nan
    ...
    def _determine_obv_signal(
        self, current_obv: float, previous_obv: float
    ) -> SignalDirection:
        """OBVシグナル判定"""
    ...
    def _calculate_performance_metrics(
        ...
        # 出来高変化率
        volume_change = (
            ((volumes[-1] - volumes[0]) / volumes[0]) * 100 if volumes[0] != 0 else 0.0
        )
        ...
        for symbol in self.symbols:
            try:
                ...
                for signal in signals:
                    ...
                    logger.info(
                        f"技術シグナル: {symbol} - {signal.signal_type.value} (信頼度: {signal.confidence:.2f})"
                    )
    ...
    def get_technical_summary(self) -> Dict[str, Any]:
        ...
        # パフォーマンス統計
        price_changes = [
            data.performance_metrics.get("price_change_percent", 0.0)
            for data in technical_data_list
        ]
        volatilities = [
            data.performance_metrics.get("volatility", 0.0)
            for data in technical_data_list
        ]
        ...
        return {
            ...
            "signal_distribution": {
                "buy": sum(
                    ...
                    if t in [SignalType.BUY, SignalType.STRONG_BUY]
                ),
                "sell": sum(
                    1
                    for t in signal_types
                    if t in [SignalType.SELL, SignalType.STRONG_SELL]
                ),
            ...
            "strength_distribution": {
                ...
                "medium": sum(
                    1 for s in signal_strengths if s == SignalStrength.MEDIUM
                ),
                "strong": sum(
                    1 for s in signal_strengths if s == SignalStrength.STRONG
                ),
                "very_strong": sum(
                    1 for s in signal_strengths if s == SignalStrength.VERY_STRONG
                ),
            },
            "performance_metrics": {
        ...
        try:
            ...
            for symbol, technical_data in self.technical_data.items():
                data["technical_data"][symbol] = {
                    ...
                    "indicator_history": {
                        name: list(history)
                        for name, history in technical_data.indicator_history.items()
                    },
...
async def main():
    ...
    async def signal_callback(signal: TechnicalSignal):
        print(
            f"📊 技術シグナル: {signal.symbol} - {signal.signal_type.value} (信頼度: {signal.confidence:.2f})"
        )
