...
class ChartDataOptimizer:
    ...
    def downsample_data(
        self, data: pd.DataFrame, max_points: Optional[int] = None
    ) -> pd.DataFrame:
        """データのダウンサンプリング"""
...
class ChartRenderer:
    ...
    def render_volume_chart(
        ...
        colors = [
            (
                "green"
                if optimized_data["Close"].iloc[i] >= optimized_data["Open"].iloc[i]
                else "red"
            )
...
class EnhancedChartOptimizer:
    ...
    def optimize_chart_rendering(
        self, data: pd.DataFrame, chart_type: str = "candlestick", **kwargs
    ) -> Dict[str, Any]:
        """チャートレンダリングの最適化"""
        start_time = time.time()
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        target_achievement_rate = (
            sum([r["target_achieved"] for r in recent_renders])
            / len(recent_renders)
            * 100
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if avg_render_time > self.config.target_render_time:
            recommendations.append(
                "レンダリング時間が目標を超えています。データポイント数を減らすことを推奨します。"
            )

        if target_achievement_rate < 80:
            recommendations.append(
                "目標達成率が80%を下回っています。より積極的な最適化を推奨します。"
            )

        if (
            self.data_optimizer.cache_stats["hits"]
            < self.data_optimizer.cache_stats["misses"]
        ):
            recommendations.append(
                "キャッシュヒット率が低いです。キャッシュ戦略の見直しを推奨します。"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"💾 元メモリ: {sample_data.memory_usage(deep=True).sum() / 1024 / 1024:.1f}MB"
    )

    # ローソク足チャートの最適化
    result = optimizer.optimize_chart_rendering(
        sample_data, chart_type="candlestick", title="最適化された株価チャート"
    )
