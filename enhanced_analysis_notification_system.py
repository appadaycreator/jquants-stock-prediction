...
class EnhancedNotificationSystem:
            ...
            try:
                ...
                if 'notification' in yaml_config:
                    ...
                    if 'email' in notif_config:
                        ...
                        self.config.smtp_server = email_config.get('smtp_server', self.config.smtp_server)
                        self.config.smtp_port = email_config.get('smtp_port', self.config.smtp_port)
                        self.config.email_user = email_config.get('email_user', self.config.email_user)
                        self.config.email_password = email_config.get('email_password', self.config.email_password)
                        self.config.email_to = [email_config.get('email_to', '')] if email_config.get('email_to') else []
                    
                    # Slack設定
                    if 'slack' in notif_config:
                        slack_config = notif_config['slack']
                        self.config.slack_enabled = slack_config.get('enabled', True)
                        self.config.slack_webhook_url = slack_config.get('webhook_url', self.config.slack_webhook_url)
                        self.config.slack_channel = slack_config.get('channel', self.config.slack_channel)
                        self.config.slack_username = slack_config.get('username', self.config.slack_username)
                    
                    # 通知フィルタリング
                    if 'filters' in notif_config:
                        filters = notif_config['filters']
                        self.config.min_confidence_threshold = filters.get('min_confidence_threshold', 0.7)
                        self.config.include_errors = filters.get('include_errors', True)
                        self.config.include_success = filters.get('include_success', True)
                    
                    # レート制限
                    if 'rate_limiting' in notif_config:
                        rate_limit = notif_config['rate_limiting']
                        self.config.rate_limit_per_hour = rate_limit.get('max_notifications_per_hour', 10)
                        
            except Exception as e:
                logger.error(f"設定ファイル読み込みエラー: {e}")
    
    async def send_analysis_notification(self, result: AnalysisResult, priority: NotificationPriority = NotificationPriority.NORMAL):
        """分析結果の通知を送信"""
        try:
            # レート制限チェック
    ...
    def _generate_notification_content(self, result: AnalysisResult, priority: NotificationPriority) -> Dict[str, Any]:
        """通知内容の生成"""
        # 基本情報
        content = {
        ...
        # エラー時の情報
            ...
            content.update({
                ...
                "suggested_actions": ["システム管理者に連絡", "ログを確認", "手動で再実行"]
            })
    ...
    def _generate_action_suggestions(self, result: AnalysisResult) -> List[str]:
        ...
        if result.risk_metrics:
            max_risk = max(result.risk_metrics.values()) if result.risk_metrics.values() else 0
            if max_risk > 0.7:
            ...
            for webhook_url in self.config.webhook_urls:
                try:
                    ...
                    if response.status_code in [200, 201, 202]:
                        ...
                        logger.error(f"Webhook通知送信失敗: {webhook_url} - {response.status_code}")
    ...
    def get_notification_stats(self) -> Dict[str, Any]:
        ...
        successful_notifications = sum(1 for n in self.notification_history if n['notification_success'])
...
async def main():
    ...
    result = AnalysisResult(
        ...
        recommendations=["買いシグナル検知", "リスク管理を強化", "ポートフォリオの再バランス"],
