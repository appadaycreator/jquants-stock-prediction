...
class IntegratedTradingSystem:
    ...
    def _generate_integrated_recommendations(self, signal_results: Dict, 
                                           analysis_results: Dict, 
                                           risk_report: Dict) -> List[TradingRecommendation]:
        """統合推奨事項を生成"""
        recommendations = []
        ...
        for signal_data in signal_results.get('top_signals', []):
            ...
            if symbol in analysis_results:
                ...
                # アクション決定
                if signal_type in ['BUY', 'STRONG_BUY'] and analysis.investment_opportunity in [InvestmentOpportunity.BUY, InvestmentOpportunity.STRONG_BUY]:
                    action = "BUY"
                    priority = 5 if signal_type == 'STRONG_BUY' else 4
                elif signal_type in ['SELL', 'STRONG_SELL'] and analysis.investment_opportunity in [InvestmentOpportunity.SELL, InvestmentOpportunity.STRONG_SELL]:
                    action = "SELL"
                ...
                stop_loss_price = entry_price * 0.95 if action == "BUY" else entry_price * 1.05
                take_profit_price = entry_price * 1.10 if action == "BUY" else entry_price * 0.90
    ...
    def _update_system_status(self, signal_results: Dict, 
                            analysis_results: Dict, 
                            risk_report: Dict) -> SystemStatus:
        """システムステータスを更新"""
        # アクティブポジション数
        active_positions = len([pos for pos in risk_report.get('positions', []) 
                              if pos['status'] == PositionStatus.OPEN.value])
        
        # シグナル統計
        total_signals = signal_results.get('signals_generated', 0)
        buy_signals = signal_results.get('summary', {}).get('buy_signals', 0) + \
                     signal_results.get('summary', {}).get('strong_buy_signals', 0)
        sell_signals = signal_results.get('summary', {}).get('sell_signals', 0) + \
                      signal_results.get('summary', {}).get('strong_sell_signals', 0)
        
        # ポートフォリオ価値
        portfolio_value = risk_report.get('account_value', self.account_value)
        unrealized_pnl = sum(pos['unrealized_pnl'] for pos in risk_report.get('positions', []))
    ...
    def _calculate_performance_metrics(self, signal_results: Dict, 
                                     analysis_results: Dict, 
        ...
        # シグナル品質
        signal_quality = np.mean([signal['confidence'] for signal in signal_results.get('top_signals', [])])
        ...
        # 総合スコア
        overall_score = (analysis_completion_rate * 0.3 + 
                        signal_quality * 0.3 + 
                        risk_efficiency * 0.2 + 
    ...
    def execute_trading_recommendations(self, recommendations: List[TradingRecommendation]) -> Dict:
        """取引推奨事項を実行"""
        execution_results = {
            'timestamp': datetime.now().isoformat(),
            ...
            try:
                if recommendation.action == "BUY":
                    ...
                    position = self.risk_system.add_position(
                        ...
                        quantity=int(recommendation.position_size / recommendation.entry_price),
                        position_type="LONG"
                    )
    ...
    def generate_dashboard_data(self) -> Dict:
        """ダッシュボード用データを生成"""
        # 最新の分析結果を取得
        if not self.last_analysis_time or (datetime.now() - self.last_analysis_time).seconds > 300:
            # 5分以上経過している場合は再分析
        ...
        dashboard_data = {
            ...
            'top_recommendations': analysis_results.get('trading_recommendations', [])[:5],
            'risk_summary': {
                'risk_score': analysis_results.get('risk_analysis', {}).get('risk_metrics', {}).get('risk_score', 0),
                'should_reduce_risk': analysis_results.get('risk_analysis', {}).get('should_reduce_risk', False),
                'high_risk_positions': analysis_results.get('risk_analysis', {}).get('high_risk_positions', [])
            },
            'portfolio_summary': {
                'total_symbols': analysis_results.get('system_status', {}).get('total_symbols', 0),
                'analyzed_symbols': analysis_results.get('portfolio_analysis', {}).get('analyzed_symbols', 0),
                'diversification_score': analysis_results.get('portfolio_analysis', {}).get('diversification_score', 0)
            }
        }
    ...
    def save_integrated_results(self, results: Dict, filename: str = "integrated_trading_results.json"):
        """統合結果を保存"""
        try:
    ...
    def run_continuous_monitoring(self, interval_minutes: int = 5):
        ...
        while True:
            try:
                ...
                # 推奨事項実行（自動取引が有効な場合）
                recommendations = [TradingRecommendation(**rec) for rec in results.get('trading_recommendations', [])]
                if recommendations:
                    execution_results = self.execute_trading_recommendations(recommendations)
                    logger.info(f"推奨事項実行完了: {len(execution_results['executed_actions'])}件")
...
def main():
    ...
    for i, rec in enumerate(recommendations[:5], 1):
        ...
        print(f"     価格: ¥{rec['entry_price']:.0f}, ポジション: ¥{rec['position_size']:,.0f}")
    ...
    print(f"  リスク削減推奨: {'はい' if risk_analysis.get('should_reduce_risk', False) else 'いいえ'}")
