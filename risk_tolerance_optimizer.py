...
class RiskToleranceOptimizer:
    ...
    def _create_profile_from_data(self, symbol: str, data: Dict[str, Any]) -> None:
        ...
        try:
            profile = RiskToleranceProfile(
                symbol=symbol,
                current_risk_level=RiskLevel(data.get('current_risk_level', 'moderate')),
    ...
    def calculate_risk_metrics(self, symbol: str, price_data: pd.DataFrame) -> RiskMetrics:
        """リスク指標を計算"""
        try:
    ...
    def optimize_risk_tolerance(self, symbol: str, risk_metrics: RiskMetrics) -> Dict[str, Any]:
        """リスク許容度を最適化"""
        ...
        if optimal_risk_level != profile.current_risk_level:
            ...
            profile.volatility_tolerance = self._calculate_volatility_tolerance(optimal_risk_level)
            profile.drawdown_tolerance = self._calculate_drawdown_tolerance(optimal_risk_level)
        ...
        return {
            ...
            'optimal_risk_level': optimal_risk_level.value,
            'risk_adjustment': risk_adjustment,
            'recommended_position_size': self._calculate_optimal_position_size(profile, risk_metrics),
    ...
    def _determine_optimal_risk_level(self, risk_metrics: RiskMetrics, risk_capacity: float) -> RiskLevel:
        """最適なリスクレベルを決定"""
    ...
    def _calculate_risk_adjustment(self, current_score: float, optimal_level: RiskLevel, profile: RiskToleranceProfile) -> Dict[str, Any]:
        """リスク調整を計算"""
        ...
        adjustment = {
            ...
            'volatility_tolerance_change': self._calculate_volatility_tolerance(optimal_level) - profile.volatility_tolerance,
            'drawdown_tolerance_change': self._calculate_drawdown_tolerance(optimal_level) - profile.drawdown_tolerance,
            'position_size_adjustment': self._calculate_position_size_adjustment(optimal_tolerance, profile.risk_tolerance)
        }
    ...
    def _calculate_position_size_adjustment(self, new_tolerance: float, current_tolerance: float) -> float:
        """ポジションサイズ調整を計算"""
    ...
    def _calculate_optimal_position_size(self, profile: RiskToleranceProfile, risk_metrics: RiskMetrics) -> float:
        """最適なポジションサイズを計算"""
    ...
    def get_portfolio_risk_summary(self) -> Dict[str, Any]:
        ...
        for risk_level in RiskLevel:
            count = sum(1 for profile in self.profiles.values() 
                       if profile.current_risk_level == risk_level)
            risk_distribution[risk_level.value] = count / total_stocks
        
        avg_risk_capacity = np.mean([profile.risk_capacity for profile in self.profiles.values()])
        avg_risk_tolerance = np.mean([profile.risk_tolerance for profile in self.profiles.values()])
