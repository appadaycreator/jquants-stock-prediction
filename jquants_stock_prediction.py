...
if compare_models:
    ...
    if not comparison_results.empty:
        ...
        for idx, row in comparison_results.iterrows():
            print(f"{row['model_name']:<15} | MAE: {row['mae']:.4f} | RMSE: {row['rmse']:.4f} | R²: {row['r2']:.4f}")
        ...
        print("❌ モデル比較で有効な結果が得られませんでした。デフォルトモデルを使用します。")
    ...
    if primary_model in models_config:
        ...
        print(f"警告: モデル '{primary_model}' の設定が見つかりません。デフォルト設定を使用します。")
...
if compare_models:
    print(f"   比較結果: {prediction_config.get('comparison_csv', 'model_comparison_results.csv')}")
