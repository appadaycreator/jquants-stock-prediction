...
class AdvancedPerformanceAnalyzer:
    ...
    def calculate_advanced_metrics(self, returns: pd.Series, 
        ...
        # パフォーマンス分解
        performance_decomposition = self._calculate_performance_decomposition(returns, benchmark_returns)
    ...
    def _calculate_basic_metrics(self, returns: pd.Series, equity_curve: pd.Series) -> Dict[str, float]:
        """基本指標計算"""
    ...
    def _calculate_risk_metrics(self, returns: pd.Series, equity_curve: pd.Series) -> Dict[str, float]:
        """リスク指標計算"""
        # 最大ドローダウン
        peak = equity_curve.expanding().max()
        drawdown = (equity_curve - peak) / peak
    ...
    def _calculate_risk_adjusted_metrics(self, returns: pd.Series, 
        ...
        # トレイナーレシオ
        treynor_ratio = (aligned_returns.mean() * 252 - self.risk_free_rate) / beta if beta > 0 else 0
    ...
    def _calculate_variance_analysis(self, returns: pd.Series, 
                                   benchmark_returns: Optional[pd.Series]) -> Dict[str, float]:
        """分散分析"""
        if benchmark_returns is None:
            return {
    ...
    def _calculate_stress_test(self, returns: pd.Series, equity_curve: pd.Series) -> Dict[str, float]:
        """ストレステスト"""
        # 月次リターン
    ...
    def _calculate_performance_decomposition(self, returns: pd.Series, 
                                           benchmark_returns: Optional[pd.Series]) -> Dict[str, float]:
        """パフォーマンス分解"""
        if benchmark_returns is None:
            return {
...
class PerformanceVisualizer:
    ...
    def plot_comprehensive_analysis(self, returns: pd.Series, equity_curve: pd.Series, 
                                  metrics: AdvancedMetrics, save_path: str = None):
        """包括的分析プロット"""
        ...
        # 3. リターン分布
        axes[0, 2].hist(returns * 100, bins=30, alpha=0.7, color='skyblue', edgecolor='black')
        axes[0, 2].axvline(returns.mean() * 100, color='red', linestyle='--', label=f'平均: {returns.mean()*100:.2f}%')
        ...
        axes[1, 0].plot(cumulative_returns.index, cumulative_returns.values * 100, linewidth=2, color='green')
        ...
        risk_values = [metrics.var_95 * 100, metrics.var_99 * 100, 
                      metrics.cvar_95 * 100, metrics.cvar_99 * 100]
        bars = axes[1, 2].bar(risk_metrics, risk_values, color=['lightcoral', 'red', 'darkred', 'maroon'])
        ...
        perf_values = [metrics.sharpe_ratio, metrics.sortino_ratio, 
                      metrics.calmar_ratio, metrics.information_ratio]
        bars = axes[2, 0].bar(perf_metrics, perf_values, color=['skyblue', 'lightgreen', 'orange', 'purple'])
        ...
        stat_values = [metrics.skewness, metrics.kurtosis, 
                      metrics.jarque_bera_stat, metrics.jarque_bera_pvalue]
        bars = axes[2, 1].bar(stat_metrics, stat_values, color=['lightblue', 'lightcoral', 'lightgreen', 'lightyellow'])
        ...
        stress_values = [metrics.worst_month * 100, metrics.worst_quarter * 100, 
                        metrics.worst_year * 100, metrics.consecutive_losses, metrics.recovery_time]
        bars = axes[2, 2].bar(stress_metrics, stress_values, color=['red', 'darkred', 'maroon', 'orange', 'yellow'])
    ...
    def plot_risk_analysis(self, returns: pd.Series, metrics: AdvancedMetrics, save_path: str = None):
        """リスク分析プロット"""
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        fig.suptitle('リスク分析', fontsize=16, fontweight='bold')
        
        # 1. VaR分析
        axes[0, 0].hist(returns * 100, bins=50, alpha=0.7, color='skyblue', density=True)
        axes[0, 0].axvline(metrics.var_95 * 100, color='red', linestyle='--', label=f'VaR 95%: {metrics.var_95*100:.2f}%')
        axes[0, 0].axvline(metrics.var_99 * 100, color='darkred', linestyle='--', label=f'VaR 99%: {metrics.var_99*100:.2f}%')
        ...
        axes[0, 1].hist(tail_returns * 100, bins=20, alpha=0.7, color='red', edgecolor='black')
        axes[0, 1].axvline(metrics.cvar_95 * 100, color='darkred', linestyle='--', 
        ...
        # 3. リスク指標比較
        risk_indicators = ['VaR 95%', 'VaR 99%', 'CVaR 95%', 'CVaR 99%', '最大ドローダウン']
        risk_values = [abs(metrics.var_95) * 100, abs(metrics.var_99) * 100, 
                      abs(metrics.cvar_95) * 100, abs(metrics.cvar_99) * 100, 
                      metrics.max_drawdown * 100]
        bars = axes[1, 0].bar(risk_indicators, risk_values, color=['lightcoral', 'red', 'darkred', 'maroon', 'black'])
        ...
        # 4. リスク調整リターン
        risk_adjusted_metrics = ['シャープレシオ', 'ソルティノレシオ', 'カルマーレシオ', '情報レシオ']
        risk_adjusted_values = [metrics.sharpe_ratio, metrics.sortino_ratio, 
                              metrics.calmar_ratio, metrics.information_ratio]
        bars = axes[1, 1].bar(risk_adjusted_metrics, risk_adjusted_values, 
...
def main():
    ...
    # 高度な指標計算
    analyzer = AdvancedPerformanceAnalyzer(risk_free_rate=0.02, benchmark_returns=benchmark_series)
    metrics = analyzer.calculate_advanced_metrics(returns_series, equity_curve, benchmark_series)
    ...
    visualizer.plot_comprehensive_analysis(returns_series, equity_curve, metrics, 
