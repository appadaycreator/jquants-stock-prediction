...
class ErrorHandler:
    ...
    def _build_error_message(self, error_info: Dict[str, Any]) -> str:
        ...
        if 'context_data' in error_info:
            context_str = ", ".join([f"{k}={v}" for k, v in
                                   error_info['context_data'].items()])
    ...
    def handle_api_error(self, error: Exception, api_name: str, endpoint: str = "", 
    ...
    def handle_data_processing_error(self, error: Exception, operation: str, 
                                   data_shape: tuple = None, column_name: str = None):
        """データ処理エラーの専用ハンドリング"""
    ...
    def handle_model_error(self, error: Exception, model_name: str, 
                          operation: str, model_params: Dict = None):
        """機械学習モデルエラーの専用ハンドリング"""
    ...
    def handle_file_error(self, error: Exception, file_path: str, operation: str):
        ...
        additional_info = {
            ...
            'file_readable': (os.access(file_path, os.R_OK) if file_path and
                            os.path.exists(file_path) else None)
        }
...
class SpecificErrorHandler:
    ...
    def handle_data_validation_error(self, error: Exception, 
                                   validation_type: str, 
    ...
    def handle_file_io_error(self, error: Exception,
                           file_path: str,
                           operation: str) -> bool:
        """ファイルI/Oエラーのハンドリング"""
        ...
        additional_info = {
            ...
            'file_readable': (os.access(file_path, os.R_OK) if
                            os.path.exists(file_path) else False)
        }
