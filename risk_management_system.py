...
class DynamicStopLoss:
    ...
    def calculate_stop_loss(self, position: Position, market_volatility: float) -> float:
        """動的損切り価格を計算"""
        if position.position_type == "LONG":
            # ボラティリティ調整
            volatility_adjustment = 1 + (market_volatility - 0.2) * 0.5  # ボラティリティが高いほど損切り幅を広げる
            ...
            if self.trailing_stop and position.current_price > position.entry_price:
                ...
                if profit_percent > self.trailing_percent:
                    ...
                    trailing_stop_price = position.current_price * (1 - self.trailing_percent / 100)
                    return max(trailing_stop_price, position.entry_price * (1 - adjusted_stop_loss / 100))
...
class PositionSizer:
    ...
    def adjust_position_size_for_correlation(self, 
                                           base_size: int, 
...
class PortfolioRiskMonitor:
    ...
    def calculate_portfolio_risk(self, account_value: float) -> RiskMetrics:
        ...
        portfolio_value = account_value + sum(pos.unrealized_pnl for pos in self.positions.values())
    ...
    def _calculate_sharpe_ratio(self) -> float:
        ...
        for symbol, prices in self.price_history.items():
            if len(prices) > 1:
                symbol_returns = [prices[i] / prices[i-1] - 1 for i in range(1, len(prices))]
    ...
    def _calculate_risk_score(self, total_exposure: float, portfolio_value: float, 
    ...
    def should_reduce_risk(self, risk_metrics: RiskMetrics) -> bool:
        """リスク削減が必要かどうか判定"""
        return (risk_metrics.risk_score > 0.7 or 
                risk_metrics.max_drawdown > 0.15 or
                risk_metrics.var_95 / risk_metrics.portfolio_value > 0.1)
...
class RiskManagementSystem:
    ...
    def add_position(self, symbol: str, entry_price: float, quantity: int, 
                    position_type: str = "LONG", market_volatility: float = 0.2) -> Position:
        """新しいポジションを追加"""
        # 損切り価格計算
        ...
        if position_type == "LONG":
            temp_position.take_profit_price = entry_price + (entry_price - temp_position.stop_loss_price) * 2
        else:
            temp_position.take_profit_price = entry_price - (temp_position.stop_loss_price - entry_price) * 2
    ...
    def check_stop_loss(self, symbol: str, market_volatility: float = 0.2) -> bool:
        ...
        position = self.positions[symbol]
        should_stop = self.dynamic_stop_loss.should_stop_loss(position, market_volatility)
    ...
    def _calculate_position_risk_score(self, position: Position) -> float:
        """個別ポジションのリスクスコア計算"""
        # 損失率
        loss_percent = abs(position.unrealized_pnl) / (position.entry_price * position.quantity)
        
        # 損切りまでの距離
        if position.position_type == "LONG":
            stop_distance = (position.current_price - position.stop_loss_price) / position.current_price
        else:
            stop_distance = (position.stop_loss_price - position.current_price) / position.current_price
    ...
    def get_risk_report(self) -> Dict:
        ...
        report = {
            ...
            'recommendations': self._generate_recommendations(risk_metrics, high_risk_positions)
        }
        
        return report
    
    def _generate_recommendations(self, risk_metrics: RiskMetrics, high_risk_positions: List[str]) -> List[str]:
        """リスク管理推奨事項生成"""
        recommendations = []
        ...
        if high_risk_positions:
            recommendations.append(f"高リスクポジション: {', '.join(high_risk_positions)} の損切りを検討してください。")
    ...
    def save_risk_report(self, report: Dict, filename: str = "risk_management_report.json"):
        """リスクレポートを保存"""
        try:
...
def main():
    ...
    for pos in report['positions']:
        ...
        print(f"  {pos['symbol']}: {pos['quantity']}株 @ ¥{pos['current_price']:.0f} "
