...
class MarketRegimeDetector:
    ...
    def detect_market_phase(self, data: pd.DataFrame, regime: MarketRegime) -> MarketPhase:
        """市場フェーズの検出"""
        try:
            if len(data) < 20:
    ...
    def _calculate_momentum(self, data: pd.DataFrame) -> float:
        ...
        # 10日間のリターン
        return (data['Close'].iloc[-1] - data['Close'].iloc[-10]) / data['Close'].iloc[-10]
...
class MarketEnvironmentAnalyzer:
    ...
    def analyze_market_environment(self, data: pd.DataFrame, market_data: Dict[str, Any] = None) -> MarketEnvironment:
        """市場環境の包括的分析"""
        try:
            # レジームとフェーズの検出
            ...
            # リスクレベル
            risk_level = self._assess_risk_level(volatility_level, trend_strength, sentiment_score)
    ...
    def _calculate_sentiment_score(self, data: pd.DataFrame, market_data: Dict[str, Any] = None) -> float:
        """センチメントスコアの計算"""
        # 簡易版センチメントスコア
        if len(data) < 10:
            return 0.5
        ...
        # ボリュームモメンタム
        volume_momentum = (data['Volume'].iloc[-5:].mean() - data['Volume'].iloc[-15:-5].mean()) / data['Volume'].iloc[-15:-5].mean()
        
        # 複合スコア
        sentiment = (price_momentum * 0.7 + volume_momentum * 0.3)
        return max(0.0, min(1.0, (sentiment + 1) / 2))  # 0-1の範囲に正規化
    
    def _assess_risk_level(self, volatility_level: str, trend_strength: float, sentiment_score: float) -> str:
        """リスクレベルの評価"""
        risk_score = 0
...
class StrategyEnvironmentAdjuster:
    ...
    def adjust_strategy_for_environment(
        ...
        try:
            ...
            # パラメータ調整
            adjusted_parameters = self._adjust_parameters(original_parameters, adjustment_rules)
            
            # リスク調整
            risk_adjustment = self._calculate_risk_adjustment(environment)
            
            # ポジションサイズ調整
            position_size_adjustment = self._calculate_position_size_adjustment(environment)
            
            # ストップロス・利確調整
            stop_loss_adjustment, take_profit_adjustment = self._calculate_stop_take_adjustment(environment)
            
            # 調整理由の生成
            adjustment_reason = self._generate_adjustment_reason(environment, adjustment_rules)
            
            # 信頼度スコア
            confidence_score = self._calculate_adjustment_confidence(environment, adjustment_rules)
            
            # 期待インパクト
            expected_impact = self._assess_expected_impact(environment, adjusted_parameters)
    ...
    def _adjust_parameters(self, original_parameters: Dict[str, Any], rules: Dict[str, Any]) -> Dict[str, Any]:
        """パラメータの調整"""
    ...
    def _calculate_risk_adjustment(self, environment: MarketEnvironment) -> Dict[str, float]:
        """リスク調整の計算"""
        base_risk = {
            "max_drawdown": 0.10,
            "var_95": 0.05,
    ...
    def _calculate_position_size_adjustment(self, environment: MarketEnvironment) -> float:
        """ポジションサイズ調整の計算"""
        base_size = 1.0
    ...
    def _calculate_stop_take_adjustment(self, environment: MarketEnvironment) -> Tuple[float, float]:
        """ストップロス・利確調整の計算"""
        base_stop = 1.0
        base_take = 1.0
...
def main():
    ...
    try:
        ...
        # 戦略の分析と調整
        result = env_strategy_system.analyze_and_adjust_strategies(symbol, data, strategies)
