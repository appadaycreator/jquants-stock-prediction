...
class ParallelPerformanceTester:
    ...
    def _run_advanced_optimization_tests(self):
        ...
        try:
            from advanced_parallel_optimizer import (
                get_advanced_optimizer,
                execute_optimized_parallel,
            )
    ...
    def _test_optimized_parallel_processing(self, optimizer) -> PerformanceTestResult:
        ...
        results = optimizer.execute_optimized_parallel(
            tasks, task_type="mixed"
        )  # mixedに変更
        ...
        for workers in worker_counts:
            ...
            if throughput > best_throughput:
                ...
                best_result = PerformanceTestResult(
                    ...
                    throughput=throughput,
                    efficiency=(
                        throughput / (psutil.cpu_percent() / 100)
                        if psutil.cpu_percent() > 0
                        else 0
                    ),
                    success_rate=100.0,
    ...
    def _generate_performance_report(self):
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            for _, row in df_results.iterrows():
                f.write(
                    f"| {row['Test Name']} | {row['Execution Time (s)']:.2f} | {row['CPU Usage (%)']:.1f} | {row['Memory Usage (%)']:.1f} | {row['Throughput (tasks/s)']:.2f} | {row['Efficiency']:.2f} | {row['Success Rate (%)']:.1f} |\n"
                )

            f.write("\n## 性能分析\n\n")

            # 最適な結果を特定
            best_throughput = df_results.loc[
                df_results["Throughput (tasks/s)"].idxmax()
            ]
            ...
            f.write(
                f"- スループット: {best_throughput['Throughput (tasks/s)']:.2f} tasks/s\n"
            )
            ...
            f.write(
                f"- 利用可能メモリ: {psutil.virtual_memory().available / (1024**3):.1f} GB\n"
            )
