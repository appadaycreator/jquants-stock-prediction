...
class EnhancedSentimentTradingSystem:
    ...
    def _initialize_sentiment_system(self):
        ...
        try:
            ...
            if news_api_key and all(twitter_credentials.values()):
                self.sentiment_system = SentimentTradingSystem(news_api_key, twitter_credentials)
    ...
    async def generate_enhanced_signals(self, symbols: List[str], 
                                      lookback_days: int = 30) -> List[EnhancedTradingSignal]:
        """拡張トレーディングシグナルの生成"""
        enhanced_signals = []
        ...
        try:
            for symbol in symbols:
                # 技術分析シグナルの取得
                technical_signal = await self._get_technical_signal(symbol, lookback_days)
    ...
    async def _get_technical_signal(self, symbol: str, lookback_days: int) -> Dict[str, Any]:
        """技術分析シグナルの取得"""
        try:
            # 株価データの取得
    ...
    def _combine_signals(self, technical_signal: Dict[str, Any], 
                        sentiment_signal: Dict[str, Any]) -> Dict[str, Any]:
        """技術分析と感情分析の統合"""
        try:
            # 重み付きスコアの計算
            ...
            combined_confidence = (technical_confidence * technical_weight) + (sentiment_confidence * sentiment_weight)
    ...
    def _calculate_risk_parameters(self, symbol: str, combined_signal: Dict[str, Any]) -> Dict[str, Any]:
        """リスク管理パラメータの計算"""
        try:
            # 現在価格の取得
            ticker = yf.Ticker(symbol)
    ...
    def update_performance_metrics(self, trade_result: Dict[str, Any]):
        ...
        try:
            ...
            if len(self.sentiment_history) > 10:
                correct_predictions = sum(1 for h in self.sentiment_history[-20:] 
                                       if (h['score'] > 0 and h.get('actual_return', 0) > 0) or 
                                          (h['score'] < 0 and h.get('actual_return', 0) < 0))
                self.performance_metrics['sentiment_accuracy'] = correct_predictions / len(self.sentiment_history[-20:])
