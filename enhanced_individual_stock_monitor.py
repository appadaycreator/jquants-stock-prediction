...
try:
    ...
    from multi_stock_monitor import MultiStockMonitor, StockAnalysis, InvestmentOpportunity
...
class EnhancedIndividualStockMonitor:
    ...
    async def _update_single_stock(self, symbol: str):
        ...
        try:
            ...
            with self.lock:
                ...
                monitor.change_percent = ((current_price - previous_price) / previous_price * 100) if previous_price > 0 else 0.0
                monitor.volume = current_volume
                monitor.volume_change_percent = ((current_volume - previous_volume) / previous_volume * 100) if previous_volume > 0 else 0.0
                ...
                if sentiment_data:
                    ...
                    monitor.sentiment_type = sentiment_data.get("type", SentimentType.NEUTRAL)
                ...
                monitor.risk_level = self._determine_risk_level(
                    technical_indicators, monitor.change_percent, monitor.volume_change_percent
                )
    ...
    async def _check_alerts(self, symbol: str):
        ...
        if abs(monitor.change_percent) >= self.config["price_change_threshold"]:
            await self._create_alert(
                ...
                priority=AlertPriority.HIGH if abs(monitor.change_percent) > 5 else AlertPriority.MEDIUM,
        ...
        if monitor.volume_change_percent >= self.config["volume_spike_threshold"]:
            await self._create_alert(
                ...
                threshold_value=monitor.volume_history[-2] if len(monitor.volume_history) > 1 else monitor.volume,
        ...
        if abs(monitor.sentiment_score) >= self.config["sentiment_change_threshold"]:
            await self._create_alert(
                ...
                change_percent=0,
                technical_indicators=monitor.technical_indicators,
                sentiment_data={"score": monitor.sentiment_score, "type": monitor.sentiment_type}
            )
    ...
    async def _send_alert_notification(self, alert: StockAlert):
        ...
        try:
            # ログ出力
            logger.warning(f"ALERT [{alert.priority.name}] {alert.symbol}: {alert.message}")
...
時刻: {alert.timestamp}
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            active_monitors = len([m for m in self.monitors.values() if m.current_price > 0])
            
            # パフォーマンス統計
            changes = [m.change_percent for m in self.monitors.values() if m.current_price > 0]
            ...
            buy_opportunities = sum(1 for opp in opportunities if opp in [InvestmentOpportunity.BUY, InvestmentOpportunity.STRONG_BUY])
    ...
    def save_monitoring_data(self, filename: str = "individual_monitoring_data.json"):
        ...
        try:
            data = {
                ...
                "monitors": {},
                "recent_alerts": [asdict(alert) for alert in self.get_recent_alerts(24)]
            }
