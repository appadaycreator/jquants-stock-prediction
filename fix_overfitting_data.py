...
def generate_realistic_stock_data(n_days=100, n_stocks=5):
    ...
    stocks = ['7203', '6758', '9984', '6861', '7974']  # トヨタ、ソニー、ソフトバンクG、キーエンス、任天堂
    data = []
    ...
    for stock in stocks:
        ...
        sma_5 = pd.Series(prices_array).rolling(window=5).mean().fillna(prices_array[0]).tolist()
        sma_10 = pd.Series(prices_array).rolling(window=10).mean().fillna(prices_array[0]).tolist()
        sma_25 = pd.Series(prices_array).rolling(window=25).mean().fillna(prices_array[0]).tolist()
        sma_50 = pd.Series(prices_array).rolling(window=50).mean().fillna(prices_array[0]).tolist()
...
def main():
    ...
    with open('web-app/public/data/evaluation_summary.json', 'w', encoding='utf-8') as f:
