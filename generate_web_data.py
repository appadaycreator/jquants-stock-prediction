...
class WebDataGenerator:
            ...
            top_gainers = [
                {
                    "code": stock.get("code", ""),
                    "name": stock.get("name", ""),
                    "change_percent": stock.get("current_price", {}).get("change_percent", 0),
    ...
    def generate_today_summary(self, stock_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 今日の市場状況
            total_stocks = len(stocks)
            gainers = sum(1 for stock in stocks.values() 
                         if stock.get("current_price", {}).get("change_percent", 0) > 0)
            losers = sum(1 for stock in stocks.values() 
                        if stock.get("current_price", {}).get("change_percent", 0) < 0)
            
            # 平均変動率
            changes = [stock.get("current_price", {}).get("change_percent", 0) 
        ...
        try:
            ...
            for code, stock_info in stocks.items():
                ...
                prediction = {
                    ...
                    "predicted_price": prediction_data.get("predicted_price", current_price * 1.02),
