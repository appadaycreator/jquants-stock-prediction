...
def validate_and_select_features(df, config_features, target):
    ...
    if len(valid_features) == 0:
        ...
        valid_features = [col for col in numeric_cols if col != target and col not in ['Date', 'Code', 'CompanyName']]
...
def generate_web_data():
            ...
            for idx in X_test.index:
                if idx < len(df):
                    stock_data.append({
                        ...
                        "Actual": float(y_test.iloc[idx - X_test.index[0]]),
                        "Predicted": float(y_pred[idx - X_test.index[0]]) if idx - X_test.index[0] < len(y_pred) else None
                    })
        else:
            stock_data = []
    ...
    try:
        performance_metrics = {
            "model_name": "XGBoost" if 'XGBRegressor' in str(type(model)) else "Random Forest",
    ...
    try:
        model_comparison = [{
            ...
            "model_type": "XGBoost" if 'XGBRegressor' in str(type(model)) else "Random Forest",
            "mae": float(metrics.get('mae', 0)),
            "rmse": float(metrics.get('rmse', 0)),
            "r2": float(metrics.get('r2', 0))
        }]
    ...
    try:
        dashboard_summary = {
            ...
            "best_model": "XGBoost" if 'XGBRegressor' in str(type(model)) else "Random Forest",
