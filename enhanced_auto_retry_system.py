...
class EnhancedAutoRetrySystem:
            ...
            try:
                ...
                if config.retry_on_success and attempt < config.max_attempts - 1:
                    ...
                    if config.log_retries:
                        logger.info(f"操作成功（リトライ継続）: {context.operation_name} - 試行 {attempt + 1}/{config.max_attempts}")
                ...
                if attempt < config.max_attempts - 1:
                    delay = self._calculate_delay(attempt, config)
                    if config.log_retries:
                        logger.warning(f"操作失敗、リトライ予定: {context.operation_name} - 試行 {attempt + 1}/{config.max_attempts} - エラー: {str(e)}")
                    await asyncio.sleep(delay)
                    retry_times.append(delay)
                else:
                    # 最後の試行で失敗
    ...
    def _is_api_error(self, error: Exception) -> bool:
        ...
        error_str = str(error).lower()
        api_keywords = [
            'api', 'http', 'status', 'unauthorized', 'forbidden',
            'not found', 'server error', 'bad request'
        ]
    ...
    def get_retry_statistics(self) -> Dict[str, Any]:
        ...
        successful_operations = sum(1 for record in self.retry_history if record["success"])
        ...
        for op_name in operation_stats:
            ...
            operation_stats[op_name]["average_attempts"] = sum(attempts) / len(attempts) if attempts else 0
            operation_stats[op_name]["success_rate"] = operation_stats[op_name]["success"] / operation_stats[op_name]["total"]
    ...
    async def retry_with_circuit_breaker(
        ...
        **kwargs
        ...
        # 通常のリトライ実行
        result = await self.retry_operation(operation, operation_name, config, *args, **kwargs)
