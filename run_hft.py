...
class HFTRunner:
    ...
    def initialize(self):
        ...
        try:
            # 高頻度取引システム初期化
            hft_config = {
                "latency_threshold": self.config.get("latency", {}).get(
                    "threshold", 0.001
                ),
                "profit_threshold": self.config.get("profit", {}).get(
                    "threshold", 0.001
                ),
                "max_position_size": self.config.get("risk", {}).get(
...
def main():
    ...
    parser.add_argument(
        "--config",
        "-c",
        default="hft_config.yaml",
        help="設定ファイルのパス (デフォルト: hft_config.yaml)",
    )
    parser.add_argument("--test", "-t", action="store_true", help="テストモードで実行")
    parser.add_argument(
        "--simulation", "-s", action="store_true", help="シミュレーションモードで実行"
    )
