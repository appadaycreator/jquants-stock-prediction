...
class TypeSafeValidator:
    ...
    def validate_dataframe_structure(self, df: pd.DataFrame, required_columns: List[str]) -> Dict[str, Any]:
        """DataFrameの構造検証"""
    ...
    def validate_numeric_columns(self, df: pd.DataFrame, numeric_columns: List[str]) -> Dict[str, Any]:
        """数値カラムの型安全性検証"""
        ...
        for col in numeric_columns:
            ...
            try:
                ...
                if original_dtype != converted_series.dtype:
                    ...
                    result["warnings"].append(f"カラム '{col}' の型変換: {original_dtype} -> {converted_series.dtype}")
            ...
            try:
                ...
                if not converted_dates.empty:
                    ...
                    if min_date.year < 1900 or max_date.year > current_year + 1:
                        ...
                        result["warnings"].append(f"カラム '{col}' に異常な日付範囲: {min_date} - {max_date}")
    ...
    def safe_nan_handling(self, df: pd.DataFrame, strategy: str = "forward_fill") -> pd.DataFrame:
        """安全なNaN値処理"""
        ...
        try:
            if strategy == "forward_fill":
                ...
                df_processed = df_processed.fillna(method='bfill')  # 前方補完で処理できない場合
            elif strategy == "backward_fill":
                df_processed = df_processed.fillna(method='bfill')
                df_processed = df_processed.fillna(method='ffill')  # 後方補完で処理できない場合
            elif strategy == "mean":
    ...
    def create_type_safe_dataframe(self, data: Union[Dict, List, pd.DataFrame], 
