...
def run_confidence_based_trading_example():
    ...
    # 設定の読み込み
    config = load_config()
    confidence_config = config.get('personal_investment_enhancement', {}).get('confidence_based_trading', {})
    ...
    for i, (prediction, market_info) in enumerate(zip(predictions, market_data)):
        ...
        if decision['should_trade']:
            print(f"日付: {market_info['date']}, 予測: {prediction:.3f}, "
...
def run_enhanced_risk_management_example():
    ...
    # 設定の読み込み
    config = load_config()
    risk_config = config.get('personal_investment_enhancement', {}).get('enhanced_risk_management', {})
...
def run_advanced_position_sizing_example():
    ...
    # 設定の読み込み
    config = load_config()
    position_config = config.get('personal_investment_enhancement', {}).get('advanced_position_sizing', {})
    ...
    for symbol, position_info in portfolio_result['positions'].items():
        print(f"{symbol}: {position_info['position_size']}株, "
...
def run_article_inspired_backtest_example():
    ...
    # 設定の読み込み
    config = load_config()
    backtest_config = config.get('personal_investment_enhancement', {}).get('article_inspired_backtest', {})
    ...
    # 改善された手法によるバックテスト
    enhanced_result = backtest_system.run_enhanced_backtest(predictions, prices, confidence_scores)
    ...
    # 改善効果の計算
    return_improvement = enhanced_result['total_return'] - article_result['total_return']
...
def run_ensemble_prediction_example():
    ...
    # 設定の読み込み
    config = load_config()
    ensemble_config = config.get('personal_investment_enhancement', {}).get('ensemble_prediction', {})
    ...
    # モデルの学習
    training_result = ensemble_system.train_ensemble_models(X_train, y_train, X_test, y_test)
    ...
    if training_result['training_successful']:
        ...
        if 'error' not in performance_eval:
            ...
            print(f"最良の個別モデルとの改善: {performance_eval['improvement_over_best_individual']:.3f}")
