...
class EnhancedNewsSentimentIntegration:
            ...
            try:
                ...
                for item in news_items:
                    ...
                    relevance_score = self._calculate_news_relevance(
                        f"{item.get('title', '')} {item.get('description', '')}", symbol
                    )
    ...
    def _calculate_social_relevance(self, text: str, symbol: str) -> float:
        ...
        relevance_score = min(
            (symbol_mentions + keyword_matches) / (len(keywords) + 1), 1.0
        )
        ...
        for symbol in self.stock_keywords.keys():
            try:
                ...
                with self.lock:
                    ...
                    stock_sentiment.social_mentions = len(
                        social_sentiment.get("posts", [])
                    )
    ...
    async def _analyze_news_sentiment(self, symbol: str) -> Dict[str, Any]:
        """ニュース感情分析"""
        with self.lock:
            news_data = self.news_cache.get(
                symbol, {"items": [], "timestamp": datetime.now()}
            )
        ...
        weighted_scores = [
            score * relevance
            for score, relevance in zip(sentiment_scores, relevance_scores)
        ]
    ...
    async def _analyze_social_sentiment(self, symbol: str) -> Dict[str, Any]:
        """ソーシャルメディア感情分析"""
        with self.lock:
            social_data = self.social_cache.get(
                symbol, {"posts": [], "timestamp": datetime.now()}
            )
        ...
        weighted_scores = [
            score * weight for score, weight in zip(sentiment_scores, weights)
        ]
    ...
    def _calculate_sentiment_trend(
        self, symbol: str, current_sentiment: Dict[str, Any]
    ) -> SentimentTrend:
        """感情トレンドの計算"""
        with self.lock:
            stock_sentiment = self.stock_sentiments[symbol]
        ...
        older_scores = (
            [h["score"] for h in history[-6:-3]] if len(history) >= 6 else recent_scores
        )
    ...
    def get_individual_sentiment(
        self, symbol: str
    ) -> Optional[IndividualStockSentiment]:
        """個別銘柄の感情分析結果取得"""
