...
class IntegratedSentimentEnhancementSystem:
    ...
    async def perform_integrated_analysis(self, symbols: List[str]) -> List[IntegratedSentimentAnalysis]:
        """統合感情分析の実行"""
    ...
    async def _generate_realtime_metrics(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            if self.metrics_generator:
                metrics = await self.metrics_generator.generate_realtime_metrics([symbol])
                return {
        ...
        try:
            if self.risk_system:
                ...
                return {
                    ...
                    "adjustment_summary": self.risk_system.get_adjustment_summary(symbol),
                    "recommendations": self.risk_system.get_risk_adjustment_recommendations(symbol)
                }
            else:
    ...
    async def _perform_trend_prediction(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            if self.trend_predictor:
                # 短期予測（15分）
                short_prediction = await self.trend_predictor.predict_sentiment_trend(symbol, horizon=15)
                
                # 中期予測（60分）
                medium_prediction = await self.trend_predictor.predict_sentiment_trend(symbol, horizon=60)
                ...
                return {
                    ...
                    "prediction_summary": self.trend_predictor.get_prediction_summary(symbol)
                }
            else:
    ...
    def _generate_trading_recommendations(self, symbol: str, realtime_metrics: Dict[str, Any], 
                                       risk_adjustments: Dict[str, Any], 
                                       trend_predictions: Dict[str, Any]) -> List[str]:
        """取引推奨事項の生成"""
        recommendations = []
            ...
            if "metrics" in realtime_metrics:
                for metric in realtime_metrics["metrics"]:
                    if metric.get("alert_level") in ["high", "critical"]:
                        recommendations.append(f"アラート: {metric.get('metric_type')} で異常値が検出されました")
            ...
            if "trend_patterns" in trend_predictions:
                for pattern in trend_predictions["trend_patterns"]:
                    ...
                    if pattern_type == "bullish":
                        ...
                        recommendations.append("下降トレンドパターンが検出されました。ネガティブな感情の継続に注意してください。")
    ...
    def _calculate_overall_confidence(self, realtime_metrics: Dict[str, Any], 
                                    risk_adjustments: Dict[str, Any], 
                                    trend_predictions: Dict[str, Any]) -> float:
        """総合信頼度の計算"""
        try:
            confidences = []
            ...
            # 感情トレンド予測の信頼度
            if "short_term_prediction" in trend_predictions:
                pred_conf = trend_predictions["short_term_prediction"].get("confidence", 0.5)
                confidences.append(pred_conf)
            
            if "medium_term_prediction" in trend_predictions:
                pred_conf = trend_predictions["medium_term_prediction"].get("confidence", 0.5)
                confidences.append(pred_conf)
    ...
    def _determine_overall_sentiment(self, realtime_metrics: Dict[str, Any], 
                                   trend_predictions: Dict[str, Any]) -> str:
        """総合感情の判定"""
        try:
            sentiment_scores = []
            ...
            # 予測から感情スコアを取得
            if "short_term_prediction" in trend_predictions:
                pred_sentiment = trend_predictions["short_term_prediction"].get("predicted_sentiment", 0)
    ...
    def _calculate_reliability_score(self, analysis: IntegratedSentimentAnalysis) -> float:
        """信頼性スコアの計算"""
        try:
            # 信頼度スコア
            ...
            # 総合信頼性スコア
            reliability = (confidence_score * 0.6 + recommendation_score * 0.4) - error_penalty
    ...
    async def start_continuous_monitoring(self, symbols: List[str], interval: int = 300):
        """継続監視の開始"""
        ...
        while self.is_running:
            try:
                ...
                for analysis in analyses:
                    logger.info(f"シンボル {analysis.symbol}: {analysis.overall_sentiment} "
                ...
                if self.performance_history:
                    ...
                    avg_reliability = np.mean([p.reliability_score for p in recent_performance])
                    
                    logger.info(f"性能統計 - 平均処理時間: {avg_processing_time:.2f}秒, "
    ...
    def _get_performance_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                "avg_processing_time": np.mean([p.total_processing_time for p in recent_performance]),
                "avg_metrics_time": np.mean([p.metrics_generation_time for p in recent_performance]),
                "avg_risk_time": np.mean([p.risk_adjustment_time for p in recent_performance]),
                "avg_prediction_time": np.mean([p.prediction_time for p in recent_performance]),
                "avg_accuracy": np.mean([p.accuracy_score for p in recent_performance]),
                "avg_reliability": np.mean([p.reliability_score for p in recent_performance]),
