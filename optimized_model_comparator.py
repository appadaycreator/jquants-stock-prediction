...
class ModelCache:
    ...
    def _generate_cache_key(self, model_name: str, X_hash: str, y_hash: str, params: Dict) -> str:
        """キャッシュキーを生成"""
    ...
    def get_cached_model(self, model_name: str, X_train: np.ndarray, y_train: np.ndarray, params: Dict) -> Optional[Any]:
        """キャッシュからモデルを取得"""
        try:
    ...
    def save_model(self, model: Any, model_name: str, X_train: np.ndarray, y_train: np.ndarray, params: Dict):
        """モデルをキャッシュに保存"""
        try:
            X_hash = self._generate_data_hash(X_train)
            y_hash = self._generate_data_hash(y_train)
...
class FeatureCache:
    ...
    def get_cached_features(self, data_hash: str, feature_func_name: str, *args, **kwargs):
        """キャッシュから特徴量を取得"""
    ...
    def save_features(self, data_hash: str, feature_func_name: str, result: Any, *args, **kwargs):
        """特徴量をキャッシュに保存"""
        cache_key = f"{data_hash}_{feature_func_name}_{hash(str(args))}_{hash(str(sorted(kwargs.items())))}"
...
class OptimizedModelComparator:
    ...
    def compare_models_optimized(
        ...
        if use_parallel:
            results = self._compare_models_parallel(
                ...
                feature_names, data_hash, use_cross_validation, cv_folds
            )
        else:
    ...
    def _compare_models_parallel(
        self, models_config: Dict, X_train: np.ndarray, X_test: np.ndarray,
        y_train: np.ndarray, y_test: np.ndarray, feature_names: List[str],
        data_hash: str, use_cross_validation: bool, cv_folds: int
    ) -> List[ModelResult]:
        """並列処理によるモデル比較"""
        results = []
    ...
    def _evaluate_single_model(
        self, model_name: str, config: Dict, X_train: np.ndarray, X_test: np.ndarray,
        y_train: np.ndarray, y_test: np.ndarray, feature_names: List[str],
        data_hash: str, use_cross_validation: bool, cv_folds: int
        ...
        try:
            ...
            cached_model = None
            if self.model_cache:
                cached_model = self.model_cache.get_cached_model(model_name, X_train, y_train, params)
            ...
            # メトリクス計算（キャッシュ活用）
            metrics = self._calculate_metrics_cached(y_test, y_pred, data_hash, model_name)
            
            # クロスバリデーション（オプション）
            cv_scores = None
            if use_cross_validation:
                cv_scores = self._calculate_cross_validation(model, X_train, y_train, cv_folds)
            ...
            self.logger.info(f"✅ {model_name} 完了: MAE={metrics['mae']:.4f}, 時間={training_time:.2f}s")
            return result
    ...
    def _calculate_metrics_cached(self, y_test: np.ndarray, y_pred: np.ndarray, data_hash: str, model_name: str) -> Dict[str, float]:
        """キャッシュを活用したメトリクス計算"""
        # キャッシュキーを生成
    ...
    def _calculate_cross_validation(self, model: Any, X: np.ndarray, y: np.ndarray, cv_folds: int) -> List[float]:
        """クロスバリデーションスコアを計算"""
        try:
    ...
    def _estimate_memory_usage(self, model: Any, X_train: np.ndarray, X_test: np.ndarray) -> float:
        """メモリ使用量を推定（MB）"""
        try:
            import sys
    ...
    def _log_comparison_summary(self, df_results: pd.DataFrame):
        ...
        self.logger.info(f"  🏆 最優秀モデル: {df_results.iloc[0]['model_name']} (MAE: {df_results.iloc[0]['mae']:.4f})")
        ...
        for i, (_, row) in enumerate(top_3.iterrows(), 1):
            self.logger.info(f"  {i}. {row['model_name']}: MAE={row['mae']:.4f}, R²={row['r2']:.4f}")
...
def create_optimized_comparator(max_workers: int = None, use_cache: bool = True) -> OptimizedModelComparator:
    """最適化されたモデル比較器を作成"""
