...
from advanced_backtest_system import BacktestEngine, MomentumStrategy, MeanReversionStrategy, BreakoutStrategy
from advanced_strategy_framework import MultiStrategyManager, AdvancedStrategyOptimizer
from advanced_performance_metrics import AdvancedPerformanceAnalyzer, PerformanceVisualizer
...
class IntegratedBacktestSystem:
    ...
    def fetch_market_data(self, start_date: datetime, end_date: datetime) -> Dict[str, pd.DataFrame]:
        """市場データ取得"""
    ...
    def _fetch_single_symbol(self, symbol: str, start_date: datetime, end_date: datetime) -> Optional[pd.DataFrame]:
        """単一銘柄データ取得"""
        try:
    ...
    def run_comprehensive_backtest(self, start_date: datetime, end_date: datetime) -> IntegratedBacktestResult:
        """包括的バックテスト実行"""
        ...
        # 4. ポートフォリオ最適化
        portfolio_performance = self._optimize_portfolio(strategy_results, optimized_weights)
        ...
        # 6. 統合結果計算
        combined_metrics = self._calculate_combined_metrics(strategy_results, optimized_weights)
    ...
    def _run_individual_strategies(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """個別戦略バックテスト実行"""
    ...
    def _optimize_strategies(self, strategy_results: Dict[str, Any]) -> Dict[str, float]:
        """戦略最適化"""
        logger.info("戦略最適化開始")
        ...
        for strategy_name, perf in strategy_performance.items():
            weight = perf['avg_sharpe'] / total_sharpe if total_sharpe > 0 else 1/len(strategy_performance)
    ...
    def _optimize_portfolio(self, strategy_results: Dict[str, Any], 
        ...
        portfolio_return = sum(
            symbol_weights.get(symbol, 0) * np.mean([
        ...
        portfolio_performance = {
            ...
            'diversification_ratio': len(symbol_weights) / sum(symbol_weights.values()) if symbol_weights else 0,
    ...
    def _calculate_advanced_metrics(self, strategy_results: Dict[str, Any]) -> Dict[str, Any]:
        """高度な指標計算"""
        ...
        for strategy_name in ['Momentum', 'MeanReversion', 'Breakout']:
            ...
            if strategy_risks:
                risk_metrics[strategy_name] = {
                    'avg_max_drawdown': np.mean([r['max_drawdown'] for r in strategy_risks]),
                    'avg_sharpe': np.mean([r['sharpe_ratio'] for r in strategy_risks]),
                    'avg_win_rate': np.mean([r['win_rate'] for r in strategy_risks]),
                    'risk_consistency': 1 - np.std([r['max_drawdown'] for r in strategy_risks])
                }
    ...
    def _calculate_combined_metrics(self, strategy_results: Dict[str, Any], 
                                 optimized_weights: Dict[str, float]) -> Dict[str, float]:
        """統合指標計算"""
    ...
    def generate_comprehensive_report(self, result: IntegratedBacktestResult) -> str:
        ...
        for symbol, results in result.strategy_results.items():
            ...
            for strategy_name, result_data in results.items():
                if result_data:
                    report.append(f"    {strategy_name}: リターン {result_data['total_return']:.2%}, "
    ...
    def save_results(self, result: IntegratedBacktestResult, base_filename: str = "integrated_backtest"):
        """結果保存"""
        logger.info("結果保存開始")
        ...
        logger.info(f"結果を保存しました: {base_filename}_results.json, {base_filename}_report.txt")
