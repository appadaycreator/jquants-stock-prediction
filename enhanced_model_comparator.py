...
class EnhancedModelResult:
    ...
    # 追加: 基準線（訓練平均予測）に対する勝率/誤差削減DD
    win_rate_vs_baseline: Optional[float] = None
    max_drawdown_error_reduction: Optional[float] = None
...
class IntelligentCacheManager:
    ...
    def __init__(
        self, cache_dir: str = "enhanced_model_cache", max_cache_size_mb: int = 2048
    ):
    ...
    def _generate_enhanced_cache_key(
        ...
        """強化されたキャッシュキーを生成"""
        feature_hash = (
            hashlib.md5(str(feature_names).encode()).hexdigest()[:8]
            if feature_names
            else "none"
        )
    ...
    def get_cached_model(
        ...
        try:
            ...
            cache_key = self._generate_enhanced_cache_key(
                model_name, X_hash, y_hash, params, feature_names
            )
...
class ParallelModelProcessor:
    ...
    def process_models_parallel(
        ...
        y_test: np.ndarray,
        feature_names: List[str] = None,
        cache_manager: IntelligentCacheManager = None,
    ) -> List[EnhancedModelResult]:
        """モデルを並列処理で評価"""
        self.logger.info(
            f"🚀 並列モデル処理開始 (ワーカー数: {self.max_workers}, プロセス: {self.use_processes})"
        )
    ...
    def _process_models_with_processes(
        self,
        models_config: Dict,
        X_train: np.ndarray,
        X_test: np.ndarray,
    ...
    def _process_models_with_threads(
        self,
        models_config: Dict,
        X_train: np.ndarray,
        X_test: np.ndarray,
    ...
    def _evaluate_single_model_isolated(
        self,
        model_name: str,
        config: Dict,
        X_train: np.ndarray,
        ...
        try:
            ...
            # 基準線（訓練平均）に対する比較
            baseline_value = float(np.mean(y_train)) if len(y_train) > 0 else 0.0
            ...
            if len(cum_improvement) > 0:
                peak = np.maximum.accumulate(cum_improvement)
                dd = cum_improvement - peak
            ...
            return EnhancedModelResult(
                ...
                max_drawdown_error_reduction=max_dd,
            )
    ...
    def _evaluate_single_model_with_cache(
        self,
        model_name: str,
        config: Dict,
        X_train: np.ndarray,
        ...
        try:
            ...
            # キャッシュからモデルを取得
            cached_model = cache_manager.get_cached_model(
                model_name, X_train, y_train, params, feature_names
            )
            ...
            if cached_model:
                ...
                cache_manager.save_model(
                    model, model_name, X_train, y_train, params, feature_names
                )
...
class EnhancedModelComparator:
    ...
    def __init__(
        self, max_workers: int = None, use_cache: bool = True, use_parallel: bool = True
    ):
        self.max_workers = max_workers or min(4, mp.cpu_count())
    ...
    def compare_models_enhanced(
        ...
        """強化されたモデル比較"""
        self.logger.info(
            f"🚀 強化されたモデル比較開始 (並列: {self.use_parallel}, キャッシュ: {self.use_cache})"
        )

        start_time = time.time()

        # 基準線（Baseline: 平均予測）を事前計算
        baseline = self._compute_baseline_metrics(X_test, y_train, y_test)
            ...
            [
                {
                    ...
                    "max_drawdown_error_reduction": r.max_drawdown_error_reduction,
                }
                for r in results
            ]
        )
    ...
    def _log_comparison_summary(self, df_results: pd.DataFrame):
        ...
        # 基準線（行）
        baseline_rows = df_results[df_results["model_type"] == "baseline"]
        baseline_info = None
        if not baseline_rows.empty:
            baseline_info = baseline_rows.iloc[0]
        ...
        parallel_processed = (
            df_results["parallel_processed"].sum()
            if "parallel_processed" in df_results.columns
            else 0
        )

        self.logger.info("📊 モデル比較結果サマリー:")
        self.logger.info(
            f"  🏆 最良モデル: {best_model['model_name']} (MAE: {best_model['mae']:.4f})"
        )
        if baseline_info is not None:
            self.logger.info(
                f"  └ 基準線: {baseline_info['model_name']} (MAE: {baseline_info['mae']:.4f}, R2: {baseline_info['r2']:.4f})"
            )
        ...
        if "win_rate_vs_baseline" in df_results.columns:
            avg_win_rate = (
                df_results["win_rate_vs_baseline"].dropna().mean()
                if not df_results["win_rate_vs_baseline"].dropna().empty
                else np.nan
            )
            self.logger.info(
        ...
        if "max_drawdown_error_reduction" in df_results.columns:
            avg_mdd = (
                df_results["max_drawdown_error_reduction"].dropna().mean()
                if not df_results["max_drawdown_error_reduction"].dropna().empty
                else np.nan
            )
...
def create_enhanced_model_comparator(
    max_workers: int = None, use_cache: bool = True, use_parallel: bool = True
) -> EnhancedModelComparator:
    """強化されたモデル比較システムを作成"""
...
if __name__ == "__main__":
    ...
    print("📊 モデル比較結果:")
    print(
        results[["model_name", "mae", "r2", "training_time", "cache_hit"]].to_string()
    )
