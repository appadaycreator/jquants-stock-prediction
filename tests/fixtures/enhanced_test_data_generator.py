...
class EnhancedTestDataGenerator:
    ...
    def generate_corrupted_data(
        self, base_data: pd.DataFrame, corruption_types: List[str] = None
    ) -> pd.DataFrame:
        """破損データを生成"""
        ...
        for corruption_type in corruption_types:
            if corruption_type == "missing_values":
                ...
                corrupted_data = pd.concat(
                    [corrupted_data, duplicated_rows], ignore_index=True
                )
    ...
    def generate_time_series_data(
        self, length: int = 100, frequency: str = "D", start_date: str = "2023-01-01"
    ) -> pd.DataFrame:
        """時系列データを生成"""
    ...
    def generate_anomaly_data(self, base_data: pd.DataFrame) -> pd.DataFrame:
        ...
        spike_indices = np.random.choice(
            anomaly_data.index, size=int(len(anomaly_data) * 0.01), replace=False
        )
