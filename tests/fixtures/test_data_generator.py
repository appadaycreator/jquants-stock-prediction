...
class DataGenerator:
    ...
    def generate_stock_data_with_patterns(
        self, days: int = 100, patterns: List[str] = None
    ) -> pd.DataFrame:
        """
        パターンを含む株価データを生成
    ...
    def generate_missing_data(
        self, base_data: pd.DataFrame, missing_ratio: float = 0.1
    ) -> pd.DataFrame:
        """
        欠損値を含むデータを生成
        ...
        for col in ["Open", "High", "Low", "Close", "Volume"]:
            if col in data.columns:
                missing_indices = np.random.choice(
                    data.index, size=int(len(data) * missing_ratio), replace=False
                )
    ...
    def generate_outlier_data(
        self, base_data: pd.DataFrame, outlier_ratio: float = 0.05
    ) -> pd.DataFrame:
        """
        外れ値を含むデータを生成
        ...
        for col in ["Open", "High", "Low", "Close"]:
            if col in data.columns:
                outlier_indices = np.random.choice(
                    data.index, size=int(len(data) * outlier_ratio), replace=False
                )
    ...
    def generate_multiple_stocks(
        self, stock_count: int = 3, days: int = 100
