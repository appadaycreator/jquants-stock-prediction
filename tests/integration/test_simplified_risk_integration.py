...
class TestSimplifiedRiskIntegration(unittest.TestCase):
    ...
    def test_api_integration(self):
        ...
        # 4. 視覚的リスク表示API
        visual_display = self.risk_api.get_visual_risk_display(self.sample_portfolio_data)
    ...
    def test_data_consistency(self):
        ...
        for i in range(1, len(results)):
            ...
            self.assertAlmostEqual(results[0].risk_score, results[i].risk_score, places=1)
            self.assertAlmostEqual(results[0].max_loss_amount, results[i].max_loss_amount, places=1)
