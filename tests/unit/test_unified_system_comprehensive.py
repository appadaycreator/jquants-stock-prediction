...
try:
    ...
    class UnifiedSystem:
        def __init__(self, module_name="UnifiedSystem", config_file="config_unified.yaml"):
            self.module_name = module_name
            self.config_file = config_file
            self.config = {}
            self.error_count = 0
            self.error_stats = {category: 0 for category in ErrorCategory.__dict__.values() if isinstance(category, str)}
            self.logger = MagicMock()

        def log_error(self, error, context="", category=ErrorCategory.API_ERROR, additional_info=None, include_traceback=True, level=LogLevel.ERROR):
            self.error_count += 1
...
class TestUnifiedSystemComprehensive:
    ...
    def test_handle_file_error(self):
        ...
        with pytest.raises(FileError):
            system.handle_file_error(Exception("File not found"), "File operation")
    ...
    def test_handle_network_error(self):
        ...
        with pytest.raises(NetworkError):
            system.handle_network_error(Exception("Connection timeout"), "Network operation")
    ...
    def test_handle_authentication_error(self):
        ...
        with pytest.raises(AuthenticationError):
            system.handle_authentication_error(Exception("Invalid credentials"), "Authentication")
    ...
    def test_validate_data_empty(self):
        ...
        result = system.validate_data(empty_data)
    ...
    def test_validate_config_invalid_keys(self):
        ...
        assert result["is_valid"] is False
        assert any("Invalid key" in issue for issue in result["issues"])
    ...
    def test_error_statistics_aggregation(self):
        ...
        # 異なるカテゴリのエラーを発生
        system.log_error(ValueError("API error"), "API context", ErrorCategory.API_ERROR)
        system.log_error(ValueError("Model error"), "Model context", ErrorCategory.MODEL_ERROR)
        system.log_error(ValueError("File error"), "File context", ErrorCategory.FILE_ERROR)
