...
class TestAutomatedSchedulerExtended:
    ...
    def test_scheduler_loop_exception_handling(
        ...
        with patch(
            "automated_scheduler.schedule.run_pending",
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_execute_routine_weekend_enabled(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """週末実行有効時のテスト"""
        # モック設定
        mock_config_instance = Mock()
        mock_config_instance.get_config.return_value = {
            "automated_scheduler": {"enable_weekend": True},
            "notifications": {},
        }
        mock_config.return_value = mock_config_instance
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_execute_routine_not_running(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """スケジューラー停止時のテスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_execute_routine_stop_event_set(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """停止イベント設定時のテスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_run_routine_with_retry_success(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """リトライ機能成功テスト"""
        # モック設定
        ...
        with patch.object(
            scheduler,
            "_call_routine_api",
            return_value={"success": True, "result": "test"},
        ):
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_run_routine_with_retry_all_failures(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """リトライ機能全失敗テスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_run_routine_with_retry_exception(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """リトライ機能例外テスト"""
        # モック設定
        ...
        # 例外が発生する場合
        with patch.object(
            scheduler, "_call_routine_api", side_effect=Exception("Test exception")
        ):
            result = scheduler._run_routine_with_retry()
            assert result == False
    ...
    def test_wait_for_job_completion_success(
        self, mock_get, mock_error_handler, mock_logging, mock_config
    ):
        """ジョブ完了待機成功テスト"""
        # モック設定
        mock_config_instance = Mock()
        mock_config_instance.get_config.return_value = {
            "automated_scheduler": {"timeout": 60},
            "notifications": {},
        }
        mock_config.return_value = mock_config_instance
    ...
    def test_wait_for_job_completion_failed(
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": "failed", "error": "Job failed"}
        mock_get.return_value = mock_response
    ...
    def test_wait_for_job_completion_timeout(
        self, mock_sleep, mock_get, mock_error_handler, mock_logging, mock_config
    ):
        """ジョブ完了待機タイムアウトテスト"""
        # モック設定
        mock_config_instance = Mock()
        mock_config_instance.get_config.return_value = {
            "automated_scheduler": {"timeout": 1},  # 短いタイムアウト
            "notifications": {},
        }
        mock_config.return_value = mock_config_instance
    ...
    @patch("automated_scheduler.requests.get")
    @patch("automated_scheduler.time.sleep")
    def test_wait_for_job_completion_http_error(
        self, mock_sleep, mock_get, mock_error_handler, mock_logging, mock_config
    ):
        """ジョブ完了待機HTTPエラーテスト"""
        # モック設定
    ...
    def test_send_notifications_all_enabled(
        ...
        with patch.object(scheduler, "_send_email_notification") as mock_email:
            with patch.object(scheduler, "_send_slack_notification") as mock_slack:
                with patch.object(
                    scheduler, "_send_browser_notification"
                ) as mock_browser:
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_send_notifications_all_disabled(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """全通知無効時のテスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_send_notifications_exception(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """通知送信例外テスト"""
        # モック設定
        ...
        # 通知送信で例外が発生する場合
        with patch.object(
            scheduler, "_send_email_notification", side_effect=Exception("Email error")
        ):
            # 例外が発生しても処理は継続される
            scheduler._send_notifications(True)
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_notification_content_with_no_last_execution(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """前回実行なし時の通知内容テスト"""
        # モック設定
    ...
    @patch("automated_scheduler.time.sleep")
    def test_email_notification_retry_success(
        self, mock_sleep, mock_smtp, mock_error_handler, mock_logging, mock_config
    ):
        """メール通知リトライ成功テスト"""
        # モック設定
        mock_config_instance = Mock()
    ...
    @patch("automated_scheduler.smtplib.SMTP")
    @patch("automated_scheduler.time.sleep")
    def test_email_notification_retry_all_failures(
        self, mock_sleep, mock_smtp, mock_error_handler, mock_logging, mock_config
    ):
        """メール通知リトライ全失敗テスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_email_notification_incomplete_config(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """メール通知設定不完全テスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_slack_notification_no_webhook(
        self, mock_error_handler, mock_logging, mock_config
    ):
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_browser_notification_file_error(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """ブラウザ通知ファイルエラーテスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_get_status_with_none_last_execution(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """前回実行なし時のステータス取得テスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_manual_execute_not_running(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """スケジューラー停止時の手動実行テスト"""
        # モック設定
    ...
    @patch("automated_scheduler.LoggingManager")
    @patch("automated_scheduler.ErrorHandler")
    def test_notification_config_loading_exception(
        self, mock_error_handler, mock_logging, mock_config
    ):
        """通知設定読み込み例外テスト"""
        # モック設定（例外発生）
