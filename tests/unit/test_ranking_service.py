...
class TestRankingService(unittest.TestCase):
    ...
    def test_config_validation(self):
        ...
        invalid_weights_config = {
            **valid_config,
            "weights": {"trend": 0.5, "valuation": 0.3, "quality": 0.3},
        }
    ...
    def _calculate_rule_based_score(self, stock, config, percentiles):
        ...
        if config["rules"]["trend"]["enabled"]:
            trend_reasons = []
            if (
                config["rules"]["trend"]["weeks13vs26"]
                and stock["weeks13"] > stock["weeks26"]
            ):
                trend_score += 0.5
                trend_reasons.append("13週移動平均＞26週移動平均")
    ...
    def _validate_config(self, config):
        ...
        # 分位値の範囲チェック
        if (
            config["rules"]["liquidity"]["percentile"] < 50
            or config["rules"]["liquidity"]["percentile"] > 90
        ):
            return False
    ...
    def _generate_candidates(self, stocks, config):
        ...
        # 除外条件を適用
        filtered_stocks = [
            stock for stock in stocks if self._apply_exclusion_rules(stock, config)
        ]

        # 分位値計算
        percentiles = {
            "volume": self._calculate_percentile(
                [s["volume"] for s in filtered_stocks], 70
            ),
            "return20": self._calculate_percentile(
                [s["return20"] for s in filtered_stocks], 40
            ),
