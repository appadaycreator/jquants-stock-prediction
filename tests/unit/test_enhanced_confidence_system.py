...
class TestEnhancedConfidenceSystem(unittest.TestCase):
    ...
    def test_trading_signal_generation(self):
        ...
        risk_metrics = {"volatility": 0.2, "max_risk": 0.02}
    ...
    def test_market_regime_detection(self):
        ...
        bull_market_data = pd.DataFrame(
            {"Date": dates, "Close": prices * 1.2}  # 20%上昇
        ).set_index("Date")
        ...
        bear_market_data = pd.DataFrame(
            {"Date": dates, "Close": prices * 0.8}  # 20%下落
        ).set_index("Date")
    ...
    def test_fundamental_confidence_calculation(self):
        ...
        fundamental_confidence = self.system._calculate_fundamental_confidence(
            good_fundamental
        )
