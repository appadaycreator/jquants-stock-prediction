...
from routine_api import JobStatus, JobRecord, JobManager, PipelineExecutor, app
...
class TestJobManager(unittest.TestCase):
    ...
    def test_update_job(self):
        ...
        # ステータスの更新
        result = self.job_manager.update_job(
            job.id, status=JobStatus.RUNNING, progress=50
        )
...
class TestFlaskEndpoints(unittest.TestCase):
    ...
    def test_run_today_endpoint(self):
        ...
        response = self.client.post(
            "/routine/run-today", json={"client_token": "test_token"}
        )
    ...
    def test_run_today_endpoint_duplicate_token(self):
        ...
        response1 = self.client.post(
            "/routine/run-today", json={"client_token": "duplicate_token"}
        )
