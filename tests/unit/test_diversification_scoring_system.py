...
from core.diversification_scoring_system import DiversificationScoringSystem, DiversificationMetrics, SectorAnalysis, CorrelationAnalysis
...
class TestDiversificationScoringSystem:
    ...
    def test_preprocess_portfolio_data(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """ポートフォリオデータ前処理テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
    ...
    def test_analyze_sector_diversification(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """セクター分散分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
    ...
    def test_analyze_correlation_diversification(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """相関分散分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        correlation_analysis = scoring_system._analyze_correlation_diversification(processed_data)
    ...
    def test_calculate_correlation_entropy(self, scoring_system):
        """相関エントロピー計算テスト"""
        correlation_matrix = np.array([[1.0, 0.5, 0.3], [0.5, 1.0, 0.4], [0.3, 0.4, 1.0]])
    ...
    def test_perform_correlation_clustering(self, scoring_system):
        """相関クラスタリングテスト"""
        correlation_matrix = np.array([[1.0, 0.8, 0.2], [0.8, 1.0, 0.3], [0.2, 0.3, 1.0]])
    ...
    def test_analyze_concentration_risk(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """集中度リスク分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
    ...
    def test_analyze_geographic_diversification(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """地理的分散分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        geographic_diversification = scoring_system._analyze_geographic_diversification(processed_data)
    ...
    def test_analyze_market_cap_diversification(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """時価総額分散分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        market_cap_diversification = scoring_system._analyze_market_cap_diversification(processed_data)
    ...
    def test_analyze_liquidity_diversification(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """流動性分散分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        liquidity_diversification = scoring_system._analyze_liquidity_diversification(processed_data)
    ...
    def test_analyze_risk_contribution_diversification(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """リスク寄与分散分析テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        correlation_analysis = scoring_system._analyze_correlation_diversification(processed_data)
        risk_contribution_diversification = scoring_system._analyze_risk_contribution_diversification(
    ...
    def test_calculate_entropy_score(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """エントロピースコア計算テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
    ...
    def test_calculate_herfindahl_index(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """ハーフィンダール指数計算テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
    ...
    def test_calculate_effective_number_of_stocks(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """実効銘柄数計算テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        effective_number = scoring_system._calculate_effective_number_of_stocks(processed_data)
        
        assert isinstance(effective_number, float)
        assert effective_number >= 1.0
        assert np.isfinite(effective_number)
    
    def test_calculate_diversification_ratio(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """分散投資比率計算テスト"""
        processed_data = scoring_system._preprocess_portfolio_data(sample_portfolio_weights, sample_stock_data)
        correlation_analysis = scoring_system._analyze_correlation_diversification(processed_data)
        diversification_ratio = scoring_system._calculate_diversification_ratio(processed_data, correlation_analysis)
    ...
    def test_calculate_diversification_score_integration(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """分散投資スコア計算統合テスト"""
    ...
    def test_generate_diversification_recommendations(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """分散投資推奨事項生成テスト"""
    ...
    def test_error_handling(self, scoring_system):
        ...
        result = scoring_system.calculate_diversification_score(empty_weights, empty_stock_data, None)
    ...
    def test_edge_cases(self, scoring_system):
        ...
        result = scoring_system.calculate_diversification_score(single_stock_weights, single_stock_data, None)
        assert isinstance(result, DiversificationMetrics)
        assert 0.0 <= result.overall_score <= 1.0
    
    def test_performance_metrics(self, scoring_system, sample_portfolio_weights, sample_stock_data):
        """パフォーマンスメトリクステスト"""
        result = scoring_system.calculate_diversification_score(
            sample_portfolio_weights, sample_stock_data, None
        )
