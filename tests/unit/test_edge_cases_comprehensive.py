...
class TestEdgeCasesComprehensive:
    ...
    def extreme_data_scenarios(self):
        ...
        scenarios['duplicate_data'] = pd.concat([base_data, base_data], ignore_index=True)
    ...
    def test_unicode_and_special_characters(self):
        ...
        unicode_data = pd.DataFrame({
            ...
            'SpecialChars': ['αβγδε', '①②③④⑤', '🚀📈💰'] * 33 + ['normal']  # 特殊文字
        })
    ...
    def test_boundary_conditions(self):
        ...
        boundary_data = pd.DataFrame({
            ...
            'Close': [0.0, 1e-10, 1e10, np.finfo(np.float64).max, np.finfo(np.float64).min] + [100.0] * 5,
            'Volume': [0, 1, 1e15, np.iinfo(np.int64).max, np.iinfo(np.int64).min] + [1000] * 5
        })
    ...
    def test_extreme_configuration_scenarios(self):
        ...
        extreme_configs = [
            ...
            {"api_key": "x" * 10000, "base_url": "http://" + "x" * 1000, "timeout": -1},  # 極端な値
        ]
