...
from scripts.test_jquants_token import (
    TestResult, EndpointConfig, TestResultData, JQuantsTokenTester, test_jquants_token
)
...
class TestJQuantsTokenTester(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.tester.headers["Authorization"], f"Bearer {self.test_token}")
        self.assertEqual(self.tester.headers["Content-Type"], "application/json")
        self.assertEqual(self.tester.headers["User-Agent"], "jQuants-Stock-Prediction/1.0")
    ...
    def test_print_summary(self):
        """サマリー表示のテスト"""
        results = [
            TestResultData("成功テスト", "https://api.example.com/success", 200, True, 100, {}),
            TestResultData("失敗テスト", "https://api.example.com/failure", 404, False, 0, {}, error_message="Not Found")
        ]
...
class TestTestJquantsTokenFunction(unittest.TestCase):
    ...
    def test_test_jquants_token_success(self, mock_tester_class):
        ...
        # モックテスターの設定
        mock_tester = Mock()
        mock_result = TestResultData("テスト", "https://api.example.com/test", 200, True, 100, {})
    ...
    def test_test_jquants_token_failure(self, mock_tester_class):
        ...
        # モックテスターの設定
        mock_tester = Mock()
        mock_result = TestResultData("テスト", "https://api.example.com/test", 404, False, 0, {})
        mock_tester.test_all_endpoints.return_value = [mock_result]
        mock_tester_class.return_value = mock_tester
