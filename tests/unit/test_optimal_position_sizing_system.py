...
from core.optimal_position_sizing_system import OptimalPositionSizingSystem, PositionSizingResult, RiskConstraints, MarketConditions
...
class TestOptimalPositionSizingSystem:
    ...
    def test_analyze_market_conditions(self, sizing_system, sample_market_conditions):
        """市場条件分析テスト"""
        market_adjustment = sizing_system._analyze_market_conditions(sample_market_conditions)
    ...
    def test_calculate_risk_parity_position_size(self, sizing_system, sample_stock_data):
    ...
    def test_calculate_volatility_adjusted_position_size(self, sizing_system, sample_stock_data, sample_market_conditions):
        """ボラティリティ調整ポジションサイズ計算テスト"""
        processed_data = sizing_system._preprocess_stock_data(sample_stock_data)
        risk_metrics = sizing_system._calculate_risk_metrics(processed_data, 150.0)
        market_adjustment = sizing_system._analyze_market_conditions(sample_market_conditions)
        
        volatility_adjusted_result = sizing_system._calculate_volatility_adjusted_position_size(
        ...
        assert isinstance(volatility_adjusted_result['volatility_adjusted_weight'], float)
    ...
    def test_optimize_position_size(self, sizing_system, sample_stock_data, sample_market_conditions, sample_existing_portfolio):
        """最適化ポジションサイズ計算テスト"""
        processed_data = sizing_system._preprocess_stock_data(sample_stock_data)
        risk_metrics = sizing_system._calculate_risk_metrics(processed_data, 150.0)
        market_adjustment = sizing_system._analyze_market_conditions(sample_market_conditions)
        ...
        optimized_result = sizing_system._optimize_position_size(
            processed_data, 150.0, 1000000, risk_metrics, market_adjustment,
            sample_existing_portfolio, 0.1, 'MEDIUM'
        )
    ...
    def test_integrate_and_validate_results(self, sizing_system, sample_stock_data, sample_market_conditions, sample_existing_portfolio):
        """結果統合・検証テスト"""
        processed_data = sizing_system._preprocess_stock_data(sample_stock_data)
        risk_metrics = sizing_system._calculate_risk_metrics(processed_data, 150.0)
        market_adjustment = sizing_system._analyze_market_conditions(sample_market_conditions)
        ...
        volatility_adjusted_result = {'volatility_adjusted_weight': 0.12, 'position_value': 120000}
        optimized_result = {'optimized_weight': 0.13, 'position_value': 130000, 'optimization_success': True}
        
        result = sizing_system._integrate_and_validate_results(
            'AAPL', 150.0, 1000000,
            kelly_result, risk_parity_result, volatility_adjusted_result, optimized_result,
            risk_metrics, market_adjustment, sample_existing_portfolio
        )
    ...
    def test_calculate_confidence_score(self, sizing_system):
        ...
        market_adjustment = {'volatility_adjustment': 1.0, 'trend_adjustment': 1.05, 'liquidity_adjustment': 1.1, 'stress_adjustment': 0.9, 'correlation_adjustment': 0.95}
    ...
    def test_check_liquidity_constraint(self, sizing_system):
        """流動性制約チェックテスト"""
        processed_data = {
            'liquidity_scores': [1000000, 2000000, 1500000]
        }
        ...
        # 不十分な流動性
        processed_data_low = {
            'liquidity_scores': [100000, 200000, 150000]
        }
        assert sizing_system._check_liquidity_constraint(processed_data_low, 1000000) == False
    ...
    def test_check_correlation_constraint(self, sizing_system, sample_existing_portfolio):
        """相関制約チェックテスト"""
        assert sizing_system._check_correlation_constraint(sample_existing_portfolio, 'AAPL') == True
    ...
    def test_calculate_optimal_position_size_integration(self, sizing_system, sample_stock_data, sample_market_conditions, sample_existing_portfolio):
        """最適ポジションサイズ計算統合テスト"""
        result = sizing_system.calculate_optimal_position_size(
            'AAPL', 150.0, 1000000, sample_stock_data, sample_market_conditions,
            sample_existing_portfolio, 0.1, 'MEDIUM'
        )
    ...
    def test_generate_position_sizing_recommendations(self, sizing_system, sample_market_conditions):
        """ポジションサイジング推奨事項生成テスト"""
    ...
    def test_performance_metrics(self, sizing_system, sample_stock_data, sample_market_conditions):
        """パフォーマンスメトリクステスト"""
        result = sizing_system.calculate_optimal_position_size(
