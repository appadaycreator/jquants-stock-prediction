...
from core.differential_updater import DifferentialUpdater, DiffResult, ValidationResult, UpdateStats
...
class TestDifferentialUpdaterCoverageImprovement:
    ...
    def test_calculate_comprehensive_diff(self):
        ...
        with patch.object(self.updater, '_calculate_data_hash') as mock_hash, \
            ...
            mock_counts.return_value = {"added": 1, "updated": 1, "removed": 0, "unchanged": 1}
    ...
    def test_validate_data_integrity_high_low_consistency(self):
        ...
        new_data = [
            {"Open": 100, "High": 50, "Low": 200, "Close": 150, "Volume": 1000}  # High < Low
        ]
        existing_data = []
    ...
    def test_validate_data_integrity_price_consistency(self):
        """データ整合性検証 - 価格の整合性テスト"""
        new_data = [
            {"Open": 100, "High": 50, "Low": 200, "Close": 150, "Volume": 1000}  # High < Open/Close
        ]
        existing_data = []
        ...
        assert not result.is_valid
        assert any("High価格がOpen/Close価格より低い" in issue for issue in result.issues)

    def test_validate_data_integrity_low_price_consistency(self):
        """データ整合性検証 - 安値の整合性テスト"""
        new_data = [
            {"Open": 100, "High": 200, "Low": 150, "Close": 50, "Volume": 1000}  # Low > Open/Close
        ]
        existing_data = []
        ...
        assert not result.is_valid
        assert any("Low価格がOpen/Close価格より高い" in issue for issue in result.issues)
    ...
    def test_validate_data_integrity_extreme_price_change(self):
        ...
        new_data = [
            {"Open": 100, "High": 200, "Low": 50, "Close": 200, "Volume": 1000}  # 100%変動
        ]
    ...
    def test_validate_data_integrity_date_validation(self):
        """データ整合性検証 - 日付検証テスト"""
        new_data = [
            {"Date": "invalid-date", "Open": 100, "High": 200, "Low": 50, "Close": 150, "Volume": 1000}
        ]
        existing_data = []
    ...
    def test_has_record_changed_exception(self):
        ...
        with patch.object(self.updater, '_calculate_hash', side_effect=Exception("Hash error")):
            result = self.updater._has_record_changed(old_record, new_record)
    ...
    def test_backup_data_failure(self):
        ...
        with patch.object(self.updater, '_create_backup_dir', side_effect=Exception("Backup error")):
            result = self.updater._backup_data(symbol, data)
