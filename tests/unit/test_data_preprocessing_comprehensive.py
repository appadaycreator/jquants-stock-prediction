...
try:
    ...
    def feature_selection_and_validation(data):
        available_features = [col for col in data.columns if col not in ["Date", "Code", "CompanyName"]]
...
class TestDataPreprocessingComprehensive:
    ...
    def corrupted_data(self):
        ...
        data = pd.DataFrame({
            ...
            'Volume': [np.nan] * 10 + list(np.random.randint(1000000, 10000000, 40))
        })
        return data
    ...
    def test_engineer_advanced_features_error_handling(self, sample_data):
        ...
        with patch('jquants_data_preprocessing.TechnicalIndicators') as mock_calculator:
            mock_instance = MagicMock()
            mock_calculator.return_value = mock_instance
            mock_instance.calculate_all_indicators.side_effect = Exception("Technical indicator error")
    ...
    def test_feature_selection_and_validation_success(self, sample_data):
        """特徴量選択と検証の成功ケース"""
        with patch('jquants_data_preprocessing.get_enhanced_features_list') as mock_features:
            mock_features.return_value = ['SMA_5', 'SMA_25', 'RSI', 'MACD']
            
            with patch('jquants_data_preprocessing.TypeSafeValidator') as mock_validator:
                mock_instance = MagicMock()
                mock_validator.return_value = mock_instance
                mock_instance.validate_numeric_columns.return_value = {'is_valid': True, 'errors': []}
                mock_instance.safe_nan_handling.return_value = sample_data
                
                result_df, available_features = feature_selection_and_validation(sample_data)
    ...
    def test_feature_selection_and_validation_infinity_handling(self, sample_data):
        ...
        with patch('jquants_data_preprocessing.get_enhanced_features_list') as mock_features:
            ...
            with patch('jquants_data_preprocessing.TypeSafeValidator') as mock_validator:
                ...
                result_df, available_features = feature_selection_and_validation(sample_data_with_inf)
    ...
    def test_validate_processed_data_success(self, sample_data):
        ...
        with patch('jquants_data_preprocessing.DataValidator') as mock_validator:
            mock_instance = MagicMock()
            mock_validator.return_value = mock_instance
            mock_instance.validate_stock_data.return_value = {'is_valid': True, 'quality_score': 0.95}
    ...
    def test_validate_processed_data_failure(self, sample_data):
        ...
        with patch('jquants_data_preprocessing.DataValidator') as mock_validator:
            mock_instance = MagicMock()
            mock_validator.return_value = mock_instance
            mock_instance.validate_stock_data.return_value = {'is_valid': False, 'quality_score': 0.3}
            mock_instance.generate_validation_report.return_value = "Validation report"
    ...
    def test_validate_processed_data_exception(self, sample_data):
        ...
        with patch('jquants_data_preprocessing.DataValidator') as mock_validator:
            mock_instance = MagicMock()
            mock_validator.return_value = mock_instance
            mock_instance.validate_stock_data.side_effect = Exception("Validation error")
    ...
    def test_main_success(self):
        ...
        with patch('jquants_data_preprocessing.load_and_clean_data') as mock_load:
            with patch('jquants_data_preprocessing.engineer_basic_features') as mock_basic:
                with patch('jquants_data_preprocessing.engineer_advanced_features') as mock_advanced:
                    with patch('jquants_data_preprocessing.feature_selection_and_validation') as mock_feature:
                        with patch('jquants_data_preprocessing.validate_processed_data') as mock_validate:
                            with patch('pandas.DataFrame.to_csv') as mock_save:
                                with patch('builtins.open', mock_open()) as mock_file:
                                    # モックの設定
                                    sample_data = pd.DataFrame({'Date': ['2023-01-01'], 'Close': [100]})
    ...
    def test_duplicate_removal(self, sample_data):
        """重複行削除のテスト"""
        # 重複データを作成
        duplicated_data = pd.concat([sample_data, sample_data.iloc[:10]], ignore_index=True)
    ...
    def test_encoding_detection(self, sample_data):
        """エンコーディング検出のテスト"""
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.csv', encoding='shift_jis') as f:
