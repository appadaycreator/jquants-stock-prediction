...
class TestCacheService(unittest.TestCase):
    ...
    def test_cache_expiration_logic(self):
        ...
        is_valid = (now.timestamp() * 1000) < (
            valid_timestamp.timestamp() * 1000 + valid_ttl
        )
        ...
        is_expired = (now.timestamp() * 1000) >= (
            expired_timestamp.timestamp() * 1000 + expired_ttl
        )
    ...
    def test_cache_metrics_calculation(self):
        """キャッシュメトリクス計算のテスト"""
        metrics = {"hits": 10, "misses": 5, "total": 15}
