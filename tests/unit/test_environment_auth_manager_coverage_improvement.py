...
class TestEnvironmentAuthManagerCoverageImprovement:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch.dict(os.environ, {"ENVIRONMENT": "development"}):
                with patch("os.path.exists", return_value=True):
                    with patch(
                        "dotenv.load_dotenv", side_effect=IOError("File read error")
                    ):
                        # 環境変数を手動で削除
                        for var in [
                            "JQUANTS_EMAIL",
    ...
    def test_get_auth_info_partial(self):
        ...
        with patch.dict(
            os.environ,
            {"JQUANTS_EMAIL": "test@example.com", "JQUANTS_PASSWORD": "password123"},
            clear=True,
        ):
            with patch("dotenv.load_dotenv"):
                manager = EnvironmentAuthManager()
    ...
    def test_is_authenticated_true(self):
        """認証状態確認（認証済み）テスト"""
        with patch.dict(os.environ, {"JQUANTS_ID_TOKEN": "valid_token"}, clear=True):
            with patch("dotenv.load_dotenv"):
                manager = EnvironmentAuthManager()
    ...
    def test_validate_auth_info_incomplete(self):
        """認証情報検証（不完全）テスト"""
        with patch.dict(os.environ, {"JQUANTS_EMAIL": "test@example.com"}, clear=True):
            with patch("dotenv.load_dotenv"):
                manager = EnvironmentAuthManager()
                result = manager.validate_auth_info()
    ...
    def test_auth_info_loading_mixed_sources(self):
        ...
        with patch.dict(
            ...
            with patch(
                "builtins.open", mock_open(read_data="JQUANTS_PASSWORD=file_password")
            ):
                with patch("os.path.exists", return_value=True):
                    # 環境変数を手動で設定
                    manager = EnvironmentAuthManager()
