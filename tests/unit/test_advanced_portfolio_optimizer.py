...
from core.advanced_portfolio_optimizer import AdvancedPortfolioOptimizer, OptimizationResult, RiskMetrics
...
class TestAdvancedPortfolioOptimizer:
    ...
    def test_preprocess_data(self, optimizer, sample_stock_data, sample_market_data):
        """データ前処理テスト"""
        processed_data = optimizer._preprocess_data(sample_stock_data, sample_market_data, None)
    ...
    def test_calculate_returns_and_covariance(self, optimizer, sample_stock_data):
        ...
        returns_matrix, cov_matrix = optimizer._calculate_returns_and_covariance(processed_data)
    ...
    def test_calculate_expected_returns(self, optimizer, sample_stock_data):
        """期待リターン計算テスト"""
        processed_data = optimizer._preprocess_data(sample_stock_data, None, None)
        expected_returns = optimizer._calculate_expected_returns(processed_data, np.array([[0.01, 0.02], [0.02, 0.03], [0.01, 0.02]]))
    ...
    def test_optimize_max_sharpe(self, optimizer, sample_stock_data):
        ...
        cov_matrix = np.array([[0.04, 0.02, 0.01], [0.02, 0.06, 0.02], [0.01, 0.02, 0.05]])
    ...
    def test_optimize_mean_variance(self, optimizer, sample_stock_data):
        ...
        result = optimizer._optimize_mean_variance(expected_returns, cov_matrix, 0.1, 0.15)
    ...
    def test_optimize_black_litterman(self, optimizer, sample_stock_data, sample_market_data):
        """ブラック・リッターマン最適化テスト"""
        processed_data = optimizer._preprocess_data(sample_stock_data, sample_market_data, None)
        expected_returns = np.array([0.1, 0.12, 0.08])
        cov_matrix = np.array([[0.04, 0.02, 0.01], [0.02, 0.06, 0.02], [0.01, 0.02, 0.05]])
        
        result = optimizer._optimize_black_litterman(expected_returns, cov_matrix, sample_market_data)
    ...
    def test_calculate_risk_metrics(self, optimizer):
        """リスクメトリクス計算テスト"""
        weights = {0: 0.4, 1: 0.3, 2: 0.3}
        returns_matrix = np.array([[0.01, 0.02, 0.01], [0.02, 0.03, 0.02], [0.01, 0.02, 0.01]])
        cov_matrix = np.array([[0.04, 0.02, 0.01], [0.02, 0.06, 0.02], [0.01, 0.02, 0.05]])
        
        risk_metrics = optimizer.calculate_risk_metrics(weights, returns_matrix, cov_matrix)
    ...
    def test_generate_recommendations(self, optimizer):
        ...
        recommendations = optimizer.generate_recommendations(optimization_result, risk_metrics)
    ...
    def test_optimize_portfolio_integration(self, optimizer, sample_stock_data, sample_market_data):
        """ポートフォリオ最適化統合テスト"""
