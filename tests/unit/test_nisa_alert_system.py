...
from core.nisa_alert_system import (
    NisaAlertSystem, QuotaAlert, InvestmentOpportunity, SystemAlert,
...
class TestNisaAlertSystem:
    ...
    def test_check_quota_alerts(self, alert_system, sample_quota_status):
        ...
        for alert in alerts:
            ...
            assert alert.type in [AlertType.WARNING.value, AlertType.CRITICAL.value, AlertType.INFO.value]
            assert alert.quota_type in ['GROWTH', 'ACCUMULATION']
            assert alert.current_usage >= 0
            assert alert.threshold >= 0
            assert alert.priority in [AlertPriority.HIGH.value, AlertPriority.MEDIUM.value, AlertPriority.LOW.value]
    ...
    def test_generate_investment_opportunities(self, alert_system, sample_market_data, sample_quota_status):
        """投資機会通知生成テスト"""
        opportunities = alert_system.generate_investment_opportunities(sample_market_data, sample_quota_status)
        ...
        for opportunity in opportunities:
            ...
            assert opportunity.priority in [AlertPriority.HIGH.value, AlertPriority.MEDIUM.value, AlertPriority.LOW.value]
    
    def test_generate_growth_opportunities(self, alert_system, sample_market_data):
        """成長投資枠投資機会生成テスト"""
        quota_status = {
            'growth_investment': {
                'available_amount': 500000.0  # 十分な枠がある
            }
        }
        
        opportunities = alert_system._generate_growth_opportunities(sample_market_data, quota_status)
    ...
    def test_generate_accumulation_opportunities(self, alert_system, sample_market_data):
        """つみたて投資枠投資機会生成テスト"""
        quota_status = {
            'accumulation_investment': {
                'available_amount': 100000.0  # 十分な枠がある
            }
        }
        
        opportunities = alert_system._generate_accumulation_opportunities(sample_market_data, quota_status)
    ...
    def test_send_notification_email(self, alert_system):
        ...
        with patch.object(alert_system, '_send_email_notification', return_value=True) as mock_send:
    ...
    def test_send_notification_browser(self, alert_system):
        ...
        with patch.object(alert_system, '_send_browser_notification', return_value=True) as mock_send:
    ...
    def test_send_notification_push(self, alert_system):
        ...
        with patch.object(alert_system, '_send_push_notification', return_value=True) as mock_send:
    ...
    def test_opportunity_generation_with_insufficient_quota(self, alert_system, sample_market_data):
        """枠不足時の投資機会生成テスト"""
        quota_status = {
            'growth_investment': {
                'available_amount': 50000.0  # 5万円（少ない）
            },
            'accumulation_investment': {
                'available_amount': 5000.0  # 5千円（少ない）
            }
        }
        
        opportunities = alert_system.generate_investment_opportunities(sample_market_data, quota_status)
    ...
    def test_error_handling(self, alert_system):
        ...
        opportunities = alert_system.generate_investment_opportunities(invalid_market_data, quota_status)
