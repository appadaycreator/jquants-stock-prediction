...
class TestTechnicalAnalysisEnhanced:
    ...
    def test_vwap_edge_cases(self):
        ...
        # 空のシリーズ
        empty_series = pd.Series([], dtype=float)
        result = TechnicalAnalysis.vwap(empty_series, empty_series, empty_series, empty_series)
        assert len(result) == 0
        ...
        single_close = pd.Series([99.5])
        single_volume = pd.Series([1000])
        result = TechnicalAnalysis.vwap(single_high, single_low, single_close, single_volume)
        assert len(result) == 1
