...
class TestDataPreprocessingCoverageEnhancement:
    ...
    def test_engineer_advanced_features_success(self, sample_stock_data):
        ...
        assert not result.empty
        # 高度な特徴量が追加されていることを確認
        advanced_features = [col for col in result.columns if any(x in col for x in ['MACD', 'BB', 'ATR', 'Stoch'])]
    ...
    def test_feature_selection_and_validation_success(self, sample_stock_data):
        ...
        result_df, selected_features = feature_selection_and_validation(df_with_features)
    ...
    def test_missing_values_handling(self, corrupted_data):
        ...
        try:
            ...
            assert isinstance(result, pd.DataFrame)
            # 欠損値が適切に処理されていることを確認
            assert result['Close'].isna().sum() == 0 or result['Close'].isna().sum() < corrupted_data['Close'].isna().sum()
        finally:
            os.unlink(temp_file)
    ...
    def test_encoding_detection(self):
        ...
        for encoding in encodings_to_test:
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.csv', encoding=encoding) as f:
                f.write("Date,Close\n2023-01-01,100\n")
                temp_file = f.name
    ...
    def test_edge_cases_comprehensive(self):
        ...
        for i, edge_data in enumerate(edge_cases):
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=f'_edge_{i}.csv') as f:
