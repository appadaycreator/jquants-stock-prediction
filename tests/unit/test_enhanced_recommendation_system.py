...
from core.enhanced_recommendation_system import (
    EnhancedRecommendationSystem, PortfolioRecommendation, Recommendation,
    RecommendationType, PriorityLevel, RiskLevel
)
...
class TestEnhancedRecommendationSystem:
    ...
    def test_assess_portfolio_overall(self, recommendation_system, sample_portfolio_data, sample_market_conditions, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics):
        """ポートフォリオ全体評価テスト"""
        overall_assessment = recommendation_system._assess_portfolio_overall(
            sample_portfolio_data, sample_market_conditions, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics
        )
        ...
        assert overall_assessment['grade'] in ['A+', 'A', 'B+', 'B', 'C+', 'C', 'D', 'F']
        assert overall_assessment['status'] in ['EXCELLENT', 'VERY_GOOD', 'GOOD', 'FAIR', 'NEEDS_IMPROVEMENT', 'POOR']
    ...
    def test_generate_individual_recommendations(self, recommendation_system, sample_portfolio_data, sample_market_conditions, sample_optimization_results, sample_risk_metrics):
        """個別推奨事項生成テスト"""
        recommendations = recommendation_system._generate_individual_recommendations(
            sample_portfolio_data, sample_market_conditions, sample_optimization_results, sample_risk_metrics
        )
        ...
        for recommendation in recommendations:
            ...
            assert recommendation.stop_loss is None or isinstance(recommendation.stop_loss, float)
            assert recommendation.take_profit is None or isinstance(recommendation.take_profit, float)
    ...
    def test_determine_recommendation_type(self, recommendation_system):
        ...
        assert recommendation_system._determine_recommendation_type(0.1, position_data) == RecommendationType.BUY_MORE
        
        # 売却ケース
        assert recommendation_system._determine_recommendation_type(-0.1, position_data) == RecommendationType.SELL
        
        # 利確ケース
        position_data_profit = {'unrealized_pnl': 15000, 'cost_basis': 100000}
        assert recommendation_system._determine_recommendation_type(0.0, position_data_profit) == RecommendationType.TAKE_PROFIT
        
        # 損切りケース
        position_data_loss = {'unrealized_pnl': -8000, 'cost_basis': 100000}
        assert recommendation_system._determine_recommendation_type(0.0, position_data_loss) == RecommendationType.STOP_LOSS
        
        # ホールドケース
        position_data_hold = {'unrealized_pnl': 2000, 'cost_basis': 100000}
        assert recommendation_system._determine_recommendation_type(0.0, position_data_hold) == RecommendationType.HOLD
    
    def test_calculate_recommendation_confidence(self, recommendation_system, sample_market_conditions, sample_risk_metrics):
        """推奨信頼度計算テスト"""
    ...
    def test_determine_priority(self, recommendation_system):
        """優先度決定テスト"""
        # 高優先度ケース
        assert recommendation_system._determine_priority(0.15, 0.8, {}) == PriorityLevel.HIGH
        
        # 中優先度ケース
        assert recommendation_system._determine_priority(0.08, 0.7, {}) == PriorityLevel.MEDIUM
        
        # 低優先度ケース
        assert recommendation_system._determine_priority(0.03, 0.5, {}) == PriorityLevel.LOW
    
    def test_calculate_expected_return(self, recommendation_system, sample_market_conditions):
        """期待リターン計算テスト"""
        position_data = {'expected_return': 0.08}
        
        expected_return = recommendation_system._calculate_expected_return(position_data, sample_market_conditions)
    ...
    def test_determine_risk_level(self, recommendation_system, sample_risk_metrics):
        ...
        assert recommendation_system._determine_risk_level(position_data_low, sample_risk_metrics) == RiskLevel.LOW
        assert recommendation_system._determine_risk_level(position_data_medium, sample_risk_metrics) == RiskLevel.MEDIUM
        assert recommendation_system._determine_risk_level(position_data_high, sample_risk_metrics) == RiskLevel.HIGH
        assert recommendation_system._determine_risk_level(position_data_very_high, sample_risk_metrics) == RiskLevel.VERY_HIGH
    
    def test_calculate_recommended_quantity(self, recommendation_system):
        """推奨数量計算テスト"""
        quantity = recommendation_system._calculate_recommended_quantity('AAPL', 150.0, 0.35, 1000000)
    ...
    def test_determine_timeframe(self, recommendation_system):
        """タイムフレーム決定テスト"""
        assert recommendation_system._determine_timeframe(RecommendationType.STOP_LOSS, PriorityLevel.HIGH) == '即座実行'
        assert recommendation_system._determine_timeframe(RecommendationType.TAKE_PROFIT, PriorityLevel.HIGH) == '即座実行'
        assert recommendation_system._determine_timeframe(RecommendationType.BUY_MORE, PriorityLevel.HIGH) == '今週中'
        assert recommendation_system._determine_timeframe(RecommendationType.BUY, PriorityLevel.MEDIUM) == '今月中'
        assert recommendation_system._determine_timeframe(RecommendationType.HOLD, PriorityLevel.LOW) == '来四半期'
    ...
    def test_generate_portfolio_actions(self, recommendation_system, sample_portfolio_data, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics):
        """ポートフォリオアクション生成テスト"""
        actions = recommendation_system._generate_portfolio_actions(
            sample_portfolio_data, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics
        )
    ...
    def test_generate_risk_warnings(self, recommendation_system, sample_portfolio_data, sample_risk_metrics, sample_market_conditions):
        """リスク警告生成テスト"""
    ...
    def test_generate_optimization_suggestions(self, recommendation_system, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics):
        """最適化提案生成テスト"""
        suggestions = recommendation_system._generate_optimization_suggestions(
            sample_optimization_results, sample_risk_metrics, sample_diversification_metrics
        )
    ...
    def test_generate_rebalancing_plan(self, recommendation_system, sample_portfolio_data, sample_optimization_results):
        """リバランス計画生成テスト"""
    ...
    def test_generate_diversification_advice(self, recommendation_system, sample_diversification_metrics, sample_portfolio_data):
        """分散投資アドバイス生成テスト"""
    ...
    def test_generate_portfolio_recommendations_integration(self, recommendation_system, sample_portfolio_data, sample_market_conditions, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics):
        """ポートフォリオ推奨事項生成統合テスト"""
        result = recommendation_system.generate_portfolio_recommendations(
            sample_portfolio_data, sample_market_conditions, sample_optimization_results,
            sample_risk_metrics, sample_diversification_metrics, None
        )
    ...
    def test_error_handling(self, recommendation_system):
        ...
        result = recommendation_system.generate_portfolio_recommendations(
            empty_portfolio_data, empty_market_conditions, empty_optimization_results,
            empty_risk_metrics, empty_diversification_metrics, None
        )
    ...
    def test_edge_cases(self, recommendation_system, sample_portfolio_data, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics):
        ...
        result = recommendation_system.generate_portfolio_recommendations(
            sample_portfolio_data, extreme_market_conditions, sample_optimization_results,
            sample_risk_metrics, sample_diversification_metrics, None
        )
    ...
    def test_performance_metrics(self, recommendation_system, sample_portfolio_data, sample_market_conditions, sample_optimization_results, sample_risk_metrics, sample_diversification_metrics):
        """パフォーマンスメトリクステスト"""
        result = recommendation_system.generate_portfolio_recommendations(
            sample_portfolio_data, sample_market_conditions, sample_optimization_results,
            sample_risk_metrics, sample_diversification_metrics, None
