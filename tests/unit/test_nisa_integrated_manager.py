...
class TestNisaIntegratedManager:
    ...
    def test_validate_transaction_data_success(
        self, integrated_manager, sample_transaction_data
    ):
        """取引データ検証成功テスト"""
    ...
    def test_generate_integrated_recommendations(self, integrated_manager):
        ...
        alert_summary = {"critical_alerts": 1, "warning_alerts": 2}
    ...
    def test_calculate_priority_score(self, integrated_manager):
        """優先度スコア計算テスト"""
        quota_optimization = {"risk_analysis": {"diversification_score": 80.0}}
    ...
    def test_calculate_portfolio_performance(self, integrated_manager):
        ...
        performance = integrated_manager._calculate_portfolio_performance(
            empty_portfolio
        )
    ...
    def test_auto_alerts_enabled(self, integrated_manager, sample_transaction_data):
        """自動アラート有効テスト"""
        with patch.object(
            integrated_manager.alert_system, "check_quota_alerts"
        ) as mock_check:
            with patch.object(
                integrated_manager.alert_system, "send_notification"
            ) as mock_send:
    ...
    def test_error_handling_in_dashboard(self, integrated_manager):
        ...
        with patch.object(
            integrated_manager.quota_manager,
            "get_quota_status",
