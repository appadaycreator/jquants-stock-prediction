...
class TestConfidenceBasedTrading:
    ...
    def test_edge_cases(self):
        ...
        # 信頼度が0の場合のテスト
        try:
            result = self.trading.should_trade(0.0, {'volume': 1000, 'price_change': 0.01, 'volatility': 0.02})
            assert 'should_trade' in result
        except Exception:
            # 信頼度0でエラーが発生しても問題ない
            pass
        ...
        # 信頼度が1の場合のテスト
        try:
            result = self.trading.should_trade(1.0, {'volume': 1000, 'price_change': 0.01, 'volatility': 0.02})
            assert 'should_trade' in result
        except Exception:
            # 信頼度1でエラーが発生しても問題ない
            pass
