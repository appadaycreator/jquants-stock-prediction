...
class TestEnvironmentAuthManager:
    ...
    def test_load_auth_info_development(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch(
                "builtins.open",
                mock_open(
                    read_data="JQUANTS_EMAIL=test@example.com\nJQUANTS_PASSWORD=password"
                ),
            ):
                manager = EnvironmentAuthManager()
    ...
    def test_is_authenticated_false(self):
        ...
        with patch.object(
            self.auth_manager,
            "auth_info",
            {"email": None, "password": None, "id_token": None, "refresh_token": None},
        ):
    ...
    def test_environment_specific_loading(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development"}, clear=True):
            with patch(
                "builtins.open", mock_open(read_data="JQUANTS_EMAIL=dev@example.com")
            ):
                manager = EnvironmentAuthManager()
                assert manager.environment == "development"
    ...
    def test_production_environment_detection(self):
        ...
        with patch.dict(
            os.environ,
            {"GITHUB_ACTIONS": "true", "JQUANTS_EMAIL": "prod@example.com"},
            clear=True,
        ):
            manager = EnvironmentAuthManager()
            assert manager.environment == "production"
    ...
    def test_staging_environment_detection(self):
        ...
        with patch.dict(
            os.environ,
            {"ENVIRONMENT": "staging", "JQUANTS_EMAIL": "staging@example.com"},
            clear=True,
        ):
            manager = EnvironmentAuthManager()
            assert manager.environment == "staging"
    ...
    def test_environment_detection_priority(self):
        ...
        with patch.dict(
            os.environ,
            {"GITHUB_ACTIONS": "true", "ENVIRONMENT": "development"},
            clear=True,
        ):
            manager = EnvironmentAuthManager()
            assert manager.environment == "development"
    ...
    def test_credentials_validation(self):
        ...
        # 無効な認証情報
        with patch.object(
            self.auth_manager, "auth_info", {"email": "", "password": ""}
        ):
            assert not self.auth_manager.is_auth_configured()
    ...
    def test_token_validation(self):
        ...
        with patch.object(
            self.auth_manager,
            "auth_info",
            {"id_token": "valid_token", "refresh_token": "valid_refresh"},
        ):
            assert self.auth_manager.is_auth_configured()

        # 無効なトークン
        with patch.object(
            self.auth_manager, "auth_info", {"id_token": "", "refresh_token": ""}
        ):
            assert not self.auth_manager.is_auth_configured()
