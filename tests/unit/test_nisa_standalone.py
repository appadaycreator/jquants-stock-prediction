...
nisa_quota_manager = load_module_from_file(
    os.path.join(core_dir, "nisa_quota_manager.py"), "nisa_quota_manager"
)

nisa_tax_calculator = load_module_from_file(
    os.path.join(core_dir, "nisa_tax_calculator.py"), "nisa_tax_calculator"
)

nisa_alert_system = load_module_from_file(
    os.path.join(core_dir, "nisa_alert_system.py"), "nisa_alert_system"
)
...
class TestNisaStandalone:
    ...
    def test_sell_transaction(self, quota_manager):
        ...
        assert (
            status.growth_investment["available_amount"] == 2280000.0
        )  # 2150000 + 130000
