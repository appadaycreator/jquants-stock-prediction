...
class TestConfigLoader:
    ...
    def test_init_with_sample_config(self):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".yaml.sample", delete=False
        ) as f:
            config_data = {
                "preprocessing": {
    ...
    def test_get_config_section_missing(self):
        """存在しない設定セクションの取得テスト"""
        config_data = {"preprocessing": {"input_file": "input.csv"}}
    ...
    def test_get_config_section_with_default(self):
        ...
        try:
            ...
            # デフォルト値付きで取得
            default_config = loader.get_config_section(
    ...
    def test_get_config_value(self):
        """設定値の取得テスト"""
        config_data = {
            "preprocessing": {"input_file": "input.csv", "nested": {"key": "value"}}
        }
    ...
    def test_config_validation(self):
        """設定の検証テスト"""
        config_data = {
            "preprocessing": {"input_file": "input.csv", "output_file": "output.csv"},
            "prediction": {"features": ["feature1", "feature2"], "target": "target"},
        }
