...
class TestEnhancedDataPreprocessing:
    ...
    def test_calculate_technical_indicators_success(self):
        ...
        # 技術指標が追加されていることを確認
        indicator_columns = [
            col for col in result.columns if col in ["RSI", "MACD", "SMA_5", "SMA_25"]
        ]
    ...
    def test_data_type_conversion(self):
        ...
        assert (
            pd.api.types.is_numeric_dtype(result["Open"])
            or result["Open"].dtype == "object"
        )
        assert (
            pd.api.types.is_numeric_dtype(result["Close"])
            or result["Close"].dtype == "object"
        )
