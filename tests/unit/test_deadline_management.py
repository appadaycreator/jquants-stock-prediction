...
class TestDeadlineManager:
    ...
    def test_get_deadlines_by_status(self):
        """ステータス別の期限取得テスト"""
        # 複数の期限を追加
        self.manager.add_deadline("action1", "TEST1", "buy", datetime.now() + timedelta(days=1), "this_week")
        self.manager.add_deadline("action2", "TEST2", "sell", datetime.now() + timedelta(days=2), "this_week")
        self.manager.add_deadline("action3", "TEST3", "hold", datetime.now() + timedelta(days=3), "this_week")
    ...
    def test_get_approaching_deadlines(self):
        ...
        # 異なる期限のアクションを追加
        now = datetime.now()
        self.manager.add_deadline("action1", "TEST1", "buy", now + timedelta(hours=2), "this_week")
        self.manager.add_deadline("action2", "TEST2", "sell", now + timedelta(hours=12), "this_week")
        self.manager.add_deadline("action3", "TEST3", "hold", now + timedelta(days=2), "this_week")
    ...
    def test_get_urgent_deadlines(self):
        """緊急期限の取得テスト"""
        now = datetime.now()
        self.manager.add_deadline("action1", "TEST1", "buy", now + timedelta(hours=1), "this_week")
        self.manager.add_deadline("action2", "TEST2", "sell", now + timedelta(hours=12), "this_week")
    ...
    def test_get_expired_deadlines(self):
        ...
        self.manager.add_deadline("action1", "TEST1", "buy", now - timedelta(hours=1), "this_week")
        # 未来の期限
        self.manager.add_deadline("action2", "TEST2", "sell", now + timedelta(hours=1), "this_week")
    ...
    def test_get_deadline_summary(self):
        ...
        # 複数の期限を追加
        now = datetime.now()
        self.manager.add_deadline("action1", "TEST1", "buy", now + timedelta(hours=1), "this_week", "high")
        self.manager.add_deadline("action2", "TEST2", "sell", now + timedelta(days=1), "this_week", "medium")
        self.manager.add_deadline("action3", "TEST3", "hold", now + timedelta(days=2), "this_week", "low")
    ...
    def test_get_deadline_timeline(self):
        """期限タイムラインの取得テスト"""
        now = datetime.now()
        self.manager.add_deadline("action1", "TEST1", "buy", now + timedelta(days=1), "this_week")
        self.manager.add_deadline("action2", "TEST2", "sell", now + timedelta(days=3), "this_week")
        self.manager.add_deadline("action3", "TEST3", "hold", now + timedelta(days=10), "this_month")
    ...
    def test_error_handling(self):
        ...
        # 存在しないアクションのステータス更新
        result = self.manager.update_deadline_status("non_existent", DeadlineStatus.COMPLETED)
        assert result is False
    ...
    def test_monitoring_thread_safety(self):
        ...
        for i in range(10):
            self.manager.add_deadline(f"action{i}", f"TEST{i}", "buy", 
                                    datetime.now() + timedelta(hours=i), "this_week")
