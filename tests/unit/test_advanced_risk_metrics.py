...
from core.advanced_risk_metrics import (
    AdvancedRiskMetrics, RiskMetricType, RiskMetricsResult
)
...
class TestAdvancedRiskMetrics:
    ...
    def test_calculate_comprehensive_risk_metrics_normal(self, risk_metrics, sample_stock_data, sample_market_data):
        """包括的リスクメトリクス計算（正常ケース）"""
    ...
    def test_get_risk_statistics_with_history(self, risk_metrics):
        ...
        risk_metrics.risk_history = [
            RiskMetricsResult(
                ...
                max_drawdown=0.10, sharpe_ratio=1.0, sortino_ratio=1.0,
                calmar_ratio=1.0, information_ratio=0.5, treynor_ratio=0.8,
                jensen_alpha=0.02, beta=1.0, correlation=0.5, volatility=0.20,
                skewness=0.1, kurtosis=3.0, risk_score=50.0, risk_level="MEDIUM",
                confidence_interval=(0.01, 0.05), calculation_date=datetime.now()
            ),
            RiskMetricsResult(
                var_95=0.03, var_99=0.06, cvar_95=0.04, cvar_99=0.07,
                max_drawdown=0.12, sharpe_ratio=0.8, sortino_ratio=0.8,
                calmar_ratio=0.8, information_ratio=0.3, treynor_ratio=0.6,
                jensen_alpha=0.01, beta=1.1, correlation=0.6, volatility=0.25,
                skewness=0.2, kurtosis=3.5, risk_score=60.0, risk_level="HIGH",
                confidence_interval=(0.02, 0.06), calculation_date=datetime.now()
            )
        ]
