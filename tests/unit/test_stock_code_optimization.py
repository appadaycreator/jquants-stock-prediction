...
class TestStockCodeValidation(unittest.TestCase):
    ...
    def test_valid_legacy_codes(self):
        ...
        for code in valid_codes:
            with self.subTest(code=code):
                result = normalize_security_code(code)
                self.assertTrue(len(result) == 4 and result.isdigit(),
    ...
    def test_valid_new_codes(self):
        ...
        for code in valid_codes:
            with self.subTest(code=code):
                result = normalize_security_code(code)
                self.assertTrue(len(result) == 5 and result[0].isalpha() and result[1:].isdigit(),
    ...
    def test_code_format_detection(self):
        ...
        for code in new_codes:
            with self.subTest(code=code):
                result = normalize_security_code(code)
                self.assertTrue(result[0].isalpha() and result[1:].isdigit() and len(result) == 5,
