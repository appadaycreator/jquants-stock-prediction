...
class ConfigLoader:
        ...
        try:
            ...
            if not os.path.exists(self.config_path):
                ...
                if os.path.exists(sample_path):
                    print(
                        f"⚠️ {self.config_path}が見つかりません。{sample_path}をコピーします..."
                    )
                    import shutil
                    ...
                    # デフォルト設定を作成
                    print(
                        f"⚠️ 設定ファイルが見つかりません。デフォルト設定を使用します: {self.config_path}"
                    )
    ...
    def _create_default_config(self) -> None:
        """デフォルト設定を作成"""
        self.config = {
            "jquants": {"api_base_url": "https://api.jquants.com", "timeout": 30},
            "data_fetch": {
