        ...
        for col in df_optimized.columns:
            if df_optimized[col].dtype == "object":
                ...
                if unique_ratio < 0.5:  # カーディナリティが低い場合
                    ...
                    logger.debug(
                        f"カテゴリカル変換: {col} (カーディナリティ: {unique_ratio:.2f})"
                    )
            ...
            if df_optimized[col].dtype in ["float64", "float32"]:
                ...
                if self.aggressive_mode:
                    ...
                    df_optimized[col] = pd.qcut(
                        df_optimized[col], q=100, duplicates="drop"
                    ).cat.codes.astype("int8")
                else:
                    # 標準的な量子化
                    df_optimized[col] = (
                        (df_optimized[col] * 100).round().astype("int16")
                    )
    ...
    def _generate_memory_recommendations(self) -> List[str]:
        ...
        if current_stats.usage_percent > 80:
            recommendations.append(
                "メモリ使用率が80%を超えています。データの分割処理を推奨します。"
            )

        if current_stats.objects_count > 100000:
            recommendations.append(
                "オブジェクト数が多すぎます。不要なオブジェクトの削除を推奨します。"
            )

        if self.optimization_stats["compression_ratio"] > 0.7:
            recommendations.append(
                "データ圧縮率が低いです。より積極的な最適化を推奨します。"
            )
...
def create_enhanced_memory_optimizer(
    memory_limit_mb: int = 1024, aggressive_mode: bool = True
...
if __name__ == "__main__":
    ...
    print(
        f"💾 最適化後メモリ: {optimized_data.memory_usage(deep=True).sum() / 1024 / 1024:.1f}MB"
    )
