...
"""
    ...
    async def test_complete_automation_execution(self, automation_system):
        ...
        result = await automation_system.run_complete_automation(
            analysis_types=["ultra_fast"], force_refresh=False
        )
...
class TestNotificationSystem:
    ...
    @pytest.mark.asyncio
    async def test_notification_content_generation(
        self, notification_system, sample_analysis_result
    ):
        """通知内容の生成テスト"""
        # 通知内容の生成
    ...
    async def test_notification_filtering(self, notification_system):
        ...
        # フィルタリングのテスト
        should_send_high = notification_system._should_send_notification(
            high_confidence_result
        )
        should_send_low = notification_system._should_send_notification(
...
class TestActionProposalSystem:
    ...
    @pytest.mark.asyncio
    async def test_action_proposal_generation(
        self, action_proposal_system, sample_analysis_context
    ):
        """アクション提案の生成テスト"""
        # 提案の生成
        proposals = await action_proposal_system.generate_action_proposals(
    ...
    @pytest.mark.asyncio
    async def test_proposal_filtering_and_ranking(
        self, action_proposal_system, sample_analysis_context
    ):
        """提案のフィルタリングとランキングテスト"""
        # 複数の提案を生成
        ...
        if proposals:
            ...
            assert (
                "critical" in priorities
                or "high" in priorities
                or "normal" in priorities
            )
...
class TestRetrySystem:
    ...
    async def test_successful_retry(self, retry_system, failing_operation):
        ...
        result = await retry_system.retry_operation(
            failing_operation, "test_operation", config
        )
    ...
    async def test_failed_retry(self, retry_system):
        ...
        config = RetryConfig(
            max_attempts=2, base_delay=0.1, strategy=RetryStrategy.FIXED
        )

        result = await retry_system.retry_operation(
            always_failing_func, "always_failing", config
        )
    ...
    async def test_retry_statistics(self, retry_system):
        ...
        await retry_system.retry_operation(
            lambda: "成功", "test_stats", RetryConfig(max_attempts=1)
        )
...
"""
        ...
        try:
            ...
            result = await automation_system.run_complete_automation(
                analysis_types=["ultra_fast"], force_refresh=True
            )
...
async def run_tests():
    ...
    try:
        ...
        await test_automation.test_automation_system_initialization(
            test_automation.automation_system(None)
        )

        # 通知システムのテスト
        logger.info("通知システムのテストを実行中...")
        notification_system = test_notification.notification_system()
        await test_notification.test_notification_system_initialization(
            notification_system
        )
        ...
        await test_action_proposal.test_action_proposal_system_initialization(
            action_proposal_system
        )
