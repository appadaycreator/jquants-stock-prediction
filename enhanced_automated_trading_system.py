...
class EnhancedAutomatedTradingSystem:
    ...
    def __init__(self, unified_system: UnifiedSystem = None, ai_prediction_system: EnhancedAIPredictionSystem = None):
        """初期化"""
        self.unified_system = unified_system or UnifiedSystem()
        self.ai_prediction_system = ai_prediction_system or EnhancedAIPredictionSystem(self.unified_system)
    ...
    def generate_trading_signal(self, symbol: str, data: pd.DataFrame, 
                              strategy: TradingStrategy = TradingStrategy.AI_PREDICTION) -> TradingSignal:
        """取引シグナルの生成"""
        try:
            signal_type = 'hold'
            strength = 0.0
            ...
            if strategy == TradingStrategy.AI_PREDICTION and self.config['ai_prediction_enabled']:
                # AI予測ベースのシグナル
                try:
                    # 最新のモデルを取得
                    ...
                    if model_names:
                        ...
                        prediction = self.ai_prediction_system.predict(latest_model, data)
            ...
            self.logger.info(f"📊 取引シグナル生成: {symbol} - {signal_type} (強度: {strength:.2f}, 信頼度: {confidence:.2f})")
    ...
    def calculate_position_size(self, symbol: str, price: float, strength: float) -> float:
        ...
        try:
            ...
            if self.config['risk_management_enabled']:
                ...
                max_position_value = max_risk_amount / (self.config['stop_loss_percentage'])
    ...
    def calculate_risk_metrics(self, symbol: str, price: float, quantity: float) -> Dict[str, float]:
        """リスクメトリクスの計算"""
        try:
            metrics = {}
    ...
    def place_order(self, signal: TradingSignal) -> Optional[Order]:
        ...
        try:
            ...
            order = Order(
                ...
                ai_prediction_id=signal.ai_prediction.prediction_id if signal.ai_prediction else None,
                risk_score=signal.risk_metrics.get('risk_percentage', 0.0) if signal.risk_metrics else 0.0,
                additional_info={
            ...
            self.logger.info(f"📝 注文実行: {order_id} - {signal.symbol} {signal.signal_type} {signal.quantity:.2f}@{signal.price:.2f}")
    ...
    def simulate_order_execution(self, order: Order) -> Order:
        ...
        try:
            ...
            order.commission = order.quantity * execution_price * self.config['commission_rate']
    ...
    def export_trading_data(self, file_path: str, format: str = 'json') -> bool:
        ...
        try:
            ...
            if format.lower() == 'json':
                ...
                # 注文データのCSV出力
                orders_df = pd.DataFrame([order.to_dict() for order in self.orders.values()])
                orders_df.to_csv(f"{file_path}_orders.csv", index=False, encoding='utf-8')
                
                # ポジションデータのCSV出力
                positions_df = pd.DataFrame([position.to_dict() for position in self.positions.values()])
                positions_df.to_csv(f"{file_path}_positions.csv", index=False, encoding='utf-8')
            else:
...
def main():
    ...
    try:
        ...
        # 取引シグナルの生成
        signal = trading_system.generate_trading_signal('TEST', data, TradingStrategy.AI_PREDICTION)
