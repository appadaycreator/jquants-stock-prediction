...
try:
    from parallel_processing_optimizer import (
        ...
        parallel_execute_optimized,
        parallel_map_optimized
    )
except ImportError:
...
class EnhancedParallelSystem:
    ...
    def optimize_data_processing(self, data_chunks: List[Any], 
        ...
        # 並列実行
        results = parallel_execute_optimized(tasks, task_type)
        
        logger.info(f"✅ データ処理並列最適化完了")
        return results
    
    def optimize_model_training(self, models_config: List[Dict[str, Any]], 
                               training_func: Callable,
                               task_type: str = "cpu_intensive") -> List[Any]:
        """
        モデル訓練の並列最適化
        ...
        # モデル訓練タスクを作成
        tasks = [lambda config=config: training_func(config) for config in models_config]
    ...
    def optimize_backtesting(self, strategies: List[Dict[str, Any]], 
        ...
        # バックテストタスクを作成
        tasks = [lambda strategy=strategy: backtest_func(strategy) for strategy in strategies]
    ...
    def optimize_sentiment_analysis(self, text_data: List[str], 
                                   analysis_func: Callable,
                                   task_type: str = "io_intensive") -> List[Any]:
        """
        感情分析の並列最適化
        ...
        # 並列マップ実行
        results = parallel_map_optimized(analysis_func, text_data, task_type)
        
        logger.info(f"✅ 感情分析並列最適化完了")
        return results
    
    def optimize_technical_indicators(self, stock_data: List[Dict[str, Any]], 
        ...
        # 並列マップ実行
        results = parallel_map_optimized(indicator_func, stock_data, task_type)
    ...
    def set_workers(self, workers: int):
        ...
        with self.optimizer.lock:
            ...
            self.optimizer.current_workers = max(1, min(workers, self.optimizer.max_workers_limit))
            logger.info(f"🔧 ワーカー数手動設定: {old_workers} → {self.optimizer.current_workers}")
