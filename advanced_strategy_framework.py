...
class AdvancedStrategyOptimizer:
    ...
    def __init__(
        self, strategies: List, data: pd.DataFrame, optimization_method: str = "genetic"
    ):
    ...
    def optimize_parameters(
        ...
        if self.optimization_method == "grid_search":
            ...
            return self._genetic_optimization(
                strategy, parameter_ranges, objective, max_iterations
            )
        else:
    ...
    def _grid_search_optimization(
        self, strategy, parameter_ranges: Dict[str, Tuple], objective: str
    ) -> Dict:
        """グリッドサーチ最適化"""
        best_params = {}
        ...
        for param_combination in itertools.product(*param_values):
            ...
            try:
                ...
                result = engine.run_backtest(
                    self.data, strategy, self.data.index[0], self.data.index[-1]
                )
    ...
    def _generate_initial_population(
        self, parameter_ranges: Dict[str, Tuple], population_size: int
    ) -> List[Dict]:
        """初期個体群生成"""
        population = []
    ...
    def _tournament_selection(
        self, population: List[Dict], scores: List[float], tournament_size: int = 3
    ) -> Dict:
        """トーナメント選択"""
...
class PortfolioOptimizer:
    ...
    def __init__(
        self, strategies: List, strategy_performances: List[StrategyPerformance]
    ):
        self.strategies = strategies
    ...
    def _maximize_sharpe_ratio(self) -> PortfolioWeights:
        ...
        expected_returns = np.array(
            [sp.annualized_return for sp in self.strategy_performances]
        )
        ...
        strategy_weights = {
            sp.strategy_name: weight
            for sp, weight in zip(self.strategy_performances, weights)
        }
...
class MultiStrategyManager:
    ...
    def _run_single_backtest(
        self, strategy, start_date: datetime, end_date: datetime
        ...
        try:
            ...
            if len(returns) > 1 and len(market_returns) > 1:
                beta = (
                    returns.corr(market_returns)
                    if not returns.corr(market_returns) != returns.corr(market_returns)
                    else 0
                )
            else:
                beta = 0
            ...
            # アルファ計算
            alpha = result.annualized_return - (
                0.02 + beta * (market_returns.mean() * 252 - 0.02)
            )
            ...
            sortino_ratio = (
                (result.annualized_return - 0.02) / downside_volatility
    ...
    def calculate_portfolio_performance(self) -> Dict[str, float]:
        ...
        weighted_return = sum(
            ...
            * self.optimized_weights.strategy_weights[sp.strategy_name]
        ...
        return {
            ...
            "diversification_ratio": (
                len(self.strategies)
                / sum(self.optimized_weights.strategy_weights.values())
                if self.optimized_weights.strategy_weights
                else 0
            ),
        }
    ...
    def generate_optimization_report(self) -> str:
        ...
        if self.optimized_weights:
            ...
            for (
                strategy_name,
                weight,
            ) in self.optimized_weights.strategy_weights.items():
