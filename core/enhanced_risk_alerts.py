...
import pandas as pd
import numpy as np
from .dynamic_risk_management import RiskLevel
...
class EnhancedRiskAlerts:
    ...
    def get_alert_summary(self) -> AlertSummary:
        ...
        try:
            ...
            emergency_alerts = len(
                [a for a in recent_alerts if a.severity == AlertSeverity.EMERGENCY]
            )
            ...
            return AlertSummary(
                ...
                top_risks=[
                    self._format_alert_for_display(alert) for alert in top_risks
                ],
            )
    ...
    def get_personalized_recommendations(
        ...
        try:
            ...
            # ユーザーのリスク許容度に基づく推奨事項
            risk_tolerance_multiplier = self._get_risk_tolerance_multiplier(
                user_risk_tolerance
            )
            ...
            for rec in recommendations:
                if user_risk_tolerance == "low":
                    rec["priority"] = (
                        "HIGH" if rec["priority"] == "MEDIUM" else rec["priority"]
                    )
                elif user_risk_tolerance == "high":
                    rec["priority"] = (
                        "MEDIUM" if rec["priority"] == "HIGH" else rec["priority"]
                    )
    ...
    def create_alert_notification(
        self, alert: RiskAlert, notification_methods: List[str] = None
    ) -> Dict[str, Any]:
        """アラート通知作成"""
        try:
            ...
            if portfolio_analysis["diversification_score"] < 0.3:
                alerts.append(
                    self._create_alert(
                        ...
                        metadata={
                            "diversification_score": portfolio_analysis[
            ...
            if portfolio_analysis["high_risk_ratio"] > 0.5:
                alerts.append(
                    self._create_alert(
                        ...
                        metadata={
                            "high_risk_ratio": portfolio_analysis["high_risk_ratio"]
                        },
                    )
                )
    ...
    def _create_alert(
        ...
        """アラート作成"""
        alert_id = (
            f"{symbol}_{alert_type.value}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        return RiskAlert(
            ...
            color_code=self.alert_settings.get("severity_colors", {}).get(
                severity.value, "#666666"
            ),
            icon=self.alert_settings.get("severity_icons", {}).get(
                severity.value, "info"
            ),
        )

    def _deduplicate_and_prioritize_alerts(
        ...
        try:
            ...
            # 優先度順にソート
            unique_alerts.sort(
                key=lambda x: self._get_alert_priority_score(x), reverse=True
            )
    ...
    def _cleanup_old_alerts(self):
        ...
        try:
            ...
            self.alert_history = [
                alert for alert in self.alert_history if alert.created_at >= cutoff_date
            ]
        ...
        try:
            ...
            for symbol, data in portfolio_data.items():
                ...
                risk_score = self._calculate_risk_score(
                    volatility, var_95, max_drawdown
                )
    ...
    def _calculate_risk_score(
        self, volatility: float, var_95: float, max_drawdown: float
    ) -> float:
        """リスクスコア計算"""
