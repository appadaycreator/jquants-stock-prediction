...
class SimplifiedRiskAPI:
    ...
    def get_simplified_risk_assessment(
        ...
        try:
            # データフレーム変換
            df = pd.DataFrame(stock_data) if isinstance(stock_data, dict) else stock_data
    ...
    def get_risk_alerts(self, portfolio_data: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """リスクアラート取得"""
        try:
    ...
    def get_visual_risk_display(self, portfolio_data: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """視覚的リスク表示データ取得"""
        try:
            display_data = self.risk_manager.get_visual_risk_display_data(portfolio_data)
            
            if not display_data:
                return {
                    "success": False,
                    "error": "表示データが取得できませんでした",
                    "data": None
                }
    ...
    def export_risk_report(self, portfolio_data: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """リスクレポートエクスポート"""
        try:
            # ポートフォリオリスクバランス取得
    ...
    def _get_risk_summary(self, balance) -> str:
        """リスクサマリー生成"""
        total_positions = balance.low_risk_count + balance.medium_risk_count + balance.high_risk_count
        ...
        elif balance.low_risk_count > balance.medium_risk_count + balance.high_risk_count:
            ...
            chart_data = {
                "risk_distribution_chart": {
                    ...
                    "data": [
                        ...
                        display_data["portfolio_balance"]["risk_distribution"]["medium"],
    ...
    def _validate_risk_settings(self, settings: Dict[str, Any]) -> bool:
        ...
        try:
            # 必須フィールドのチェック
            required_fields = ["risk_thresholds", "volatility_thresholds", "color_codes"]
            ...
            if not all(key in risk_thresholds for key in ["low_risk_max", "medium_risk_max", "high_risk_min"]):
                return False
