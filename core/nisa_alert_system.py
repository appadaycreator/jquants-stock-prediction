...
class NisaAlertSystem:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.alert_thresholds = {
            ...
            'accumulation_warning': self.config.get('accumulation_warning_threshold', 80.0),
            'accumulation_critical': self.config.get('accumulation_critical_threshold', 95.0)
        }
    ...
    def _check_growth_quota_alerts(self, quota_status: Dict[str, Any]) -> List[QuotaAlert]:
        """成長投資枠のアラートチェック"""
        try:
            alerts = []
    ...
    def _check_accumulation_quota_alerts(self, quota_status: Dict[str, Any]) -> List[QuotaAlert]:
        """つみたて投資枠のアラートチェック"""
        try:
            alerts = []
    ...
    def generate_investment_opportunities(self, market_data: List[Dict[str, Any]], 
                                        quota_status: Dict[str, Any]) -> List[InvestmentOpportunity]:
        """投資機会通知の生成"""
        try:
            opportunities = []
            
            # 成長投資枠の投資機会
            growth_opportunities = self._generate_growth_opportunities(market_data, quota_status)
            opportunities.extend(growth_opportunities)
            
            # つみたて投資枠の投資機会
            accumulation_opportunities = self._generate_accumulation_opportunities(market_data, quota_status)
    ...
    def _generate_growth_opportunities(self, market_data: List[Dict[str, Any]], 
                                     quota_status: Dict[str, Any]) -> List[InvestmentOpportunity]:
        """成長投資枠の投資機会生成"""
        try:
            opportunities = []
            ...
            for data in market_data:
                if data.get('recommendation_score', 0) > 0.7:  # 推奨スコアが高い場合
                    opportunity = InvestmentOpportunity(
                        ...
                        suggested_amount=min(growth_available * 0.1, 100000),  # 利用可能枠の10%または10万円
                        priority=AlertPriority.MEDIUM.value,
                        created_at=datetime.now().isoformat()
                    )
    ...
    def _generate_accumulation_opportunities(self, market_data: List[Dict[str, Any]], 
                                          quota_status: Dict[str, Any]) -> List[InvestmentOpportunity]:
        """つみたて投資枠の投資機会生成"""
        try:
            opportunities = []
            ...
            # つみたて投資に適した銘柄を抽出
            for data in market_data:
                if (data.get('risk_level') == 'LOW' and 
                    data.get('dividend_yield', 0) > 0.02):  # 低リスクで配当利回りが高い
                    opportunity = InvestmentOpportunity(
                        symbol=data.get('symbol', ''),
                        symbol_name=data.get('symbol_name', ''),
                        ...
                        suggested_amount=min(accumulation_available * 0.2, 20000),  # 利用可能枠の20%または2万円
                        priority=AlertPriority.MEDIUM.value,
                        created_at=datetime.now().isoformat()
                    )
                    opportunities.append(opportunity)
    ...
    def send_notification(self, alert: Any, notification_type: str = 'browser') -> bool:
        ...
        try:
            ...
            elif notification_type == 'browser' and self.notification_settings['browser']:
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_alerts = len([a for a in self.alert_history if a.type == AlertType.CRITICAL.value])
            warning_alerts = len([a for a in self.alert_history if a.type == AlertType.WARNING.value])
            info_alerts = len([a for a in self.alert_history if a.type == AlertType.INFO.value])
            
            # 最近のアラート（過去24時間）
            recent_alerts = [
                a for a in self.alert_history 
                if datetime.fromisoformat(a.created_at) > datetime.now() - timedelta(hours=24)
            ]
            ...
            return {
                ...
                'alert_rate': len(recent_alerts) / 24 if recent_alerts else 0,  # 時間あたりのアラート数
