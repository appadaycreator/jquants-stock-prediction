...
class EnsemblePredictionSystem:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.default_ensemble_method = self.config.get('ensemble_method', 'weighted_average')
        
        # 重み設定
        self.model_weights = self.config.get('model_weights', {})
        self.performance_based_weights = self.config.get('performance_based_weights', True)
        ...
        try:
            ...
            for model_name, model in self.models.items():
                try:
                    ...
                    self.logger.info(f"{model_name}の学習完了: R² = {model_performance[model_name].get('r2', 'N/A')}")
    ...
    def predict_ensemble(self, X: np.ndarray, method: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if method == 'weighted_average':
                ensemble_prediction = self._weighted_average_prediction(individual_predictions)
            elif method == 'stacking':
                ensemble_prediction = self._stacking_prediction(individual_predictions, X)
            elif method == 'voting':
            ...
            # 信頼度の計算
            confidence = self._calculate_ensemble_confidence(individual_predictions, ensemble_prediction)
    ...
    def _weighted_average_prediction(self, individual_predictions: Dict[str, np.ndarray]) -> np.ndarray:
        """重み付き平均による予測"""
        try:
            # 重みの計算
    ...
    def _stacking_prediction(self, individual_predictions: Dict[str, np.ndarray], X: np.ndarray) -> np.ndarray:
        """スタッキングによる予測"""
        try:
            # メタ特徴量の作成
            ...
            meta_learner.fit(meta_features, np.mean(list(individual_predictions.values()), axis=0))
    ...
    def _voting_prediction(self, individual_predictions: Dict[str, np.ndarray]) -> np.ndarray:
        """投票による予測"""
        try:
            # 各予測の平均
            ensemble_prediction = np.mean(list(individual_predictions.values()), axis=0)
    ...
    def _calculate_model_weights(self) -> Dict[str, float]:
        ...
        try:
            if self.performance_based_weights and self.model_performance:
                # 性能ベースの重み
                weights = {}
                total_score = sum(perf.get('score', 0) for perf in self.model_performance.values())
    ...
    def _calculate_ensemble_confidence(self, individual_predictions: Dict[str, np.ndarray], 
                                     ensemble_prediction: np.ndarray) -> float:
        """アンサンブル信頼度の計算"""
        try:
            # 予測の分散を計算
    ...
    def _calculate_uncertainty(self, individual_predictions: Dict[str, np.ndarray]) -> float:
        """不確実性の計算"""
        try:
            # 予測の標準偏差
            predictions_array = np.array(list(individual_predictions.values()))
    ...
    def evaluate_ensemble_performance(self, X_test: np.ndarray, y_test: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'improvement_over_best_individual': r2 - max(perf.get('r2', 0) for perf in individual_performance.values()),
    ...
    def optimize_ensemble(self, X_train: np.ndarray, y_train: np.ndarray, 
        ...
        try:
            ...
            for method in ensemble_methods:
                try:
                    # モデルの学習
                    training_result = self.train_ensemble_models(X_train, y_train, X_val, y_val)
            ...
            if method_performance:
                best_method = max(method_performance.keys(), 
                                key=lambda x: method_performance[x]['r2'])
