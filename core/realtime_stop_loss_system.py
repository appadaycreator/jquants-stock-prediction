...
class VolatilityCalculator:
    ...
    def calculate_volatility_regime(self, prices: List[float], short_period: int = 10, long_period: int = 30) -> str:
        """ボラティリティレジーム判定"""
        try:
    ...
    def calculate_dynamic_stop_loss(
        ...
        try:
            ...
            return entry_price * (1 - risk_percentage) if direction == 'BUY' else entry_price * (1 + risk_percentage)
    ...
    def calculate_dynamic_take_profit(
        ...
        try:
            ...
            return entry_price * (1 + 0.10) if direction == 'BUY' else entry_price * (1 - 0.10)
...
class RealtimeStopLossSystem:
    ...
    def add_stop_loss_setting(
        ...
        try:
            ...
            if risk_percentage is None:
                risk_percentage = self.config["risk_management"]["default_risk_percentage"]
            ...
            # ボラティリティレジーム判定
            volatility_regime = self.volatility_calculator.calculate_volatility_regime(prices)
            ...
            # 動的利確価格計算
            take_profit_price = self.volatility_calculator.calculate_dynamic_take_profit(
            ...
            self.logger.info(f"損切り設定を追加: {symbol}, 損切り価格: {stop_loss_price:.2f}, 利確価格: {take_profit_price:.2f}")
    ...
    def check_stop_loss_conditions(self, symbol: str, current_price: float) -> List[PriceAlert]:
        """損切り条件チェック"""
        try:
            alerts = []
            ...
            elif setting.direction == 'SELL' and current_price >= setting.stop_loss_price:
                stop_loss_triggered = True
            ...
            if setting.direction == 'BUY' and current_price >= setting.take_profit_price:
                take_profit_triggered = True
            elif setting.direction == 'SELL' and current_price <= setting.take_profit_price:
                take_profit_triggered = True
            ...
            if abs(price_change) > self.config["alerts"]["price_change_threshold"]:
                alert_level = AlertLevel.CRITICAL if abs(price_change) > 0.1 else AlertLevel.WARNING
                alert = PriceAlert(
                    timestamp=datetime.now(),
                    symbol=symbol,
    ...
    def execute_trade(self, symbol: str, trade_type: str, current_price: float) -> TradeExecution:
        """取引執行"""
        try:
            if symbol not in self.stop_loss_settings:
            ...
            self.logger.info(f"取引執行: {symbol}, {trade_type}, 価格: {current_price:.2f}, PnL: {unrealized_pnl:.2f}")
    ...
    def _process_price_update(self, price_update: Dict[str, Any]):
        ...
        try:
            ...
            if symbol in self.active_positions:
                ...
                self.active_positions[symbol]['unrealized_pnl'] = self._calculate_unrealized_pnl(
    ...
    def _process_alert(self, alert: PriceAlert):
        ...
        try:
            ...
            self.logger.info(f"アラート処理: {alert.symbol}, {alert.alert_type}, {alert.message}")
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        """監視状況取得"""
        try:
            active_positions = len([p for p in self.active_positions.values() if p.get('status') == 'ACTIVE'])
            total_alerts = len(self.alert_history)
            critical_alerts = len([a for a in self.alert_history if a.alert_level == AlertLevel.CRITICAL])
