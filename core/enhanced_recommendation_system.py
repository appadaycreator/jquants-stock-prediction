...
class EnhancedRecommendationSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.diversification_threshold = self.config.get(
    ...
    def _assess_portfolio_overall(
        ...
        try:
            ...
            sharpe_performance = (
                current_sharpe / target_sharpe if target_sharpe > 0 else 0
            )
            ...
            effective_stocks = diversification_metrics.get(
            ...
            return {
                ...
                "diversification_metrics": {
                    ...
                    "sector_diversification": diversification_metrics.get(
                        "sector_diversification", 0
                    ),
                    "correlation_diversification": diversification_metrics.get(
                        "correlation_diversification", 0
                    ),
                },
                "market_conditions": {
                    "market_stress": market_stress,
                    "volatility_regime": volatility_regime,
                    "trend_direction": market_conditions.get(
                        "trend_direction", "SIDEWAYS"
                    ),
                    "liquidity_level": market_conditions.get(
        ...
        try:
            ...
            for symbol, position_data in positions.items():
                ...
                priority = self._determine_priority(
                    weight_diff, confidence, position_data
                )
    ...
    def _determine_recommendation_type(
        self, weight_diff: float, position_data: Dict[str, Any]
    ) -> RecommendationType:
        """推奨タイプ決定"""
        try:
            if weight_diff > self.rebalancing_threshold:
                ...
                > position_data.get("cost_basis", 0) * 0.1
            ):
                return RecommendationType.TAKE_PROFIT
            elif (
                position_data.get("unrealized_pnl", 0)
                < -position_data.get("cost_basis", 0) * 0.05
            ):
    ...
    def _determine_priority(
        self, weight_diff: float, confidence: float, position_data: Dict[str, Any]
    ) -> PriorityLevel:
        """優先度決定"""
        try:
            # 重み差分による優先度
    ...
    def _calculate_expected_return(
        ...
        try:
            ...
            market_adjustment = 1.0 + (
                0.1 if trend == "BULL" else (-0.1 if trend == "BEAR" else 0.0)
            )
            volatility_adjustment = 1.0 + (
                -0.1 if regime == "HIGH" else (-0.2 if regime == "EXTREME" else 0.0)
            )
    ...
    def _determine_risk_level(
        self, position_data: Dict[str, Any], risk_metrics: Dict[str, Any]
    ) -> RiskLevel:
        """リスクレベル判定"""
        try:
            volatility = position_data.get("volatility", 0.2)
    ...
    def _determine_timeframe(
        self, recommendation_type: RecommendationType, priority: PriorityLevel
    ) -> str:
        """タイムフレーム決定"""
        try:
            if recommendation_type in [
        ...
        try:
            ...
            for symbol in set(current_weights.keys()) | set(target_weights.keys()):
                ...
                if abs(weight_diff) > self.rebalancing_threshold:
                    trade_value = portfolio_data.get("total_value", 0) * abs(
                        weight_diff
                    )
    ...
    def _generate_diversification_advice(
        self, diversification_metrics: Dict[str, Any], portfolio_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """分散投資アドバイス生成"""
        try:
            advice = []
            ...
            # 相関分散アドバイス
            correlation_score = diversification_metrics.get(
                "correlation_diversification", 0
            )
