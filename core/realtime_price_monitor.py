...
class TechnicalAnalyzer:
    ...
    def detect_breakout(self, prices: List[float], support: float, resistance: float) -> Dict[str, bool]:
        """ブレイクアウト検出"""
        try:
            if len(prices) < 2:
            ...
            # 上向きブレイクアウト
            upward_breakout = (current_price > resistance and previous_price <= resistance)
...
class RealtimePriceMonitor:
    ...
    def check_price_alerts(self, symbol: str, price_data: PriceData) -> List[PriceAlert]:
        """価格アラートチェック"""
        try:
            alerts = []
            ...
            # 価格変動アラート
            if abs(price_data.change_percent) > self.config["alerts"]["price_change_threshold"]:
                alert = PriceAlert(
                    timestamp=datetime.now(),
                    symbol=symbol,
                    ...
                    severity="HIGH" if abs(price_data.change_percent) > 0.1 else "MEDIUM",
                    metadata={
            ...
            if len(price_history) >= 2:
                ...
                if previous_volume > 0 and current_volume > previous_volume * self.config["alerts"]["volume_spike_threshold"]:
                    alert = PriceAlert(
                        timestamp=datetime.now(),
                        symbol=symbol,
                ...
                if volatility > self.config["alerts"]["volatility_threshold"]:
                    alert = PriceAlert(
                        ...
                        metadata={
                            ...
                            'volatility_threshold': self.config["alerts"]["volatility_threshold"]
                        }
                    )
                    alerts.append(alert)
                ...
                if support_resistance:
                    ...
                    if breakout['upward_breakout']:
                        alert = PriceAlert(
                            ...
                            alert_type=PriceAlertType.PRICE_BREAKOUT,
                            current_price=price_data.price,
                            threshold_price=support_resistance.get('resistance_level', 0),
    ...
    def create_market_snapshot(self, symbol: str, price_data: PriceData) -> MarketSnapshot:
        """市場スナップショット作成"""
        try:
            ...
            # 市場状況判定
            market_condition = self._determine_market_condition(volatility, trend_direction)
    ...
    def _determine_market_condition(self, volatility: float, trend_direction: str) -> MarketCondition:
        """市場状況判定"""
        try:
            if volatility > 0.3:
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_alerts = len([a for a in self.alert_history if a.severity == "HIGH"])
            ...
            return {
                ...
                "active_alerts": {symbol: len(alerts) for symbol, alerts in self.active_alerts.items()},
