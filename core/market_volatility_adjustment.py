...
class MarketVolatilityAdjustment:
    ...
    def calculate_dynamic_adjustment(
        ...
        try:
            ...
            # リバランス頻度調整
            rebalancing_frequency_adjustment = (
                self._calculate_rebalancing_frequency_adjustment(market_conditions)
            )
    ...
    def _detect_market_regime(self, market_data: pd.DataFrame) -> MarketRegime:
        ...
        if trend_ratio > 1 + trend_threshold:
            ...
            # ボラティリティによる判定
            volatility = self._calculate_market_volatility(market_data)
            vol_threshold = self.config["market_regime_detection"][
    ...
    def _calculate_regime_confidence(
        ...
        # ボラティリティの安定性
        vol_stability = self._calculate_volatility_stability(
            market_data, volatility_regime
        )

        confidence = (
            data_sufficiency * 0.3 + regime_consistency * 0.4 + vol_stability * 0.3
        )

        return max(0.0, min(1.0, confidence))

    def _calculate_regime_consistency(
        self, market_data: pd.DataFrame, market_regime: MarketRegime
    ) -> float:
        """レジーム一貫性計算"""
        if "Close" not in market_data.columns or market_data.empty:
            return 0.5
    ...
    def _calculate_volatility_stability(
        self, market_data: pd.DataFrame, volatility_regime: VolatilityRegime
    ) -> float:
        """ボラティリティ安定性計算"""
        if "Close" not in market_data.columns or market_data.empty:
            return 0.5
    ...
    def _calculate_stop_loss_adjustment(
        self,
        market_conditions: MarketConditions,
        stock_specific_factors: Optional[Dict[str, float]],
    ) -> float:
        """損切り調整計算"""
    ...
    def _calculate_rebalancing_frequency_adjustment(
