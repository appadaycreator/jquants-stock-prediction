...
class DeadlineManager:
    ...
    def get_approaching_deadlines(self, hours: int = 24) -> List[DeadlineInfo]:
        ...
        for deadline in self.deadlines.values():
            if (
                ...
                and deadline.deadline <= cutoff_time
            ):
    ...
    def get_expired_deadlines(self) -> List[DeadlineInfo]:
        ...
        for deadline in self.deadlines.values():
            if deadline.status == DeadlineStatus.PENDING and deadline.deadline < now:
    ...
    def get_deadline_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            completed_count = len(
                self.get_deadlines_by_status(DeadlineStatus.COMPLETED)
            )
    ...
    def cleanup_expired_data(self, days: int = 30) -> int:
        ...
        try:
            ...
            expired_actions = [
                action_id
                for action_id, deadline in self.deadlines.items()
                if deadline.status == DeadlineStatus.EXPIRED
                and deadline.updated_at < cutoff_date
            ]
            ...
            # 古いアラートの削除
            old_alerts = [
                alert for alert in self.alerts if alert.created_at < cutoff_date
            ]
