...
class ArticleMethodAnalyzer:
    ...
    def _run_article_backtest(self, data: pd.DataFrame, model, features: List[str]) -> Dict:
        """記事の手法のバックテスト"""
        # バックテスト期間の設定
        ...
        # 最終的な資産価値
        final_value = capital + (position * test_data.iloc[-1]['Close'] if position > 0 else 0)
...
class ImprovedMethodAnalyzer:
    ...
    def _implement_improved_method(self, data: pd.DataFrame) -> ImprovedMethodResult:
        ...
        # 特徴量とターゲットの準備
        features = [
            'Price_Change', 'Volume_Change', 'Price_MA5', 'Price_MA20',
            'RSI', 'MACD', 'BB_Upper', 'BB_Lower', 'ATR', 'Volatility'
        ]
        X = data[features]
        ...
        # 改善されたバックテスト実行
        backtest_result = self._run_improved_backtest(data, models, features, confidence_scores)
    ...
    def _calculate_confidence_scores(self, X_test: pd.DataFrame, models: Dict) -> np.ndarray:
        """信頼度スコアの計算"""
        # 各モデルの予測
        predictions = []
    ...
    def _calculate_improved_accuracy(self, y_true: pd.Series, y_pred: np.ndarray, confidence_scores: np.ndarray) -> float:
        """改善された精度計算"""
        # 信頼度70%以上の予測のみを考慮
    ...
    def _run_improved_backtest(self, data: pd.DataFrame, models: Dict, features: List[str], confidence_scores: np.ndarray) -> Dict:
        """改善されたバックテスト"""
        # バックテスト期間の設定
        test_start = int(len(data) * 0.8)
        test_data = data.iloc[test_start:].copy()
        ...
        for i in range(len(test_data) - 1):
            ...
            # 信頼度70%以上での取引判定
            if confidence >= 0.7:
                if predicted_price > current_price * 1.02 and position == 0:  # 2%以上の上昇予測
                    # 買いシグナル
                ...
                elif predicted_price < current_price * 0.98 and position > 0:  # 2%以上の下落予測
                    # 売りシグナル
        ...
        total_return = (final_value - initial_capital) / initial_capital
        total_trades = len(trades)
        winning_trades = sum(1 for trade in trades if trade['type'] in ['SELL', 'TAKE_PROFIT'])
        ...
        # プロフィットファクター
        profits = [trade for trade in trades if trade['type'] in ['SELL', 'TAKE_PROFIT']]
...
class MethodComparison:
    ...
    def compare_methods(self, data: pd.DataFrame) -> ComparisonResult:
        ...
        try:
            ...
            # 改善効果の計算
            improvement_metrics = self._calculate_improvement_metrics(article_result, improved_result)
