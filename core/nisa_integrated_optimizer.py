...
# 既存モジュールのインポート
from .nisa_quota_manager import NisaQuotaManager, NisaQuotaStatus, NisaTransaction, NisaPortfolio
...
class NisaIntegratedOptimizer:
    ...
    def get_comprehensive_optimization(self) -> NisaOptimizationResult:
        ...
        try:
            ...
            tax_calculation = self.tax_calculator.calculate_tax_savings(quota_data, portfolio_data)
            
            # 3. 最適化レポート生成
            optimization_report = self.optimization_system.generate_optimization_report(quota_data, portfolio_data)
    ...
    def _create_integrated_result(self, quota_status: NisaQuotaStatus, 
                                portfolio: NisaPortfolio,
                                tax_calculation: TaxCalculation,
                                optimization_report: OptimizationReport) -> NisaOptimizationResult:
        """統合結果の作成"""
        try:
            # 利用率の計算
            ...
            current_utilization_rate = (growth_utilization + accumulation_utilization) / 2
            
            # 改善ポテンシャルの計算
            improvement_potential = max(0, self.target_utilization_rate - current_utilization_rate)
            
            # 税務節約額の計算
            tax_savings_current = tax_calculation.tax_savings.get('estimated_tax_savings', 0)
            tax_savings_potential = tax_calculation.tax_savings.get('lifetime_savings', 0)
            
            # 最適化スコア
            optimization_score = optimization_report.current_score
            
            # 推奨事項の変換
            recommendations = [asdict(rec) for rec in optimization_report.recommendations]
            
            # AI戦略の変換
            ai_strategies = [asdict(strategy) for strategy in optimization_report.ai_strategies]
    ...
    def _calculate_efficiency_metrics(self, quota_status: NisaQuotaStatus,
                                    portfolio: NisaPortfolio,
                                    tax_calculation: TaxCalculation,
                                    optimization_report: OptimizationReport) -> Dict[str, Any]:
        """効率指標の計算"""
        try:
            # 基本効率指標
            growth_utilization = quota_status.growth_investment.get('utilization_rate', 0)
            ...
            return {
                ...
                'optimization_level': self._determine_optimization_level(overall_efficiency)
            }
    ...
    def _calculate_portfolio_efficiency(self, portfolio: NisaPortfolio) -> float:
        ...
        try:
            ...
            if portfolio.total_cost > 0:
                return_rate = (portfolio.unrealized_profit_loss / portfolio.total_cost) * 100
        ...
        try:
            ...
            return {
                'summary': {
                    ...
                    'optimization_level': result.efficiency_metrics.get('optimization_level', 'UNKNOWN')
                },
                'tax_benefits': {
    ...
    def execute_optimization_action(self, action_type: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """最適化アクションの実行"""
        try:
    ...
    def _execute_add_transaction(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            transaction = NisaTransaction(
                id=parameters.get('id', f'txn_{datetime.now().strftime("%Y%m%d_%H%M%S")}'),
                ...
                transaction_date=parameters.get('transaction_date', datetime.now().isoformat()),
    ...
    def _execute_optimize_allocation(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """配分最適化の実行"""
        try:
            # 配分最適化のロジック
