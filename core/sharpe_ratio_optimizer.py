...
class SharpeRatioOptimizer:
    ...
    def optimize_sharpe_ratio(
        ...
        try:
            ...
            if optimization_method == "max_sharpe":
                ...
                optimized_result = self._optimize_black_litterman(
                ...
                optimized_result = self._optimize_equal_risk_contribution(
                    processed_data
                )
            else:
                optimized_result = self._optimize_max_sharpe(processed_data)
            ...
            # 改善率計算
            improvement_ratio = (
                (float(optimized_result["sharpe_ratio"]) - float(original_sharpe))
                / float(original_sharpe)
                if float(original_sharpe) > 0
                else 0.0
            )
            # Pythonのbool型へ正規化
            target_achieved = bool(improvement_ratio >= self.target_improvement)

            # リスクメトリクス計算
            risk_metrics = self._calculate_risk_metrics(
                optimized_result, processed_data
            )
            ...
            return SharpeOptimizationResult(
                ...
                optimized_sharpe=float(optimized_result["sharpe_ratio"]),
    ...
    def _preprocess_portfolio_data(
        ...
        try:
            ...
            for symbol, position_data in positions.items():
                ...
                processed["liquidity_scores"].append(
                    position_data.get("liquidity_score", 0)
                )
    ...
    def _optimize_black_litterman(
        self, processed_data: Dict[str, Any], market_data: Optional[pd.DataFrame]
    ) -> Dict[str, Any]:
        """ブラック・リッターマンモデル最適化"""
        try:
            n_assets = len(processed_data["symbols"])
    ...
    def _calculate_black_litterman_returns(
        ...
        try:
            ...
            # ブラック・リッターマンの均衡超過リターン π = λ Σ w_mkt
            pi = risk_aversion * np.dot(cov_matrix, market_weights)

            # シンプルなブレンド: 期待リターンと均衡リターンの加重平均
            # テスト安定性のために非負にクリップ
            bl_returns = 0.5 * (expected_returns + pi)
            bl_returns = np.maximum(bl_returns, 0.0)

            return bl_returns.astype(float)
    ...
    def _optimize_max_sharpe_with_returns(
        self, processed_data: Dict[str, Any], expected_returns: np.ndarray
    ) -> Dict[str, Any]:
    ...
    def _calculate_risk_metrics(
        self, optimization_result: Dict[str, Any], processed_data: Dict[str, Any]
    ) -> Dict[str, float]:
        """リスクメトリクス計算"""
        try:
    ...
    def generate_optimization_recommendations(
        self, optimization_result: SharpeOptimizationResult
    ) -> Dict[str, Any]:
        """最適化推奨事項生成"""
        try:
            recommendations = {
