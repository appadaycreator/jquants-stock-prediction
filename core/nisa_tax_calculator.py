...
class NisaTaxCalculator:
    ...
    def _calculate_current_year_tax(
        self, quota_status: Dict[str, Any], portfolio: Dict[str, Any]
    ) -> Dict[str, Any]:
        """現在年度の税務計算"""
        try:
            growth_quota_used = quota_status.get("growth_investment", {}).get(
                "used_amount", 0
            )
            accumulation_quota_used = quota_status.get(
    ...
    def _calculate_next_year_tax(
        ...
        try:
            ...
            next_year_accumulation = (
                self.accumulation_annual_limit + accumulation_available
            )
    ...
    def _calculate_effective_tax_rate(
        ...
        try:
            ...
            if total_tax_free_amount > 0:
                # 非課税枠の活用率に基づく実効税率の調整
                utilization_rate = total_tax_free_amount / (
                    self.growth_annual_limit + self.accumulation_annual_limit
                )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "HIGH",
                        "potential_savings": (self.growth_annual_limit * 0.5)
                        * self.total_tax_rate,
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "MEDIUM",
                        "potential_savings": (self.accumulation_annual_limit * 0.5)
                        * self.total_tax_rate,
                    }
                )
    ...
    def _calculate_potential_tax_savings(
        self, quota_status: Dict[str, Any], portfolio: Dict[str, Any]
    ) -> float:
        """潜在的な税務節約額の計算"""
        try:
    ...
    def calculate_annual_tax_report(
        self, transactions: List[Dict[str, Any]], quota_status: Dict[str, Any]
    ) -> Dict[str, Any]:
        """年間税務レポートの計算"""
        try:
            # 年間取引の集計
            ...
            # 税務効果の計算
            tax_effectiveness = self._calculate_tax_effectiveness(
    ...
    def _summarize_annual_transactions(
    ...
    def _calculate_tax_effectiveness(
        self, annual_summary: Dict[str, Any], quota_status: Dict[str, Any]
    ) -> Dict[str, Any]:
        """税務効果の計算"""
        try:
            net_investment = annual_summary.get("net_investment", 0)
            total_tax_free_amount = quota_status.get("growth_investment", {}).get(
                "used_amount", 0
            ) + quota_status.get("accumulation_investment", {}).get("used_amount", 0)

            # 税務効果率の計算
            tax_effectiveness_rate = (
                (total_tax_free_amount / net_investment * 100)
                if net_investment > 0
                else 0
            )
    ...
    def _generate_annual_recommendations(
        self, annual_summary: Dict[str, Any], quota_status: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """年間推奨事項の生成"""
        try:
    ...
    def _calculate_optimization_metrics(
        ...
        try:
            ...
            # 目標達成率の計算
            target_achievement = (
                overall_utilization / self.target_utilization_rate
            ) * 100
    ...
    def _calculate_efficiency_score(
        ...
        try:
            ...
            # 総合効率スコア
            efficiency_score = (
                utilization_score * 0.4 + tax_score * 0.3 + portfolio_score * 0.3
            )
    ...
    def _calculate_portfolio_efficiency(self, portfolio: Dict[str, Any]) -> float:
        ...
        try:
            ...
            portfolio_efficiency = (
                diversification_score * 0.4 + risk_score * 0.3 + return_score * 0.3
            )
