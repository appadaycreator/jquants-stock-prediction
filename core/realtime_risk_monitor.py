...
class RealtimeRiskMonitor:
    ...
    def get_current_risk_status(self, symbol: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.is_monitoring or len(self.snapshot_history) > 0:
                # 統計計算
                total_alerts = (
                    sum(len(s.alerts) for s in recent_snapshots)
                    if recent_snapshots
                    else 0
                )
                critical_alerts = (
    ...
    def get_risk_alerts(
        self, symbol: str = None, level: AlertLevel = None
    ) -> List[RiskAlert]:
        """リスクアラート取得"""
        try:
            alerts = []
    ...
    def _check_risk_alerts(self, snapshot: RiskSnapshot) -> List[RiskAlert]:
        ...
        thresholds = self.config.get(
            "risk_thresholds", self._get_default_config()["risk_thresholds"]
        )
        ...
        # ポジションサイズチェック
        position_limits = self.config.get(
            "position_limits", self._get_default_config()["position_limits"]
        )
    ...
    def _generate_recommendations(
        self, snapshot: RiskSnapshot, alerts: List[RiskAlert]
    ) -> List[str]:
        """推奨事項生成"""
        recommendations = []
    ...
    def _add_to_history(self, snapshot: RiskSnapshot, alerts: List[RiskAlert]):
        ...
        if len(self.alert_history) > self.config["monitoring"]["alert_retention"]:
            self.alert_history = self.alert_history[
                -self.config["monitoring"]["alert_retention"] :
            ]
    ...
    def export_risk_report(self, symbol: str = None, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            if symbol:
                ...
                alerts = [a for a in self.alert_history if a.timestamp >= cutoff_date]
