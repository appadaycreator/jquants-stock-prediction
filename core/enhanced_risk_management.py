...
class EnhancedRiskManagement:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # リスク管理設定
        self.stop_loss_percentage = self.config.get(
    ...
    def calculate_stop_loss(
        self, entry_price: float, direction: str, volatility: float = 0.02
    ) -> float:
        """
        動的損切り価格の計算
        ...
        try:
            ...
            return (
                ...
                if direction == "BUY"
                else entry_price * (1 + self.stop_loss_percentage)
            )

    def calculate_take_profit(
        self, entry_price: float, direction: str, confidence: float = 0.7
    ) -> float:
        """
        動的利確価格の計算
        信頼度に基づく利確価格の調整
        ...
        try:
            ...
            return (
                ...
                if direction == "BUY"
                else entry_price * (1 - self.take_profit_percentage)
            )
    ...
    def update_position(self, symbol: str, current_price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"position": position, "risk_check": risk_check}
    ...
    def get_risk_summary(self) -> Dict[str, Any]:
        ...
        try:
            active_positions = [
                p for p in self.positions.values() if p["status"] == "ACTIVE"
            ]
            closed_positions = [
            ...
            total_unrealized_pnl = sum(
                p.get("unrealized_pnl", 0) for p in active_positions
            )
            ...
            risk_metrics = {
                ...
                "avg_confidence": np.mean(
                    [p.get("confidence", 0) for p in active_positions]
                )
                if active_positions
                else 0,
                "high_risk_positions": len(
                    [p for p in active_positions if p.get("volatility", 0) > 0.03]
                ),
            }
    ...
    def _calculate_portfolio_var(
        ...
        try:
            ...
            for position in positions:
                position_value = position.get("position_size", 0) * position.get(
                    "current_price", 0
                )
    ...
    def _calculate_portfolio_volatility(self, positions: List[Dict[str, Any]]) -> float:
        ...
        try:
            ...
            # 各ポジションのボラティリティの加重平均
            total_value = sum(
                p.get("position_size", 0) * p.get("current_price", 0) for p in positions
            )
    ...
    def _calculate_portfolio_sharpe_ratio(
        self, positions: List[Dict[str, Any]]
    ) -> float:
        """ポートフォリオシャープレシオ計算"""
        try:
    ...
    def _calculate_portfolio_risk_score(
        ...
        try:
            ...
            # 総合リスクスコア
            risk_score = (
                var_score + volatility_score + drawdown_score + sharpe_score
            ) / 4
    ...
    def set_individual_stock_limits(self, symbol: str, max_loss_amount: float) -> bool:
        ...
        try:
            ...
            if direction == "BUY":
                max_loss_price = entry_price - max_loss_amount / position.get(
                    "position_size", 1
                )
                position["stop_loss"] = max(
                    position.get("stop_loss", 0), max_loss_price
                )
            else:  # SELL
                max_loss_price = entry_price + max_loss_amount / position.get(
                    "position_size", 1
                ...
                position["stop_loss"] = min(
                    position.get("stop_loss", float("inf")), max_loss_price
                )
    ...
    def get_risk_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for position in active_positions:
                ...
                if max_loss_amount:
                    current_loss = abs(
                        (current_price - entry_price) * position.get("position_size", 0)
                    )
