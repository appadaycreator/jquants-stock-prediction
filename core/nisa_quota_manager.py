...
class NisaOptimizationEngine:
    ...
    def calculate_optimization_score(self, quota_status: NisaQuotaStatus) -> float:
        ...
        try:
            ...
            weighted_utilization = (
                growth_utilization * 0.8 + accumulation_utilization * 0.2
            )

            # 目標利用率との比較
            target_score = min(
                weighted_utilization / self.quota_manager.target_utilization_rate, 1.0
            )
    ...
    def get_optimization_recommendations(
        self, quota_status: NisaQuotaStatus
    ) -> Dict[str, Any]:
        """最適化推奨事項の取得"""
        try:
            recommendations = {
                "growth_quota": self._get_growth_quota_recommendations(quota_status),
                "accumulation_quota": self._get_accumulation_quota_recommendations(
                    quota_status
                ),
    ...
    def _get_growth_quota_recommendations(
        self, quota_status: NisaQuotaStatus
    ) -> Dict[str, Any]:
        """成長投資枠の推奨事項"""
        try:
    ...
    def _get_accumulation_quota_recommendations(
        self, quota_status: NisaQuotaStatus
    ) -> Dict[str, Any]:
        """つみたて投資枠の推奨事項"""
        try:
        ...
        try:
            ...
            return {
                ...
                "improvement_potential": max(
                    0, self.quota_manager.target_utilization_rate - overall_utilization
                ),
            }
...
class NisaQuotaManager:
        ...
        try:
            ...
            initial_data = {
                ...
                "settings": {
                    ...
                    "notifications": {"email": True, "browser": True, "push": False},
                },
            }
    ...
    def _calculate_optimization_metrics(self, quotas: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "improvement_potential": max(
                    0, self.target_utilization_rate - overall_utilization
                ),
            }
        ...
        try:
            ...
            if transaction.type == "BUY":
                if transaction.quota_type == "GROWTH":
                    available = quota_status.growth_investment.get(
                        "available_amount", 0
                    )
    ...
    def _update_utilization_rates(self) -> None:
        ...
        try:
            ...
            self.nisa_data["quotas"]["growth_investment"]["utilization_rate"] = (
                growth_used / growth_limit
            ) * 100

            # つみたて投資枠の利用率
            accumulation_used = self.nisa_data["quotas"]["accumulation_investment"][
                "used_amount"
            ]
            accumulation_limit = self.nisa_data["quotas"]["accumulation_investment"][
                "annual_limit"
            ]
            self.nisa_data["quotas"]["accumulation_investment"]["utilization_rate"] = (
                accumulation_used / accumulation_limit
            ) * 100
    ...
    def _add_or_update_position(self, transaction: NisaTransaction) -> None:
        ...
        try:
            ...
            for pos in positions:
                if (
                    pos["symbol"] == transaction.symbol
                    and pos["quota_type"] == transaction.quota_type
                ):
            ...
            for i, pos in enumerate(positions):
                if (
                    ...
                    if pos["quantity"] <= transaction.quantity:
                        ...
                        pos["average_price"] = (
                            pos["cost"] / pos["quantity"] if pos["quantity"] > 0 else 0
                        )
                    break
    ...
    def get_transactions(self, limit: int = 100) -> List[NisaTransaction]:
        ...
        try:
            ...
            # 最新の取引から取得
            recent_transactions = (
                transactions_data[-limit:] if limit > 0 else transactions_data
            )
    ...
    def get_quota_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_recommendations = (
                self.optimization_engine.get_optimization_recommendations(quota_status)
            )
    ...
    def _calculate_tax_optimization(
        ...
        try:
            ...
            accumulation_used = quota_status.accumulation_investment.get(
                "used_amount", 0
            )
            ...
            accumulation_available = quota_status.accumulation_investment.get(
                "available_amount", 0
            )
    ...
    def _calculate_target_achievement(
        ...
        """目標達成状況の計算"""
        try:
            overall_utilization = quota_status.optimization.get(
                "overall_utilization_rate", 0
            )
            target_utilization = quota_status.optimization.get(
                "target_utilization_rate", 90
            )

            achievement_rate = (
                (overall_utilization / target_utilization) * 100
            ...
            return {
                ...
                "completion_forecast": self._forecast_completion(
                    quota_status, remaining_days
                ),
            }
    ...
    def _forecast_completion(
        self, quota_status: NisaQuotaStatus, remaining_days: int
    ) -> str:
        """完了予測の計算"""
        try:
            overall_utilization = quota_status.optimization.get(
