...
class DiversificationScoringSystem:
    ...
    def calculate_diversification_score(
        ...
        try:
            # データ前処理
            processed_data = self._preprocess_portfolio_data(
            ...
            # 相関分散分析
            correlation_analysis = self._analyze_correlation_diversification(
                processed_data
            )
            ...
            # 地理的分散分析
            geographic_diversification = self._analyze_geographic_diversification(
                processed_data
            )

            # 時価総額分散分析
            market_cap_diversification = self._analyze_market_cap_diversification(
                processed_data
            )

            # 流動性分散分析
            liquidity_diversification = self._analyze_liquidity_diversification(
                processed_data
            )

            # リスク寄与分散分析
            risk_contribution_diversification = (
                self._analyze_risk_contribution_diversification(
            ...
            # 実効銘柄数計算
            effective_number_of_stocks = self._calculate_effective_number_of_stocks(
                processed_data
            )
    ...
    def _preprocess_portfolio_data(
        self, portfolio_weights: Dict[str, float], stock_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """ポートフォリオデータ前処理"""
        try:
            processed = {
            ...
            for symbol, weight in portfolio_weights.items():
                ...
                if price_data and len(price_data) > 1:
                    prices = [
                        float(p.get("close", 0)) for p in price_data if p.get("close")
                    ]
                    if len(prices) > 1:
        ...
        try:
            ...
            for sector in sector_weights.keys():
                ...
                if len(sector_indices) > 0:
                    # セクター内リターン計算
                    sector_return_data = [
                        processed_data["returns"][i]
                        for i in sector_indices
                        if len(processed_data["returns"][i]) > 0
                    ]
                    if sector_return_data:
                        sector_returns[sector] = np.mean(
                            [np.mean(r) for r in sector_return_data]
                        )
                        sector_volatilities[sector] = np.mean(
                            [np.std(r) * np.sqrt(252) for r in sector_return_data]
                        )
                    else:
                    ...
                    if len(sector_indices) > 1:
                        ...
                        for i in sector_indices:
                            for j in sector_indices:
                                if (
                                    i != j
                                    and len(processed_data["returns"][i]) > 0
                                    and len(processed_data["returns"][j]) > 0
                                ):
                                    corr = np.corrcoef(
                                        processed_data["returns"][i],
                                        processed_data["returns"][j],
                                    )[0, 1]
                                    if not np.isnan(corr):
                                        sector_corr_data.append(corr)
                        sector_correlations[sector] = (
                            np.mean(sector_corr_data) if sector_corr_data else 0.0
                        )
                    else:
            ...
            # セクターエントロピー計算
            sector_weights_normalized = sector_weights_array / np.sum(
                sector_weights_array
            )
            sector_entropy = entropy(sector_weights_normalized)
            max_entropy = np.log(len(sector_weights))
            sector_entropy_normalized = (
                sector_entropy / max_entropy if max_entropy > 0 else 0.0
            )
    ...
    def _analyze_correlation_diversification(
        ...
        try:
            ...
            # 相関エントロピー計算
            correlation_entropy = self._calculate_correlation_entropy(
                correlation_matrix
            )

            # 相関クラスタリング
            correlation_clusters = self._perform_correlation_clustering(
                correlation_matrix
            )
    ...
    def _calculate_correlation_entropy(self, correlation_matrix: np.ndarray) -> float:
        ...
        try:
            ...
            entropy_value = -np.sum(
                eigenvalues_normalized * np.log(eigenvalues_normalized + 1e-10)
            )
    ...
    def _analyze_concentration_risk(self, processed_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            concentration_risk = (
                herfindahl_index + max_weight + top5_concentration
            ) / 3.0
    ...
    def _analyze_geographic_diversification(
        self, processed_data: Dict[str, Any]
    ) -> float:
        """地理的分散分析"""
        try:
            ...
            region_weights_normalized = region_weights_array / np.sum(
                region_weights_array
            )
    ...
    def _analyze_market_cap_diversification(
        self, processed_data: Dict[str, Any]
    ) -> float:
        """時価総額分散分析"""
        try:
            ...
            category_weights_normalized = category_weights_array / np.sum(
                category_weights_array
            )
    ...
    def _analyze_liquidity_diversification(
        self, processed_data: Dict[str, Any]
    ) -> float:
        """流動性分散分析"""
        try:
    ...
    def _analyze_risk_contribution_diversification(
        self, processed_data: Dict[str, Any], correlation_analysis: CorrelationAnalysis
    ) -> float:
        """リスク寄与分散分析"""
        try:
            weights = np.array(processed_data["weights"])
    ...
    def _calculate_effective_number_of_stocks(
        self, processed_data: Dict[str, Any]
    ) -> float:
        """実効銘柄数計算"""
        try:
    ...
    def _calculate_diversification_ratio(
        ...
        try:
            ...
            equal_weight_volatility = np.sqrt(
                np.dot(
                    equal_weights,
                    np.dot(correlation_matrix, equal_weights * volatilities),
                )
            )

            # 実際のポートフォリオボラティリティ
            actual_volatility = np.sqrt(
                np.dot(weights, np.dot(correlation_matrix, weights * volatilities))
            )

            # 分散投資比率
            diversification_ratio = (
                equal_weight_volatility / actual_volatility
        ...
        try:
            recommendations = {
                "overall_assessment": {
                    ...
                    "grade": self._get_diversification_grade(
                        diversification_metrics.overall_score
                    ),
                    "status": self._get_diversification_status(
                        diversification_metrics.overall_score
                    ),
                },
                "detailed_analysis": {
                    "sector_diversification": {
                        ...
                        "recommendation": self._get_sector_recommendation(
                            diversification_metrics.sector_diversification
                        ),
                    },
                    "correlation_diversification": {
                        "score": diversification_metrics.correlation_diversification,
                        "recommendation": self._get_correlation_recommendation(
                            diversification_metrics.correlation_diversification
                        ),
                    },
                    "concentration_risk": {
                        "score": diversification_metrics.concentration_risk,
                        "recommendation": self._get_concentration_recommendation(
                            diversification_metrics.concentration_risk
                        ),
                    },
                },
                "action_items": [],
