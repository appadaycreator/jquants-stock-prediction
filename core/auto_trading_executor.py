    ...
    def update_trailing_stop(
        ...
        try:
            if position.direction == 'BUY':
                # 買いポジションの場合
                if position.max_profit_price is None or current_price > position.max_profit_price:
...
class AutoTradingExecutor:
    ...
    def create_position(
        ...
        try:
            ...
            self.logger.info(f"ポジション作成: {symbol}, {direction}, {quantity}@{entry_price}")
    ...
    def update_position_price(self, symbol: str, current_price: float):
        ...
        try:
            ...
            if self.config["trailing_stop"]["enabled"]:
                new_trailing_stop, should_execute = self.trailing_stop_manager.update_trailing_stop(
                    position, current_price
                )
                
                if should_execute:
                    self._create_stop_loss_order(symbol, new_trailing_stop, TradeType.TRAILING_STOP)
    ...
    def _check_stop_loss_take_profit(self, symbol: str, current_price: float):
        ...
        try:
            ...
            if self.config["partial_close"]["enabled"]:
                profit_target = position.entry_price * self.config["partial_close"]["profit_target_ratio"]
                partial_quantity = self.partial_close_manager.calculate_partial_close_quantity(
                    position, profit_target
                )
                
                if partial_quantity > 0:
                    self._create_partial_close_order(symbol, current_price, partial_quantity)
    ...
    def _execute_order(self, order: TradeOrder):
        ...
        try:
            ...
            if execution_result.status == ExecutionStatus.COMPLETED:
                ...
                self.logger.error(f"注文執行失敗: {order.order_id}, {execution_result.error_message}")
    ...
    def _simulate_order_execution(self, order: TradeOrder) -> ExecutionResult:
        ...
        try:
            ...
            # 手数料計算
            commission = order.quantity * executed_price * self.config["risk_management"]["commission_rate"]
    ...
    def _update_position_after_execution(self, order: TradeOrder, execution_result: ExecutionResult):
        """執行後のポジション更新"""
        try:
    ...
    def get_execution_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_orders = len([o for o in self.orders.values() if o.status == ExecutionStatus.PENDING])
            executing_orders = len([o for o in self.orders.values() if o.status == ExecutionStatus.EXECUTING])
    ...
    def get_performance_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_executions = [
                ...
                if e.execution_time >= cutoff_date
            ]
