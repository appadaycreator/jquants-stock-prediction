...
# 既存システムのインポート
from .clear_investment_actions import ClearInvestmentActions, InvestmentActionDetail, PositionInfo
from .advanced_quantity_calculator import AdvancedQuantityCalculator, QuantityCalculationResult
...
class EnhancedInvestmentDecisionSystem:
    ...
    def _preprocess_market_data(self, market_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """市場データの前処理"""
        try:
            processed_data = []
    ...
    def _generate_predictions(self, market_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """予測の生成"""
        try:
            # アンサンブル予測の実行
            ensemble_result = self.ensemble_prediction.run_ensemble_prediction(market_data)
    ...
    def _analyze_performance(self, decisions: List[InvestmentDecisionResult]):
        ...
        try:
            ...
            self.performance_metrics = {
                ...
                'risk_return_ratio': total_risk / abs(avg_expected_return) if avg_expected_return != 0 else 0
            }
    ...
    def export_decisions_to_json(self, decisions: List[InvestmentDecisionResult], filepath: str) -> bool:
        """投資判断のJSONエクスポート"""
        try:
            export_data = {
                'decisions': [],
