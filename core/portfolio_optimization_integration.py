...
# 統合システムのインポート
from .advanced_portfolio_optimizer import AdvancedPortfolioOptimizer, OptimizationResult, RiskMetrics
from .diversification_scoring_system import DiversificationScoringSystem, DiversificationMetrics
from .optimal_position_sizing_system import OptimalPositionSizingSystem, PositionSizingResult, MarketConditions
from .enhanced_recommendation_system import EnhancedRecommendationSystem, PortfolioRecommendation
...
class IntegratedPortfolioOptimizationSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 各システムの初期化
        self.portfolio_optimizer = AdvancedPortfolioOptimizer(self.config.get('portfolio_optimizer', {}))
        self.diversification_system = DiversificationScoringSystem(self.config.get('diversification', {}))
        self.position_sizing_system = OptimalPositionSizingSystem(self.config.get('position_sizing', {}))
        self.recommendation_system = EnhancedRecommendationSystem(self.config.get('recommendation', {}))
        self.sharpe_optimizer = SharpeRatioOptimizer(self.config.get('sharpe_optimizer', {}))
    ...
    def optimize_portfolio_comprehensive(
        ...
        try:
            ...
            diversification_metrics = self.diversification_system.calculate_diversification_score(
            ...
            for symbol, weight in optimization_result.weights.items():
                if weight > 0:
                    stock_data = self._get_stock_data(symbol, portfolio_data.get('stock_data', []))
                    if stock_data:
                        position_result = self.position_sizing_system.calculate_optimal_position_size(
                            symbol,
                            ...
                            portfolio_data.get('target_return'),
                            user_preferences.get('risk_tolerance', 'MEDIUM') if user_preferences else 'MEDIUM'
                        )
            ...
            risk_metrics = self._calculate_integrated_risk_metrics(optimization_result, portfolio_data)
            portfolio_recommendations = self.recommendation_system.generate_portfolio_recommendations(
                portfolio_data,
            ...
            system_performance = self._calculate_system_performance(start_time, datetime.now())
    ...
    def _get_stock_data(self, symbol: str, stock_data_list: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """銘柄データ取得"""
    ...
    def _convert_market_conditions(self, market_conditions: MarketConditions) -> Dict[str, Any]:
        """市場条件変換"""
        return {
    ...
    def _calculate_overall_score(
        ...
        try:
            ...
            sharpe_improvement_score = min(1.0, sharpe_optimization_result.improvement_ratio / 0.5)
    ...
    def _evaluate_improvement_achievement(
        ...
        try:
            ...
            return sharpe_improvement and optimization_convergence and confidence_achieved
            
        except Exception as e:
            self.logger.error(f"改善達成評価エラー: {e}")
            return False
    
    def _calculate_system_performance(self, start_time: datetime, end_time: datetime) -> Dict[str, Any]:
        """システムパフォーマンス計算"""
        try:
            ...
            return {
                ...
                'optimization_efficiency': self._calculate_optimization_efficiency(execution_time)
            }
    ...
    def generate_comprehensive_report(
        self,
        result: IntegratedPortfolioOptimizationResult
    ) -> Dict[str, Any]:
        """包括的レポート生成"""
        try:
            report = {
                'executive_summary': {
                    ...
                    'execution_time': result.system_performance['execution_time_seconds']
                },
                ...
                'recommendations': {
                    ...
                    'individual_recommendations_count': len(result.portfolio_recommendations.individual_recommendations),
                    'portfolio_actions_count': len(result.portfolio_recommendations.portfolio_actions),
                    'risk_warnings_count': len(result.portfolio_recommendations.risk_warnings),
                    'optimization_suggestions_count': len(result.portfolio_recommendations.optimization_suggestions)
                },
    ...
    def validate_optimization_results(
        ...
        try:
            ...
            if abs(total_weight - 1.0) > 0.01:
                validation_results['validation_errors'].append(f"ウェイト合計が1.0ではありません: {total_weight}")
                validation_results['validation_passed'] = False
            
            # 個別ウェイト検証
            for symbol, weight in weights.items():
                if weight < 0:
                    validation_results['validation_errors'].append(f"負のウェイト: {symbol} = {weight}")
                    validation_results['validation_passed'] = False
                elif weight > 0.5:
                    validation_results['validation_warnings'].append(f"高いウェイト: {symbol} = {weight}")
            
            # シャープレシオ検証
            if result.optimization_result.sharpe_ratio < 0:
                validation_results['validation_warnings'].append(f"負のシャープレシオ: {result.optimization_result.sharpe_ratio}")
            
            # 分散投資スコア検証
            if result.diversification_metrics.overall_score < 0.3:
                validation_results['validation_warnings'].append(f"低い分散投資スコア: {result.diversification_metrics.overall_score}")
