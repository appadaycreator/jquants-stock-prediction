...
class EnhancedAlertSystem:
    ...
    def _determine_notification_channels(self, alert_level: AlertLevel) -> List[NotificationChannel]:
        """通知チャネル決定"""
        channels = []
        
        # レベルに応じた通知チャネル選択
        if alert_level == AlertLevel.EMERGENCY:
            channels = [NotificationChannel.EMAIL, NotificationChannel.SLACK, NotificationChannel.WEB]
    ...
    def _send_notification(self, alert: Alert, channel: NotificationChannel):
        ...
        try:
            ...
            # 非同期で通知送信
            future = self.executor.submit(self._send_notification_async, notifier, alert, channel)
            
        except Exception as e:
            self.logger.error(f"通知送信エラー: {e}")
    
    def _send_notification_async(self, notifier, alert: Alert, channel: NotificationChannel):
        """非同期通知送信"""
        try:
            if channel == NotificationChannel.EMAIL:
                success = notifier.send_alert(alert, self.notification_settings.email_address)
            else:
    ...
    def get_alert_history(self, symbol: str = None, days: int = 7) -> List[Dict[str, Any]]:
        """アラート履歴取得"""
        try:
