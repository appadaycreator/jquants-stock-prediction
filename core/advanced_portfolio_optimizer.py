...
class AdvancedPortfolioOptimizer:
    ...
    def optimize_portfolio(
        ...
        try:
            ...
            # リターン・共分散行列計算
            returns_matrix, cov_matrix = self._calculate_returns_and_covariance(
                processed_data
            )

            # 期待リターン計算
            expected_returns = self._calculate_expected_returns(
                processed_data, returns_matrix
            )
            ...
            if optimization_method == "max_sharpe":
                ...
                result = self._optimize_mean_variance(
                    expected_returns, cov_matrix, target_return, max_risk
                )
            elif optimization_method == "black_litterman":
                result = self._optimize_black_litterman(
                    expected_returns, cov_matrix, market_data
                )
    ...
    def _preprocess_data(
        ...
        try:
            ...
            for stock in stock_data:
                ...
                # 流動性スコア計算
                volume_data = [
                    float(p.get("volume", 0)) for p in price_data if p.get("volume")
                ]
    ...
    def _calculate_returns_and_covariance(
        ...
        try:
            ...
            returns_matrix = np.array(
                [returns[:min_length] for returns in returns_list]
            )
    ...
    def _calculate_expected_returns(
        self, processed_data: Dict[str, Any], returns_matrix: np.ndarray
    ) -> np.ndarray:
        """期待リターン計算"""
        try:
            # 平均リターン計算
    ...
    def _validate_and_adjust_result(
        self, result: OptimizationResult, processed_data: Dict[str, Any]
    ) -> OptimizationResult:
        """結果検証・調整"""
        try:
            # ウェイト正規化
    ...
    def _verify_sharpe_improvement(self, result: OptimizationResult) -> bool:
        ...
        try:
            ...
            # 改善率計算
            improvement_ratio = (
                result.sharpe_ratio - baseline_sharpe
            ) / baseline_sharpe
            ...
            if improvement_achieved:
                ...
                self.logger.warning(
                    f"シャープレシオ改善未達成: {improvement_ratio:.2%} < {target_improvement:.2%}"
                )
    ...
    def calculate_risk_metrics(
        ...
        try:
            ...
            sortino_ratio = (
                (portfolio_return - self.risk_free_rate) / downside_volatility
            ...
            # カルマーレシオ計算
            calmar_ratio = (
                portfolio_return / abs(max_drawdown) if max_drawdown != 0 else 0
            )
