...
class DynamicRiskManager:
    ...
    def calculate_risk_metrics(
        ...
        try:
            ...
            # リスクレベル決定
            risk_level = self._determine_risk_level(var_95, max_drawdown, volatility)
    ...
    def calculate_dynamic_risk_adjustment(
        ...
        try:
            ...
            # セクターリスク調整
            sector_risk_adjustment = self._calculate_sector_risk_adjustment(sector_data)

            # 流動性調整
            liquidity_adjustment = self._calculate_liquidity_adjustment(stock_data)
            ...
            final_adjustment = (
                ...
                * self.config["dynamic_adjustments"]["volatility_sensitivity"]
                + sector_risk_adjustment
                * self.config["dynamic_adjustments"]["market_regime_weight"]
                + confidence_adjustment
                * self.config["dynamic_adjustments"]["confidence_weight"]
                + liquidity_adjustment
                * self.config["dynamic_adjustments"]["liquidity_weight"]
    ...
    def should_adjust_position(
        ...
        try:
            ...
            if (
                ...
                > self.config["risk_limits"]["max_drawdown_limit"]
            ):
                adjustments["should_reduce"] = True
            ...
            if previous_metrics:
                if (
                    current_metrics.risk_level.value
                    != previous_metrics.risk_level.value
                ):
    ...
    def _calculate_beta(
        ...
        try:
            ...
            if (
                market_variance == 0
                or np.isnan(covariance)
                or np.isnan(market_variance)
            ):
                return 1.0
    ...
    def _calculate_dynamic_position_size(
        ...
        return max(
            0.01, min(final_size, self.config["risk_limits"]["max_position_size"])
        )
    ...
    def _calculate_dynamic_stop_take(
        ...
        """動的損切り・利確計算"""
        # 基本損切り・利確
        stop_loss_config = self.config.get(
            "stop_loss", self._get_default_config()["stop_loss"]
        )
        take_profit_config = self.config.get(
            "take_profit", self._get_default_config()["take_profit"]
        )
    ...
    def _calculate_sector_risk_adjustment(
        self, sector_data: Optional[pd.DataFrame]
    ) -> float:
        """セクターリスク調整計算"""
