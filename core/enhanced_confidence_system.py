...
class EnhancedConfidenceSystem:
    ...
    def calculate_enhanced_confidence(
        ...
        try:
            ...
            # ファンダメンタル信頼度計算
            fundamental_confidence = self._calculate_fundamental_confidence(
                fundamental_data
            )

            # アンサンブル信頼度計算
            ensemble_confidence = self._calculate_ensemble_confidence(
                prediction_models, stock_data
            )
    ...
    def _calculate_base_confidence(self, stock_data: pd.DataFrame) -> float:
        ...
        try:
            ...
            # 重み付き平均
            base_confidence = (
                price_stability * 0.4 + volume_stability * 0.3 + data_completeness * 0.3
            )
    ...
    def _calculate_technical_confidence(
        ...
        try:
            ...
            # 移動平均信頼度
            ma_confidence = self._calculate_moving_average_confidence(
                technical_indicators
            )
    ...
    def _calculate_fundamental_confidence(
    ...
    def _calculate_ensemble_confidence(
        self, prediction_models: Dict[str, Any], stock_data: pd.DataFrame
    ) -> float:
        """アンサンブル信頼度計算"""
        try:
    ...
    def _calculate_final_confidence(
        ...
        try:
            weights = self.config.get(
                "risk_adjustment", self._get_default_config()["risk_adjustment"]
            )
    ...
    def generate_trading_signal(
        ...
        try:
            # 信頼度閾値チェック
            if (
                confidence_metrics.risk_adjusted_confidence
                < self.config["trading_threshold"]
            ):
                return None
    ...
    def _get_regime_adjustment(self) -> float:
        """レジーム調整係数"""
        adjustments = {"bull": 1.1, "bear": 0.9, "normal": 1.0}
    ...
    def _calculate_moving_average_confidence(
    ...
    def _calculate_valuation_attractiveness(
        self, fundamental_data: Dict[str, float]
    ) -> float:
        """バリュエーション魅力計算"""
        # 簡易的なバリュエーションスコア
    ...
    def _calculate_model_consistency(
        self, model: Any, stock_data: pd.DataFrame
    ) -> float:
        """モデル一貫性計算"""
        try:
            # 簡易的な一貫性計算
    ...
    def get_confidence_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            / len(confidences),
            "trading_eligible_ratio": sum(
                1 for c in confidences if c >= self.config["trading_threshold"]
            )
            / len(confidences),
        }
