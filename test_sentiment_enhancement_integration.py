...
class TestRealtimeSentimentMetrics(unittest.TestCase):
    ...
    async def test_generate_realtime_metrics(self, mock_ticker):
        ...
        # 指標の生成
        metrics = await self.metrics_generator.generate_realtime_metrics(self.test_symbols)
...
class TestSentimentTrendPrediction(unittest.TestCase):
    ...
    async def test_sentiment_trend_prediction(self, mock_ticker):
        ...
        # 予測の実行
        prediction = await self.predictor.predict_sentiment_trend(self.test_symbol, horizon=15)
    ...
    def test_trend_pattern_detection(self):
        ...
        # テスト用の履歴データを設定
        test_history = [
            {'sentiment_score': 0.1, 'volatility': 0.1, 'trend_strength': 0.1, 'risk_level': 0.1},
            {'sentiment_score': 0.2, 'volatility': 0.1, 'trend_strength': 0.1, 'risk_level': 0.1},
            {'sentiment_score': 0.3, 'volatility': 0.1, 'trend_strength': 0.1, 'risk_level': 0.1},
            {'sentiment_score': 0.4, 'volatility': 0.1, 'trend_strength': 0.1, 'risk_level': 0.1},
            {'sentiment_score': 0.5, 'volatility': 0.1, 'trend_strength': 0.1, 'risk_level': 0.1}
        ]
...
class TestIntegratedSentimentEnhancement(unittest.TestCase):
    ...
    def test_integrated_system_initialization(self):
        ...
        self.assertIsInstance(self.integrated_system, IntegratedSentimentEnhancementSystem)
    ...
    async def test_integrated_analysis(self, mock_ticker):
        ...
        # 統合分析の実行
        analyses = await self.integrated_system.perform_integrated_analysis(self.test_symbols)
...
class TestSystemIntegration(unittest.TestCase):
    ...
    async def test_end_to_end_integration(self):
        ...
        try:
            ...
            analyses = await integrated_system.perform_integrated_analysis(self.test_symbols)
            ...
            for analysis in analyses:
                ...
                logger.info(f"シンボル {analysis.symbol}: "
...
async def run_performance_test():
    ...
    try:
        ...
        if performance_stats and 'avg_processing_time' in performance_stats:
            logger.info(f"  平均処理時間（統計）: {performance_stats['avg_processing_time']:.2f}秒")
