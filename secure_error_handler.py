...
class SecureErrorHandler:
    ...
    def _sanitize_error_message(self, error_message: str) -> str:
        ...
        for pattern in sensitive_patterns:
            sanitized_message = re.sub(
                pattern, r"\1***MASKED***", sanitized_message, flags=re.IGNORECASE
            )
    ...
    def log_error(
        ...
        if self.error_context:
            error_info["context_data"] = self._sanitize_context_data(
                self.error_context.copy()
            )
    ...
    def handle_api_error(
        ...
        if retry_count < self.max_retries:
            self.logger.info(
                f"⏳ {self.retry_delay}秒後にリトライします (試行 {retry_count + 1}/{self.max_retries})"
            )
            return True  # リトライ可能
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if isinstance(e, (ValueError, TypeError, KeyError)):
                        ...
                        if retry_count < max_retries:
                            handler.log_error(
                                e,
                                f"{context or func.__name__} - リトライ {retry_count + 1}",
                            )
                            retry_count += 1
                            continue
                        else:
                            handler.log_error(
                                e,
                                f"{context or func.__name__} - 最大リトライ回数に達しました",
                            )
                            if reraise:
                                raise
                            return return_on_error
            ...
            final_error = Exception(
                f"関数 {func.__name__} が {max_retries} 回失敗しました"
            )
