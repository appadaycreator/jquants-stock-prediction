...
class PerformanceOptimizationTester:
    ...
    def _test_dataframe_processing(self) -> Dict[str, Any]:
        ...
        baseline_df = baseline_df.astype({f"col_{i}": np.float32 for i in range(n_cols)})
    ...
    def _test_technical_indicators(self) -> Dict[str, Any]:
        ...
        baseline_df["RSI"] = 100 - (100 / (1 + baseline_df["Close"].diff().rolling(window=14).mean()))
    ...
    def _generate_comprehensive_report(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            "test_summary": {
                ...
                "failed_tests": sum(1 for result in test_results.values() if not result.get("success", False))
            },
        ...
        for test_name, result in test_results.items():
            if result.get("success", False):
                if "time_improvement" in result:
                    report["performance_improvements"][f"{test_name}_time_improvement"] = result["time_improvement"]
                if "memory_improvement" in result:
                    report["performance_improvements"][f"{test_name}_memory_improvement"] = result["memory_improvement"]
                if "speedup" in result:
                    report["performance_improvements"][f"{test_name}_speedup"] = result["speedup"]
